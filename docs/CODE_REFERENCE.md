...
on-device client knows what LiDAR sees. AR configuration requests take a separate guarded path:
`SensorModeInterlock` returns allow/refuse/defer decisions before `AppModel` calls any `DepthEngine`
restart, and a deferred thermal mesh change is applied only after the route and any serialized
MultiCam teardown are finished.
...
### `SensorModeInterlock.swift` — route-time AR configuration safety policy
- **`enum SensorMode`** — `faceTracking`, `highFrameRate`, `meshClassification`; the AR-backed
  settings whose changes can restart the camera/depth session.
- **`enum SensorModeRequestSource`** — `user` or `thermal`.
- **`enum SensorModeDecision`** — `allowRestart`, `refuseWhileStarting`,
  `refuseWhileNavigating`, `refuseWhileFinishing`, `deferUntilRouteEnds`.
- **`struct SensorModeInterlock: Sendable, Equatable`** — pure phase machine owned by `AppModel`.
  `routeStartQueued()` reserves the configuration during the depth gate; user requests are refused
  in `.starting`, `.navigating` and `.finishing`; thermal requests are recorded for deferred
  application; and `routeEnded()` / `routeStartCanceled()` drains deferred modes in stable order.
  `routeFinishing()` keeps the policy protected while a serialized camera teardown drains. Pinned by
  `SensorModeInterlockTests` (idle permission, queued/active refusal, thermal deferral and release).
...
| `voiceInput` | `VoiceInputEngine` | 23 / 31 | Push-to-talk `SFSpeechRecognizer` using the `SpeechQueue.MicrophoneOwner.voiceInput` lease through `setMicrophoneEnabled(_:owner:)` (never `.allowBluetoothHFP`; rejected while sound recognition owns the input). Requests both permissions before activation, shows a cancellable pending-start state, and generation-fences recognition, route, interruption and permission callbacks; any failure drops partial text, restores playback/beacon and speaks an explicit error. Pure policy: `CaneKitLogic.VoiceInputGuard`; tests: `VoiceInputGuardTests.swift`. |
...
Logic state held as `@ObservationIgnored private var` value types (CaneKitLogic): `cueSpeech: CueSpeechPolicy` (which obstacle cues are spoken; replaced with a fresh value in `beginRoute`), `straightWalk: StraightWalkDetector` (auto-recenter trigger), `groundPolicy: GroundHazardPolicy` (when a confirmed ground hazard is spoken again; `reset()` in `beginRoute`) and `sensorModeInterlock: SensorModeInterlock` (refuses user AR configuration changes during route start/navigation/finishing and defers thermal mesh restarts). `depthSafetyDegraded` latches a failed/interrupted AR channel until a trusted frame returns, clearing stale obstacle haptics/names and notifying the walker. Other private flags: `isForeground` (false while backgrounded; gates `liveFrameJPEG`), `wasHot` (thermal notice on transitions only), `lastNavKind`, `recenterPending`, `ticker`, observer tokens.
...
| `commonLines` | 24 strings | pre-synthesised at start and route begin (includes route-readiness, cancellation, sensor-mode and danger-sound alerts) |
...
- **`scenePhaseChanged(_ phase: ScenePhase)`** — no-op until `started`.
  - `.active`: `isForeground = true`, `haptics.resume()`, `beacon.resumeIfNeeded()` (the audio engine can die across a screen lock without an interruption notification), then `resumeARKitAfterCameraWork()` waits for any serialized MultiCam teardown before `depth.resume()`, `hazards.start()` and optional face tracking. This ordering prevents ARKit and the two-camera session from competing for the same cameras.
  - `.inactive`: nothing.
  - `.background`: `isForeground = false`, `hazards.stop()` (no scanning a frozen last frame), **`sceneContext.set("")`** (LiDAR facts are stale once we come back), `depth.pause()` (stops gyro and drops the retained camera frame), `depthSafetyDegraded = true` for a running route, `haptics.stopAll()`, `decider.reset()`, `namer.reset()`, `activeCue = .clear`, `logger.flush()`.
...
0. If `depthSafetyDegraded` is latched by an AR failure/interruption, a report clears it only when
   the depth engine is running **and** that same frame has `.normal` tracking, valid depth and
   `isTrusted`; then "Obstacle detection is back." is sent through speech/watch. This avoids
   treating an "AR resumed" status or a stale queued frame after a terminal failure as recovery.
...
- **`beginRoute(_ route: Route)`** (private) — refuses a route while a debug sensor self-test is running (the delayed self-test cleanup must not restart ARKit under guidance), then ends a currently running route as before, refuses denied Location, turns off two-camera mode (serialized), and on a LiDAR phone with camera access queues the route in `pendingRouteStart` and says/sets `routeStartStatus` while `DepthReadiness` waits. `SensorModeInterlock.routeStartQueued()` reserves the AR configuration for this window, so face-tracking and 60-fps setting writes are refused until the request ends. `DepthEngine` reports same-frame `.normal` tracking + valid scene depth + sweep trust; after 3 consecutive reports the queued route enters `startRouteNow`, which moves the interlock to `.navigating`. A 5 s request deadline (including a two-camera transition that never drains) clears the request, sets `routeError = "Obstacle detection is not ready"`, sends a `.safety` spoken failure and leaves an explicit UI status. Stop or a newer destination cancels the queue. The documented degraded paths (no LiDAR or camera denied) still start GPS guidance after saying that obstacle warnings cannot work. `startRouteNow` then performs the existing prefetch → location → `nav.start` → beacon/head → recenter → ticker/trip/Live Activity → log/watch/channel sequence.
...
- **`stopRoute()`** — cancels any queued start, `nav.stop()` → releases `SensorModeInterlock` and applies deferred thermal mesh configuration while no route is active → `beacon.stop()` → `head.stop()` → `stopTicker()` → `Task { await trip.stop() }` → `liveActivity.end()` → `speech.stopAll()` (**queued waypoint lines must not play after Stop**) → `speech.say("Route stopped.", .nav)` → log `route {action: stop}` → `pushStatusToWatch()`.
...
- **`endRouteQuietly()`** (private; only from `beginRoute` when a route is already running) — `nav.stop()` → releases `SensorModeInterlock` and applies any deferred thermal mesh configuration while no route is active → `beacon.stop()` → `head.stop()` → `stopTicker()` → **`trip.cancel()`** (synchronous: an async `stop()` would still be tracking when the new `trip.start()` runs, and that start would be a no-op) → `liveActivity.end()` → `speech.stopAll()` → log `route {action: restart}`. Speaks nothing and leaves location running.
...
- `static let commonLines: [String]` — 24 fixed lines: `"OpenCane ready."`, `"Route started."`, `"Route stopped."`, `"Next."`, `"Recentered."`, `"Veer left."`, `"Veer right."`, `"GPS weak. Waypoint cues paused until it recovers."`, `"GPS back."`, `"No route running."`, `"No GPS fix yet. Try again outside."`, `"Head height."`, `"Left."`, `"Right."`, `"Passed one waypoint."`, `"Obstacle detection warming up. Route will start when it is ready."`, `"Route start canceled."`, `"Finish the sensor self-test before starting a route."`, `"Obstacle detection is back."`, `"Wait for the camera transition to finish."`, `"Sensor settings cannot change while a route is starting. Wait for obstacle detection to be ready."`, `"Sensor settings cannot change while a route is guiding you. Stop the route first."`, `"Siren. Do not start crossing."`, `"Horn nearby."`, `"Vehicle sound nearby."`. Must stay byte-identical to the strings spoken elsewhere (`NavigationEngine`, `CueSpeechPolicy`, `DangerSound`, this file) or the prefetch cache misses. (`"Route started."` and `"Next."` are not currently spoken standalone — the intro is `"Route started. <name>. First: …"`, prefetched separately.)
 - `static let commonLines: [String]` — 24 fixed lines: `"OpenCane ready."`, `"Route started."`, `"Route stopped."`, `"Next."`, `"Recentered."`, `"Veer left."`, `"Veer right."`, `"GPS weak. Waypoint cues paused until it recovers."`, `"GPS back."`, `"No route running."`, `"No GPS fix yet. Try again outside."`, `"Head height."`, `"Left."`, `"Right."`, `"Passed one waypoint."`, `"Obstacle detection warming up. Route will start when it is ready."`, `"Route start canceled."`, `"Obstacle detection warming up. Guiding with GPS."`, `"Finish the sensor self-test before starting a route."`, `"Obstacle detection is back."`, `"Wait for the camera transition to finish."`, `"Sensor settings cannot change while a route is starting. Wait for obstacle detection to be ready."`, `"Sensor settings cannot change while a route is guiding you. Stop the route first."`, `"Siren. Do not start crossing."`, `"Horn nearby."`, `"Vehicle sound nearby."`. Must stay byte-identical to the strings spoken elsewhere (`NavigationEngine`, `CueSpeechPolicy`, `DangerSound`, this file) or the prefetch cache misses. (`"Route started."` and `"Next."` are not currently spoken standalone — the intro is `"Route started. <name>. First: …"`, prefetched separately.)
...
- `updateThermal()` — maps state to `thermalName`; `hot = .serious || .critical` → the thermal request goes through `SensorModeInterlock`. While idle, `depth.setMeshClassification(!hot)` may restart ARKit; during route start/navigation, `depth.setMeshClassification(!hot, restartSession: false)` turns off processor mesh lookup immediately and defers the AR configuration restart until route end (cheapest downgrade without a depth gap). `hazards.paused = hot` (sign scans and the hazard watch stop; ARKit, lanes and haptics keep running). On a cool→hot transition after `started` it says `"Phone is hot. Door and wall names and sign reading paused."` (`.nav`, ttl 10) once (`wasHot` tracks the last state — Muse L5). Ground hazards are not paused by heat.
...
- `@ObservationIgnored var onReport: ((LaneReport) -> Void)?` — invoked on the main actor for every report; `AppModel.start()` sets it to `handle(report)` (the cue router). `onSessionFailure` and `onSessionInterruption` relay terminal/temporary AR health loss to `AppModel`, which clears stale cues and uses the existing speech/watch safety channels while GPS guidance continues.
...
- `setHighFrameRate(_ on: Bool)` — updates the camera format and the processor publish cap (30 or 60 Hz); in high-rate mode every camera frame is published so readiness cannot skip an untrusted frame. While running, drains the old delegate queue, invalidates readiness and re-runs the session without a tracking reset. `AppModel` refuses the setting write during route start/navigation through `SensorModeInterlock`.
- `setFaceTracking(_ on: Bool)` / `setMeshClassification(_ on: Bool, restartSession: Bool = true)` — update their configuration flag, drain the old delegate queue, invalidate readiness and, when running, re-run the session without reset options. Re-running costs ~1–2 s of depth; `AppModel` refuses face changes during route start/navigation and passes `restartSession: false` for thermal mesh changes during a route. `applyPendingConfigurationIfNeeded()` performs the deferred mesh restart after route end.
...
- `fileprivate sessionFailed(code:message:)` — maps `ARError.Code` (`.cameraUnauthorized` → "Camera access denied — enable it in Settings"; `.sensorUnavailable`/`.sensorFailed` → "LiDAR sensor unavailable"; `.unsupportedConfiguration` → "Unsupported AR configuration"; else raw message) into "AR error: …", sets `isRunning = false` and calls `onSessionFailure`.
- `fileprivate sessionInterrupted(_:)`, `fileprivate trackingChanged(_:)` — status/tracking strings; an interruption invalidates an active route-start freshness run and calls `onSessionInterruption`. `SessionObserver` still carries only Sendable values across the actor hop.
...
- **`make test`** → `ios/scripts/test.sh` → `swift test` in `ios/Logic` (567 current test cases). Tests touching this module: `ios/Logic/Tests/CaneKitLogicTests/VLMCodecTests.swift` (request/response codecs, `SpokenDistance`), `HazardTests.swift` (sign phrases, hazard-watch replies), `SoundAlertsTests.swift` (sound alert thresholds plus the recognition lifetime guard), `VoiceInputGuardTests.swift` (push-to-talk route/permission/recognizer lifecycle), `SensorModeInterlockTests.swift` (route-time AR configuration refusal/deferral) and `NavSupportTests.swift` (`headHeightIsSpokenOncePerEpisode`, `headEpisodesAreRateLimitedAcrossEpisodes`, `sideCuesAreSpokenOnlyWhenThePhoneCannotBuzz` — which cues `AppModel` hands to `SpeechQueue`; `straightWalkNeedsThreeSteadyFixesCountingTheFirst`, `straightWalkRestartsOnATurnAStopOrAHeadTurn` — when `HeadPoseTracker.recenter()` auto-fires; the `settle…`/`crossingSilencesTheBeaconAndReleasesAtTheCurb` tests — what bearing reaches the beacon). Nothing in `SpeechQueue`, `BeaconEngine`, `AudioRouteMonitor`, `HeadPoseTracker`, `ObstacleNamer`, `HazardScanner` or `OnDeviceVision` is unit-tested — they are device-only (or simulator-only via FrameReplay).
...
### `ios/CaneKit/Conversation/VoiceInputEngine.swift` — push-to-talk capture

`VoiceInputEngine` is the `@MainActor` adapter for `CaneKitLogic.VoiceInputGuard`. It requests
microphone and Speech permissions before touching the shared audio session, marks a pending
permission/graph negotiation as `isStarting` (the Guide card can cancel it), then leases
`SpeechQueue.MicrophoneOwner.voiceInput`. While active it receives the queue's complete
route-change and interruption stream, observes `AVAudioEngineConfigurationChange`, polls both
permissions every `VoiceInputGuard.permissionPollInterval`, and fences every callback with
a monotonically increasing run generation. A route/output/input change, HFP or missing input,
interruption, recognizer error, stopped engine or permission loss calls the guard exactly once;
teardown removes the tap, notification/task tokens and recognition request, restores `.playback`
and the beacon, discards any partial transcript and speaks the short failure through `SpeechQueue`.
Only a finalized non-empty transcript reaches `onTranscriptionFinalized`; a late callback from a
canceled run is ignored. Tests: `ios/Logic/Tests/CaneKitLogicTests/VoiceInputGuardTests.swift`.
...
- `setNavigating(_:)` — arms or invalidates `CLBackgroundActivitySession` and switches `liveUpdates` configuration between `.otherNavigation` (during active route) and default (idle), preventing the system navigation indicator from anchoring 24/7.
