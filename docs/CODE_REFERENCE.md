...
- `LaunchRecoveryTests.swift` (6) — `LaunchRecovery.mode`, `optionalFeatureKeys`, `spokenLine`, `healthySeconds`.
...
- `VoiceEngineChoiceTests.swift` (16, Steps 53–54) — the engine decision table, pinned raw values, the 2.5 s deadline, `resolvedEngine`, `describe`, and the session-sticky `VoiceBreaker`.
...
- **`scenePhaseChanged(_ phase: ScenePhase)`** — no-op until `started`.
  ...
  - `.inactive`: nothing.
  - `.background`: `hazards.stop()` (no scanning a frozen last frame), **`sceneContext.set("")`** (LiDAR facts are stale once we come back), `depth.pause()` (stops gyro and drops the retained camera frame), `haptics.stopAll()`, `decider.reset()`, `namer.reset()`, `activeCue = .clear`, `logger.flush()`.
