...
- **`announceCameraDenied()`** (private; from `start()` and `beginRoute`) — only for `AVCaptureDevice.authorizationStatus(for: .video)` `.denied` / `.restricted` (`.notDetermined` is fine, ARKit prompts): `routeError = "Camera is off for CaneKit"` and `speech.say("Camera access is off, so obstacle warnings cannot work. Turn on Camera for CaneKit in Settings.", .nav, ttl: 20)`. Returns `true` when refused (`@discardableResult`): `start()` then skips "CaneKit ready."; `beginRoute` clears `routeError` first, then calls it, and **still guides** (GPS, beacon and watch work without the camera). The spoken warning is rate-limited to once a minute (`cameraDeniedSpokenAt`) so launch + route start do not queue it twice; the on-screen error is set every time.
...
Purpose: root screen — a `NavigationStack` > **`ScrollViewReader`** > `ScrollView` > `VStack(spacing: xl)` stacking, in order: `GuideCard(scroller: proxy)`, `ArrivalCardView()` (only if `model.nav.isNavigating || model.nav.arrived`), `statusCard`, `ObstaclesCard()` (thin wrapper passing `model.depth.report` to `LaneGridView` so `ContentView` avoids 30 Hz re-evaluation), `HapticsCard()`, **`HazardsCard()`**, `WatchCard()`, `mountSettings($model)` (with `MountAimRow()` isolated), `capabilityCard`. Padding `gutter`; background `CKColor.background`; `.navigationTitle("CaneKit")`. A `.background(CameraControlInteraction { model.cameraControlPressed() })` (in `CaneKit/Scene/CameraControlInteraction.swift`) routes Camera Control / volume presses to "Where am I" (the press counter is no longer shown). The card order is the VoiceOver focus order.
...
- Keyboard (the destination field is the app's only text input, Step 14): the `ScrollViewReader`'s proxy is handed to `GuideCard` → `DestinationField`, which scrolls itself to the top (`DestinationField.anchorID`) when it takes focus, so the card and its suggestion list are never behind the keyboard. The scroll view is `.scrollDismissesKeyboard(.interactively)`; the content `VStack` has `.contentShape(Rectangle()).onTapGesture { dismissKeyboard() }` so a tap on anything that is not a control gives the keyboard back (buttons, toggles and the field consume their own taps first).
- `private func dismissKeyboard()` — `UIApplication.shared.sendAction(#selector(UIResponder.resignFirstResponder), to: nil, …)`. Resigning the first responder updates `DestinationField`'s `@FocusState`, so no focus binding has to be threaded through two views.
...
- `private struct ObstaclesCard: View` — thin wrapper reading `model.depth.report` from `@Environment(AppModel.self)` and forwarding it to `LaneGridView(report:)`, isolating the 30 Hz observation dependency from `ContentView.body`.
...
- Body inside `CKCard(title: "Guide")`:
  ...
  - Else (idle):
    ...
    - `"Start route to CIF"` (`figure.walk`, primary, hint "Starts the recorded ISR Townsend Hall to CIF route" → `model.startDemoRoute()`).
...
Runs the `CaneKitLogic` Swift Testing suite from `ios/Logic` (243 tests; per-file counts are in the Tests section of the logic module above — CueDecider 14, GeoMath 20, NavSupport 19, LaneMath 12, Route 4, VLMCodec 8, WatchMessage 3, Hazard 25, CourseSmoother 3, DestinationSuggestions 14, plus CampusPlaces, LiveView, SceneVocabulary and TripLogRecord). If `xcode-select -p` points at `Xcode.app`: `exec swift test "$@"`. Otherwise (Command Line Tools only) adds `-Xswiftc -Fsystem <CLT Frameworks>`, `-disable-cross-import-overlays`, and linker `-F`/`-rpath` so Swift Testing links (tests use only core Testing + Foundation).
