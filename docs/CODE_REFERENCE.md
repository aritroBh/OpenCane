...
- `enum VoiceInputState: Equatable, Sendable { idle, listening, recognizing(partialText:), processing, error(String) }` — published as `state`; no view reads it (the UI keys off `isListening` and `ConversationCoordinator.isProcessing`).
- `@MainActor @Observable final class VoiceInputEngine` — published `state`, `isListening`, `latestTranscript`; callbacks `onTranscriptionFinalized: ((String) -> Void)?` (→ `ConversationCoordinator.handleQuery`, then `finishProcessing()`), `shouldRestorePlaybackSession: (() -> Bool)?` (false while `SoundWatcher` owns the microphone), `onEvent: ((String, [String: Any]) -> Void)?` (→ trip log: `logStart` / `logEnd`, `voice_end` carries the reason). Methods: `startListening()`, `stopListeningAndSubmit(reason: "press" | "silence" | "timeout")`, `finishProcessing()`, `cancel()`; private `startEngine(attempt:sessionField:)` (format retry, one `UtteranceEndDetector` per session, the end ticker), `recognitionUpdate(text:isFinal:error:)`, `checkEnd()`, `cleanupAudioPipeline()`, `denied(speech:)`, `fail(with:)`.
