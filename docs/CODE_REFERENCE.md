...
Contract type: `NavActivityAttributes` (`ios/Shared/LiveActivity/NavActivityAttributes.swift`, `nonisolated public struct … : ActivityAttributes, Sendable`, shared with the widget target): `routeName: String` and a seven-field `ContentState` (`instruction`, `distanceM`, `kind`, `obstacleStatus`, `obstacleDistanceM`, `headClearanceM`, `statusDetail`) — see the `NavActivityAttributes.swift` section under `watch-widget-shared` for the fields and the tolerant decoder. `kind` ∈ `"turnLeft" | "turnRight" | "crossing" | "arrived" | "straight"` (picks the glyph in `CaneKitWidget/NavLiveActivity.swift`; `AppModel.lastNavKind` feeds it from `NavCue.rawValue`, reset to `"straight"` at `beginRoute`; a passed-by advance sends no cue, so the kind stays unchanged).

**`final class LiveActivityController`** — `@MainActor @Observable`. Published: `isActive`, `lastError`. Private: `activity: Activity<NavActivityAttributes>?`, `lastState`, and a serialized `activityOperation` task used to order every update/end operation so an old update cannot revive a finished route.

| Constant | Value |
|---|---|
| Update coalescing | decided by `LiveActivityCoalescer.shouldEmit` (CaneKitLogic) — distance bands 2 / 5 / 10 m by range, immediate on hazard transitions, 0.8 s floor; see "Live Activity update coalescing" under `watch-widget-shared` |
...
- `update(instruction:distanceM:kind:)` — no-op without an activity or when coalesced; otherwise records `lastState` and appends the ActivityKit update to the serialized `activityOperation` chain. The chain keeps an older update from completing after `end()` and reviving stale route information. Called on every fix while navigating.
...
### `ios/CaneKitWidget/NavLiveActivity.swift` — lock screen + Dynamic Island (redesigned Step 47)

`struct NavLiveActivity: Widget` — `ActivityConfiguration(for: NavActivityAttributes.self)`. No buttons by design (phone on a cane is glanced at, not touched). Colours are the file-private `Palette` (ink ground, ivory text, glance tints green / orange / red / purple) so the widget has no dependency on `Theme.swift`. Redesigned from the first `make island` pictures (docs/design.md §6.7 lists what was wrong).

Private tables (one place for glyph, word and VoiceOver, so they can never disagree):
- `struct Manoeuvre(kind:)` — `symbol` / `word` / `spoken` / `tint` per `kind`: `turnLeft` → `arrow.turn.up.left` "Turn left"; `turnRight` → `arrow.turn.up.right` "Turn right"; `crossing` → `figure.walk.diamond.fill` "Crossing" (yellow); `arrived` → `flag.checkered` "Arrived" (green); anything else → `figure.walk` "Straight on" (`arrow.up` retired: it read as a second copy of the OS's blue location arrow). ⚠ Add a case when `AppModel.lastNavKind` gains a value.
- `struct Glance(status:distanceM:headM:)` — `symbol` / `tint` / `word` (pill: "Path clear", "Obstacle 1.2 m", "Head height 1.4 m", "Drop-off ahead") / `compact` (bubble: "" when clear, metres / "HEAD" / "CURB") / `spoken`.
- `NavLiveActivity.summary(_:routeName:isStale:)` — the one VoiceOver sentence every presentation reads ("OpenCane, on route …: in 120 meters, turn right. <instruction> Path is clear." + "No update for a while." when stale).

Views: `LockScreenCard` (glyph 30 pt over its word in a 64 pt column · instruction `.headline` ≤ 2 lines over `GlancePill` (`layoutPriority(1)`) + route name, then `RouteProgressBar` · `DistanceText` 28 pt rounded heavy + "to next" / "arrived" / "No update"), `DistanceText(metres:isStale:)` ("N m" / "N.N km" / "—" for 0; dimmed when `context.isStale`), `GlancePill`, `FactPill` ("±5m GPS"), `RouteProgressBar(progress:tint:)` (8 pt capsule track, tinted fill, ivory dot at the head; `accessibilityHidden` — the sentence carries the distance).
Dynamic Island: expanded `.leading` = glyph + word, `.trailing` = distance + caption, `.bottom` = instruction full width (2 lines, `fixedSize`) then one row `GlancePill` (`layoutPriority(1)`) + `RouteProgressBar` filling the rest — no route name and no GPS pill in the island (the row truncated on the phone, a third row fell below the clip); `statusDetail` joins the trailing caption via `trailingCaption(_:isStale:)` ("to next · ±5m GPS"); `compactLeading` = glyph + distance; `compactTrailing` = glance glyph, plus `compact` text only when not clear; `minimal` = glance glyph when not clear else the manoeuvre glyph. Every region carries `summary` or the glance sentence as its accessibility label.
Pictures: `make island` (`CaneKitIslandTour`) → `NN-island-{compact,expanded,walking,after-stop}.png`.
