...
`torchEnabled: Bool = false` (`private(set)`, **not** persisted — pocket-heater risk) is the measured back-camera torch state (`isTorchActive` read back, never the request); HazardsCard "Flashlight" binds through `setTorch(_:)`, which works mid-route, with ARKit alive, and inside both-cameras mode (device-level, no session touched), speaks the outcome, and logs `torch {action, active}`.
