...
the engine then clears distance/bearing/course evidence and speaks the existing GPS-weak line until
a fresh fix arrives. Tests: `NavigationHealthTests.swift` (fresh, boundary, invalid/future clocks).
...
Other triggers into the same paths: `describeScene(trigger: DescribeTrigger = .button)` (logs `describe {provider, trigger}` then `describer.describe(trigger:)`; `.button` from `GuideCard`, `.watch` from `handleWatchCommand`, `.actionButton` from `WhereAmIIntent`, `.cameraControl` from `cameraControlPressed`, `.waypoint` from the `describeEveryWaypoint` hook — Step 47), `cameraControlPressed()` (`describeScene(trigger: .cameraControl)` — one `describe {trigger: cameraControl}` record per press since the Step 47 review; the old separate `source` record is gone), debug `watchTest(_ cue: NavCue)` (`watch.send(nav:)`), `speechTest()` (a `.scene` line then an `.obstacle` line to prove interrupt ordering).
...
- Attached once in `ContentView`: `.background(CameraControlInteraction { model.cameraControlPressed() })`; `AppModel.cameraControlPressed()` calls `describeScene(trigger: .cameraControl)`, whose `describe {trigger: cameraControl}` record is how the trip log shows whether presses reach the app under ARKit.
