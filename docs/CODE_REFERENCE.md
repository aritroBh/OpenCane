...
**CueDeciderTests.swift** (15; helper `report(head:torso:trusted:)` builds a trusted, depth-available report)
...
- `VoicePrefetchTests.swift` (5) — `VoicePrefetch.maxConcurrent` (2), `isFatal` (401 / 403 / 422 only), `queue` keeps order and drops cached lines.
...
- Every answer is spoken at `.scene` (lowest band) with `immediate: true` (Step 31: novel text never hits the natural-voice cache, so the system voice speaks at once). Effects that announce themselves (`setHapticsSilenced`, `setOption`, `stopRoute`, `navigate(to:)`) are not echoed (double-speak). Every turn is logged: `conv_turn` / `conv_error` / `marker_dropped`. The JPEG is encoded in a detached task.
...
Purpose: `SFSpeechRecognizer` (on-device when supported, Apple's server otherwise) on the iPhone's own microphone array, coordinated with `SpeechQueue` under AGENTS.md hard rule 7: keeps AirPods on A2DP (never Bluetooth HFP), silences the beacon while listening and restores `.playback` at once on release / end of utterance / timeout, holds the speech channel (`SpeechQueue.setVoiceHold` — route and obstacle lines queue with their TTLs, `.safety` still breaks through; released before the answer), ends listening on its own (`UtteranceEndDetector`), and gives every press an audible outcome (an answer, "I did not catch that.", or why it could not listen — silence is a bug).
...
- `@MainActor @Observable final class VoiceInputEngine` — published `state`, `isListening`, `isStarting`, `latestTranscript`; callbacks `onTranscriptionFinalized: ((String) -> Void)?` (→ `ConversationCoordinator.handleQuery`, then `finishProcessing()`), `shouldRestorePlaybackSession: (() -> Bool)?` (false while `SoundWatcher` owns the microphone), `onEvent: ((String, [String: Any]) -> Void)?` (→ trip log: `logStart` / `logEnd`, `voice_end` carries the reason). Methods: `startListening()`, `stopListeningAndSubmit(reason: "press" | "silence" | "timeout")`, `finishProcessing()`, `cancel()`; private `startEngine(attempt:sessionField:)` (format retry, one `UtteranceEndDetector` per session, the end ticker), `recognitionUpdate(text:isFinal:error:)`, `checkEnd()`, `cleanupAudioPipeline()`, `denied(speech:)`, `fail(with:)`. Route/interruption callbacks and permission/engine polling live for the full capture lifetime and are fenced by `VoiceInputGuard`.
- Relays: `private nonisolated final class SpeechBufferBox: @unchecked Sendable` (installs the audio tap; the tap fires on an audio thread) and `SpeechResultsRelay: Sendable` (carries only Sendable values into a main-actor hop, mirroring `SoundResultsRelay`). ⚠ Every framework callback closure formed in a main-actor method must be `@Sendable` (or formed in a nonisolated relay): an inferred `@MainActor` closure invoked off main traps (Step 24 device crash; 23 crash reports pulled on 2026-09-12).
...
- Body inside `CKCard(title: "Guide")`:
  ...
  - When `model.nav.isNavigating`:
    - Row of `"Repeat"` (`arrow.counterclockwise`, primary, hint "Says the current instruction again" → `model.repeatInstruction()`) and `"Next"` (`forward.fill`, secondary, hint "Skips to the next instruction" → `model.nextWaypoint()`); the shared AppModel path also serves Watch/crown/Siri and says "No route running." when idle. Comment: two per row because three-up hyphenates "Recenter" on a 17 Pro Max at default type size.
    ...
    - `"Stop route"` (`stop.fill`, `.destructive`) uses the `StopRouteConfirmation` two-tap gate: the first tap arms a three-second window and speaks/shows `"Stop route is armed. Tap Stop route again within 3 seconds to end guidance."`; only the second tap calls `model.stopRoute()`. The accessibility label remains `"Stop route"` for the XCUITest contract.
  - Else (idle):
...
- `snap` / `save` / `pause` — same shape as the visual tour, kept local so the two tours stay independent. Run: `make island` (PNGs in `SHOTS`), also part of `make uitest`. The simulator shows no system location pill, so the blue background-location indicator the phone adds during a route is not in these pictures.
