...
(deployment target iOS 26) clamped to a non-metal 27.65 mm cane, using LiDAR depth for obstacles and
...
- [Module `conversation` — `ios/CaneKit/Conversation/` ("Talk to OpenCane", Step 23)](#module-conversation--ioscanekitconversation-talk-to-opencane-step-23)
...
| `groundSkipFraction` | `0.25` | Rows mode only: bottom fraction of the upright scene skipped as ground (metric mode drops the floor by height) |
...
| `cameraHeightCm` | `95` cm | Metric mode (Step 51): camera height above the ground (constant; ground-plane calibration is in docs/todo.md) |
| `floorMaxHeightCm` | `25` cm | Metric mode: samples lower than this are floor / kerb / cane tip, dropped from both bands |
| `headMinHeightCm` | `140` cm | Metric mode: samples at or above this are the head band; 25–140 cm is torso |
| `coverageRangeCm` | `150` cm | Metric mode: range at which each ray's *nominal* band is judged (coverage + blind bucketing) |

**`struct LaneGrid: Sendable, Equatable`** — `head: [Float]`, `torso: [Float]` (index 0 = left, 1 = centre, 2 = right; metres; `.infinity` = clear / no data / no coverage), `centerDepth: Float` (median of centre window, `.infinity` if unknown), `headBlind` / `torsoBlind`, `headHeld` / `torsoHeld` (Step 48), and (Step 51) `headCoverage` / `torsoCoverage: [Bool]` (per lane: ≥ `minSamplesPerCell` rays of the lane reach the band at `coverageRangeCm`; default all true, always true in rows mode) and `bandMode: LaneBandMode` (default `.rows`). ⚠ **No coverage = `.infinity` plus the flag false, never NaN or −1** (`TripLogger.num` maps non-finite to −1 and every `.infinity == clear` consumer — watch, island, `SpokenPhrases`, `TorsoHapticPolicy` — must keep working); only `HeadGate`, `TileLevel`, `NearHold`, the Mount card and the trip log read the flags. `static let empty` = all infinity. `nearest(lane:) -> Float` = `min(head[lane], torso[lane])`.

**`enum LaneBandMode: String, Sendable, Codable`** (Step 51) — `rows` (no ARKit pose yet: the old fixed rows) / `metric`; raw value = the `lanes.bands` log field.

**`struct LaneGeometry: Sendable, Equatable`** (Step 51) — `fx, fy, cx, cy` (depth-map px, landscape buffer coordinates, scaled from `camera.intrinsics` exactly as `GroundSampler` scales them) and `upX, upY, upZ` (world-up components of the camera's x / y / z axes = `camera.transform.columns.0.y / .1.y / .2.y`). Height of the sample at buffer `(u, v)` with z-depth `d`: `h = camH + d · g`, `g = upX·(u−cx)/fx − upY·(v−cy)/fy − upZ` — the y-component of `GroundSampler`'s back-projection, so roll is compensated for free and the portrait remap needs no reasoning. `heightGain(u:v:)`, `heightCm(u:v:depthM:cameraHeightCm:)` (the one metre → centimetre conversion; owner decision 2026-09-13: the new geometry works in cm), `pitchDownDeg` (= `MountTilt.downDegrees(upZ)`), `static pitchOnly(downDeg:portrait:fx:fy:cx:cy:)` (tests / docs: portrait `(−cos θ, 0, sin θ)`). Built per frame by `DepthFrameProcessor.computeGrid` (nil while `trackingState == .notAvailable`), carried on `LaneReport.geometry`, logged as `depth_geometry`. Tests: `LaneGeometryTests` (16).

**`enum LaneMath`**
- `static func computeLanes(depth: UnsafeRawPointer, depthBytesPerRow: Int, confidence: UnsafeRawPointer?, confidenceBytesPerRow: Int, width: Int, height: Int, config: LaneConfig, scratch: inout [Float], geometry: LaneGeometry? = nil) -> LaneGrid`
  - **Metric mode** (`geometry != nil`, Step 51): per lane one pass over **all** scene rows; per sample `g = heightGain`, nominal height `camH + coverageRange · g` decides coverage (rays per band) and where a *blind* sample counts (Step 48 semantics kept), actual height `camH + d · g` decides the band (floor < 25 cm dropped, head ≥ 140 cm); an uncovered band is `.infinity`; the centre window drops floor samples; `bandMode = .metric`. ~2× the rows-mode cost. Nil geometry = rows mode, below.
  ...
  - A sample is valid iff depth `isFinite && > 0.05 m` and either (a) depth `< closeOverrideThreshold` (35 cm, near-field saturation override), or (b) confidence ≥ `minConfidence` (when confidence map is given). Zero, NaN, and ≤ 5 cm are invalid — and counted as **blind** (Step 48): each cell's blind share (`blind / sampled`, every visited pixel counts as sampled) is returned in `LaneGrid.headBlind` / `torsoBlind`; the centre window is not counted. `blindFractionIsReportedPerCell`.
...
- `static func computeLanes(depth: [Float], confidence: [UInt8]?, width: Int, height: Int, config: LaneConfig = LaneConfig(), geometry: LaneGeometry? = nil) -> LaneGrid` — array convenience for tests/replay; `precondition(depth.count == width*height)` (and same for confidence); assumes tight rows (`bytesPerRow = width*4` / `width`).
- ⚠ Do not change the rotation mapping, ground skip, percentile logic, or stride handling without re-running `LaneMathTests` (esp. `rawEntrypointHonoursPaddedRowStrides`, `headRowIsTopBandWithoutGeometry`, `groundBandIsSkippedWithoutGeometry`, `pointBlankLowConfidenceObstacleIsDetected`) and `LaneGeometryTests` **and** a device test with the phone clamped upright on the cane (the L/R/head/torso orientation is only verifiable on hardware).

---

### `NearHold.swift` — a blind cell right after a near reading is a wall, not a clear path (Step 48)
Why: inside ~10 cm the LiDAR returns 0 / NaN (not a low-confidence distance), `LaneMath` discards them, the cell reports `.infinity`, and `.infinity` is CLEAR everywhere — a teammate's photo showed six green tiles against a wall.
- **`struct NearHoldConfig: Sendable, Equatable`** — `blindFraction = 0.5` (a cell at least this blind is "too close to read"), `armBelowM = 0.6` (the last *measured* distance must have been under this; above `closeOverrideThreshold` on purpose — a step takes 0.5 m straight into the blind zone), `pointBlankM = 0.1` (what a held cell reports; under `CueThresholds.centerNear`, so STOP / urgent). `defaultsAreTheDesign`.
- **`struct NearHold: Sendable, Equatable`** — `config`; `reset()`; `apply(_ grid: LaneGrid) -> LaneGrid`: per cell, a finite value is remembered and passed through; an `.infinity` cell with blind ≥ `blindFraction` whose remembered value is < `armBelowM` becomes `pointBlankM` with `headHeld` / `torsoHeld` true; an `.infinity` cell with blind < `blindFraction` (a real no-return clear) forgets its memory. Never arms without a prior near reading (a blind sky / corridor stays clear). Owner: `DepthFrameProcessor.nearHold` (queue-only), applied to every grid before publishing; reset by a frame without depth. Tests: `NearHoldTests.swift` (7, suite "Near hold").
- `LaneGrid` gained `headBlind` / `torsoBlind: [Float]` (0…1) and `headHeld` / `torsoHeld: [Bool]`, all defaulted in `init`; `LaneGrid.empty` has zeros / falses.
- Step 51: uncovered cells (`headCoverage` / `torsoCoverage` false) are skipped entirely, and the cold start needs `min(coldStartMinCells, covered cells)` blind cells — at 45° only the three torso cells exist, and 4-of-6 could never arm (`coldStartCountsOnlyCoveredCells`). Coverage flags pass through (`out = grid`).
...
- **`enum MountTilt`** — the mount's camera aim. Since Step 51 the lanes are gravity-corrected, so the pitch is a **coverage** question, not a correctness one. `static func downDegrees(cameraZColumnY:) -> Float` converts `camera.transform.columns.2.y` to degrees below the horizon (pinned by `tiltSignIsDownPositive`); `DepthFrameProcessor.trackTilt` uses it.
  - `static let aim: ClosedRange<Float> = 3...8` — the hinge recommendation (hardware/mount/DESIGN.md), no longer read by the lane code; `static let groundAim` 0–15° for the ground detector.
  - `static func headCoverLimitDeg(cameraHeightCm: 95, headMinHeightCm: 140, coverageRangeCm: 150, halfFovDeg: 33.5) -> Float` — steepest pitch at which the top ray reaches head height at the cover range in **z-depth**: `acos(k · cos h) − (90° − h)`, `k = (headMin − camH) / range` → ≈ 19.0°. Docs, the one-argument `status`, `depth_geometry` and `cue_audit.py`. Pinned by `headCoverLimitFollowsTheGeometry`.
  - `static func status(downDeg: Float, headCover: Bool) -> (text: String, ok: Bool)` — `n = Int(|d|.rounded())`; `n == 0` → "Camera level: tilt the phone down"; `d < 0` → "Camera tilt N° up: tilt the phone down"; `!headCover` → "Camera tilt N° down: too steep for head-height cover" (ok false); else "Camera tilt N° down, good". `status(downDeg:)` estimates `headCover` from the shown angle against `headCoverLimitDeg()`. Caller: `MountAimRow` with `grid.headCoverage.contains(true)`. Pinned by `mountTiltStatus`, `mountTiltStatusSaysTooSteepForHeadCover`.
- **`enum TileLevel: Sendable`** — `clear, far, near, urgent, noData, noCover`. `static func level(for distance: Float, hasData: Bool, covered: Bool = true) -> TileLevel`: `!hasData → .noData`; `!covered → .noCover` (NO COVER, never CLEAR); non-finite → `.clear`; `< 0.7 → .urgent`; `< 1.2 → .near`; `< 2.0 → .far`; else `.clear`. Used by `LaneGridView` tiles. Pinned by `tileLevels`, `tileNoCoverIsNotClear`.
- **`struct HeadCoverNotice: Sendable, Equatable`** (Step 51) — `static line` "Camera too steep for head-height cover. Torso obstacles only." (⚠ byte-identical in `AppModel.commonLines`); `holdSeconds = 2`; `routeStarted()`; `update(metric:trusted:headCovered:now:) -> Bool` — true once per route after 2 s of trusted metric frames with no head lane covered (rows frames and sweeps neither count nor reset; a covered frame resets). Owner `AppModel.headCoverNotice` (`.nav`, logged `head_cover`). Pinned by `headCoverNoticeSpeaksOncePerRouteAfterTheHold`, `headCoverNoticeIgnoresRowsModeAndTransients`.
...
- **`DepthReadiness.TimeoutReason: String`** (Step 49) — `depthMissing` ("depth_missing": no report with scene depth arrived), `trackingLimitedDepthLive` ("tracking_limited_depth_live": depth arrived but no frame ever had `.normal` tracking — a dark hallway; LiDAR cues run, only the pose drifts), `depthUnsteady` ("depth_unsteady": both seen, never `requiredFrames` in a row). `timeoutReason: TimeoutReason?` is set by both timeout paths (`frame` at/past the deadline, `poll`) from two counters kept since `begin` and across `invalidate` (`depthFramesSeen`, `normalTrackingSeen`); `cancel` clears it — `AppModel.failQueuedRouteStart` reads `DepthEngine.readinessTimeoutReason` first and speaks the LiDAR line for `trackingLimitedDepthLive`, "Guiding with GPS." otherwise. Tests: `timeoutInTheDarkNamesLimitedTrackingWithDepthLive`, `timeoutReasonsTellDepthMissingFromUnsteady`.
...
- **`enum HapticCue: Sendable, Equatable`** — `centerApproach(distance: Float)`, `left`, `right`, `head(distance: Float, onset: Bool)` (Step 52: the gated head distance, and whether this fire opened the episode — `CueSpeechPolicy` speaks the onset, and once more under 0.6 m; `HapticPlayer` plays the same pattern; the payload never leaves the phone); `kind: CueKind`.
...
| `repeatInterval` | 1.0 s | minimum time before the same side cue (left/right) fires again — since Step 52 **not** head |
| `nearDropoutHoldSeconds` | 1.5 s | hold active state across non-finite dropouts when obstacle was in urgent proximity (< 0.7 m) |
| `overhangGapM` | 0.5 m | Step 52 overhang signature: the torso cell must be at least this much farther (or non-finite / uncovered) |
...
- **Step 66 fixes (found by `StressTests.cueDeciderSurvivesRandomLaneStreams`, 400 seeded 30 Hz streams):**
  (a) a head **onset** needs a finite gated distance on the frame itself (`headSpeaks`): the point-blank
  dropout latch may keep the head zone alive across a blind frame, but never starts "Head height." for a
  cell `HeadGate` rejects now — before, a latched zone fired `.head(distance: ∞, onset: true)` 116 times
  (pinned by `noHeadOnsetOnANonFiniteDistance`); (b) a head onset while `active` is still `.head` from an
  episode that already ended takes the change branch (`onsetWhileHeadActive`) — before, a gated hand-off to
  the centre loop plus a sweep longer than `headClearSeconds` left `active == .head` with no episode, and
  the returning overhang was silent until the zone cleared (82 hits; pinned by
...
- **`final class CueDecider`** — **not Sendable**; owned by `AppModel` (`@ObservationIgnored private let decider`). State: `thresholds` (var), `active: CueKind` (private(set), starts `.clear`), `lastChange: TimeInterval` (starts `-∞`), private `lastFired: [CueKind: TimeInterval]`, private `zoneActive` per kind.
  ...
  - `update(_ r: LaneReport, now: TimeInterval) -> CueOutput?` — `nil` = nothing new. Algorithm, in order:
    ...
    6. Same kind active: center → `.updateCenter(distance:)` every frame (exempt from the 1 s floor); left / right → re-`fire` only when `now − lastFired ≥ repeatInterval`.
    7. **Head (Step 52):** after the 400 ms change gate, an onset (no live episode) fires `.head(distance:onset: true)` at once — no repeat floor — and starts a `HeadEpisode` whose consumed bands are those the onset distance is already inside; inside the episode (a re-entry or the same cue) it fires `.head(distance:onset: false)` only when the distance crosses the next unconsumed band ≥ `headRefireMinGap` after the last fire (a jump across both bands fires once); a silent re-entry takes `active = .head`. `.stop` when the zone clears is unchanged; the episode outlives it. `headEpisodeActive: Bool` is public (log / tests).
  - `now` is caller-supplied (monotonic, seconds); the decider never reads the clock.
  - ⚠ Do not change thresholds, priority order, the 400 ms / 1 s gates, or the freeze rule without re-running `CueDeciderTests` (all 26) and a walk test with the cane; these values are the spec'd haptic grammar.

### `HeadGate.swift` — which lane, if any, holds a head-height obstacle (Step 52)
`public enum HeadGate` (pure, stateless). `struct Candidate: Sendable, Equatable { lane: Int; distance: Float }`. `static func candidate(in grid: LaneGrid, enter: Float, overhangGap: Float?) -> Candidate?` — the nearest lane whose head cell is covered (`headCoverage`), finite and `< enter`, and, when `overhangGap` is non-nil, whose torso cell is uncovered, non-finite, or ≥ head + gap (fail-safe: a torso dropout or blind torso keeps the warning). `overhangGap == nil` = signature off. Callers: `CueDecider.update` (head zone distance), `AppModel.contextLine` ("Something at head height."), the island `headNear` in `AppModel.handle` — one rule for haptic, describer and island. Python mirror: `cue_audit.py head_gate`. Why: on the first cane walk 587 of 625 head cells were near in the torso band too (walls, people). Tests: CueDeciderTests `wallNearInBothBandsIsNotHead`, `hangingSignWithClearTorsoIsHead`, `torsoHalfAMetreFartherIsHead`, `torsoNoDataFailsSafeToHead`, `overhangSignatureCanBeSwitchedOff`, `headIgnoresLanesWithoutCoverage`, `torsoWithoutCoverageFailsSafeToHead`.
...
**`struct CueSpeechPolicy: Sendable, Equatable`** — which obstacle cues are also spoken. Review round Steps 67–68: `static closeTier: Tier = .safety` ("Close." cuts the route intro; it was `.obstacle` and its 3 s TTL dropped it) and `static closeAllowed(navigating:indoorActive:level:) -> Bool` (while a route or indoor script guides, at every cue level including Quiet); caller `AppModel.handle`; test `closeIsASafetyLineAtEveryCueLevel`.
- `enum Tier: Sendable, Equatable { safety, obstacle }`. Vars `headInterval = 4 s`, `sideInterval = 4 s`, `headSecondLineBelowM = 0.6` (Step 52); private `lastSpoken: [CueKind: TimeInterval]`, `headSecondLineSpoken`.
- `mutating line(for cue: HapticCue, phoneCannotBuzz: Bool, now: TimeInterval) -> (text: String, tier: Tier)?` — `.head(d, onset: true)` → `"Head height."` (`.safety`) under the 4 s `headInterval` limiter, and re-arms the second line; `.head(d, onset: false)` → `"Head height."` once per episode when `d < 0.6` (not under the 4 s limiter: the decider bounds it), else nil; regardless of `phoneCannotBuzz`. `.left` → `"Left."`, `.right` → `"Right."`, `.centerApproach(d)` → distance-first `"<Capitalized phrase> ahead."` (all `.obstacle`, `sideInterval`) only when `phoneCannotBuzz`. Then the per-kind limiter. There is no `cleared()` and no `episodeKind` since Step 52: the episode is `CueDecider`'s (a `.stop` inside an episode used to make the next fire "new" — 8 lines in 12 minutes on the first cane walk).
- Owner: `AppModel.cueSpeech` (`AppModel.speakCueIfNeeded`): on every rendered fire with `phoneCannotBuzz = !haptics.isHealthy || haptics.silenced`; `.safety` → `SpeechPriority.safety`, else `.obstacle`; ttl 6 s. A fresh instance at route start.
- ⚠ Pinned by `headHeightIsSpokenOncePerEpisode`, `headEpisodesAreRateLimitedAcrossEpisodes`, `secondHeadLineNeedsUnderSixtyCentimetres`, `sideCuesAreSpokenOnlyWhenThePhoneCannotBuzz`, `aBuzzedSideCueDoesNotSplitAHeadEpisode`. The head onset line must never be suppressed except by the 4 s limiter (spec: an overhanging sign has no mesh class).
- Step 68: `static closeText = "Close."`, vars `closeClearSeconds = 3`, `closeInterval = 4`; private `closeEpisode`, `closeClearSince`, `lastCloseAt`. `mutating close(torsoCentreM:covered:held:trusted:now:) -> String?` — "Close." when the centre torso cell enters `TileLevel.urgent` (< 0.7 m, the red tile), once per red episode; an episode ends after 3 s of trusted, covered frames out of the red; never two lines within 4 s (a red entry inside the limiter speaks when it expires); a `held` (NearHold) reading continues but never starts an episode; untrusted / uncovered frames are ignored. Owner `AppModel.handle` (route or indoor script guiding, level not Quiet) → `.obstacle`, ttl 3, `speech` record. ⚠ Pinned by `closeIsSpokenOncePerRedEpisode`, `closeNeedsThreeSecondsOutOfTheRed`, `closeIsRateLimitedToOneEveryFourSeconds`, `aPointBlankHoldNeverStartsClose`, `sweepAndUncoveredFramesNeitherStartNorEndClose`.
...
- **`enum ScenePrompt`** — `static let text` = "You are describing what a cane-mounted camera sees, for a blind pedestrian. One sentence, under 20 words. Name what is actually there, hazards first, and say whether each thing is on the left, in the center or on the right. Never give a number, a distance or a count. Never say the way is clear, empty or safe. No preamble. If it is too dark to see, answer exactly: It is too dark to see." (sides not clock faces, no numbers — the measured VLM failures in the file header; the last sentence is Step 49, paired with `CloudSceneGate.tooDark`; < 400 chars, `scenePromptAsksForSidesNotNumbers`). (This entry used to quote the pre-gate clock-face prompt; the code is the truth.)
...
**`enum HazardPrompt`** — `static let text`: the hazard-watch prompt ("You are the eyes of a blind pedestrian walking forward. Look only at the walking path in the next 5 meters… reply with ONE short phrase under 8 words naming the distance FIRST in meters, then the hazard and where — e.g. "3 meters ahead, cones". Otherwise reply exactly NONE. If the image is too dark to see, reply exactly NONE." — the last sentence is Step 49). The ordering contract lives in the prompt because `HazardWatchPolicy.line` wraps the first sentence as written and never reorders. `OnDeviceVLMClient` detects hazard mode by `prompt == HazardPrompt.text` — ⚠ keep it a single constant.
...
**`struct HazardRecord: Sendable, Equatable, Codable`** — `kind` ("dropOff" / "pothole" / "stepUp" / "lowObstacle" / "sign" / "vision"), `text` (what was spoken), `latitude`, `longitude` (degrees), `accuracy` (m; −1 = no fix), `time` (seconds since 1970), `photo: String?` (JPEG file name next to the GeoJSON), plus enriched Grok Bot / sidewalk telemetry: `distanceM: Double?`, `heightM: Double?`, `direction: String?`, `headingDeg: Double?`, `speedMps: Double?`, `routeName: String?`, `instruction: String?`, `source: String?`, `whatItSaw: String?`, `severity: String?`. `init(kind:text:latitude:longitude:accuracy:time:photo:...)` (defaults for enriched fields maintain backwards compatibility). `asGrokBotEvent(user:caneID:) -> OpenCaneEvent` converts directly to the webhook event contract.

**`enum HazardGeoJSON`** — `static func encode(_ records: [HazardRecord]) throws -> Data`: RFC 7946 `FeatureCollection` of `Point`s with **`[longitude, latitude]`** order; a record with **`accuracy < 0` (no fix) gets `"geometry": null`** (valid per RFC 7946 §3.2) instead of a bogus point at 0, 0; properties `kind`, `text`, `accuracy_m`, `time` (ISO 8601 via `ISO8601DateFormatter`), `photo`, `distance_m`, `height_m`, `direction`, `heading_deg`, `speed_mps`, `route_name`, `instruction`, `source`, `what_it_saw`, `severity` when present; `JSONSerialization` with `.prettyPrinted, .sortedKeys`. Opens in geojson.io, QGIS, Google My Maps and the Files preview. Pinned by `hazardMapIsValidGeoJSON`, `aHazardWithoutAFixHasNullGeometry`, `enrichedHazardRecordSerializesAllProperties`. Caller: `HazardLog.record`.
...
- **`public enum WalkingIntro`** — `line(place:meters:)` "Walking to <place>, N meters."; `routeStarted(_ route:)` / `routeStarted(destination:firstLine:)` (Step 54; Step 68 wording "Route to <destination>. <first say>", `destinationName(_:)`: "To X" → X, "A to B" → B, else the last waypoint's `placeName`; `introNamesTheDestinationNotTheRoute`) — the one builder `NavigationEngine.start` speaks and `AppModel.routeStartLines` prefetches (`introLineIsWhatNavigationSpeaks`); `distancePhrase(_:)` (**public** since Step 14, also used by `DestinationSuggestion.voiceOverLabel`) rounds to 10 m below 1 km, tenths of a kilometre above.
- Step 67 aliases (recogniser mishearings, exact, never fuzzy): Grainger "Granger Engineering Library", "Grainger Engineering"; CIF "CIF building", "C I F", "see eye eff". "library" alone stays unmatched (Main Library). Pinned in `everyCampusPlaceAnswersToItsAliases`.
- ⚠ Every `name` must match its own place (`everyCampusPlaceNameRoundTripsThroughMatch`, Step 62): the fast path hands the *name* to `AppModel.navigate(to:)`, which matches again. ISR gained the alias "Townsend Hall doors" so "the Townsend Hall doors" no longer falls through to MapKit.
- Tests: `CampusPlacesTests.swift`.

### `DestinationSuggestions.swift` — the destination search box's ranked list (Step 14; `Locality` filter Step 50)
- **`struct Locality: Sendable, Equatable`** — `countryCode: String` (ISO 3166, "US"); `static campus`; `countryName` (US-English, MapKit's subtitle spelling); `plausiblyNearby(subtitle:) -> Bool` drops a completer row only when its subtitle's last comma-part is the English name of another country (`countryByName`, built once from `Locale.Region.isoRegions`); a bare street, a city, another US state, a zip — all kept (a state line is not a distance). `suggestions(query:completions:from:locality:)` applies it before ranking (default `.campus`). Owner of the live value: `DestinationSearch.locality`, reverse-geocoded with `MKReverseGeocodingRequest` once per 500 m of fix movement. Tests: `foreignCompletionsAreDropped`, `localAndUnmarkedCompletionsAreKept`.
...
Pure builders behind the Siri intents. `QuestionPrompt.clean` (nil for unanswerable questions) / `.text(for:)` (the model prompt); `StatusFacts` (gathered by `AppModel` at speak time) → `StatusSummary.lines` / `.sentence` (fixed clause order: obstacle detection, GPS, audio, voice (Step 54, only when `StatusFacts.voice` is set), haptics, route, battery); `VoiceFacts {hasKey, naturalEnabled, breakerOpen, cachedShare}` → `voiceReady(_:)` (key ∧ Natural ∧ breaker closed ∧ `cachedShare ≥ voiceReadyShare` = 1) / `voiceLine(_:)` ("Natural voice ready." / "Natural voice warming up, N percent cached." rounded down / "System voice. No natural voice key." / "System voice, by your setting." / "Natural voice unreachable. Using the system voice."; tests `voiceClause…` ×3) / `.hapticsLine` (shared with `announceChannels` so one fact has one sentence everywhere) / `.fixedLines` (Step 56: the number-free clauses as named constants — `noDepthSensorLine`, `detectionOffLine`, `noDepthFramesLine`, `locationDeniedLine`, `noGPSFixLine`, `gpsUnknownAccuracyLine`, `noHeadphonesLine`, `noRouteLine` — plus every haptics form, prefetched through `SpokenPhrases.shellLines`; `everyNumberFreeClauseIsAFixedLine`). Tests: `QuestionPromptTests.swift`, `StatusSummaryTests.swift`.
...
- **Step 67 — `FastPathIntentClassifier` rule 14b `travelIntent(_:)`** (after rule 14, before `nil`): a travel verb phrase anywhere (`travelVerbPhrases`: show me the way, take / bring / get / navigate / walk / guide / lead / route me, directions, navigate, go, get, walk, travel — longest first; not "went"; skipped after "used to"; `gazetteerOnlyVerbPhrases` going / heading / headed / head count only for a campus place) followed by `to <B>` (a trailing `from <A>` splits at the last "from") or `from <A> to <B>`. Words via `words(_:)` (`Word {text, key}`: edge punctuation trimmed, key lower-case without apostrophes); `trailingFillers` (right now, thank you, for me, please, now, thanks) trimmed by `trimTrailingFillers(_:)`; a real origin → `.routeFromTo(from:to:)`; with a `hereOrigins` / no origin every verb is tried and a gazetteer hit wins, else the first MapKit candidate (≤ `maxMapKitDestinationWords` 4 words, none of `nonPlaceWords`); a destination ending in a bare "from" / "to" skips the verb. `destinationAction` now trims trailing fillers too (rules 13b / 14: "take me to the Union please" matches the gazetteer) and returns nil for a target starting or ending in a bare "from" / "to" (Muse review). Tests add `travelIntentReviewCases`. Evidence: `canekit-2026-09-13T15-48-34Z.jsonl`, "I just wanna get from here to Granger library" missed every rule and went to the cloud. Tests: `ConversationLogicTests.spokenDestinationsRouteOnThePhone`, `aTravelVerbAnywhereKeepsARealOrigin`, `noTravelIntentIsNotARoute`; `StressTests.fastPathFuzz` still green.
- **Review round Steps 67–68 — `FastPathIntentClassifier`**: rule 1 compares `stopKey(_:)` (word keys minus `leadingStopFillers` please / okay / ok / um / uh and `trailingFillers`) with `stopPhrases` ("please stop", "stop please" stop; "stop the beacon" does not). Rule **1c** `isEmergencyRequest(_:)` (after 1b, before every route rule): whole word "emergency" / "emergencies" not followed by `emergencyObjectWords` (exit, door, stairs…), or "911" / "9-1-1" / "nine one one", with no `emergencyNegations` word (not, isnt, cancel, canceled, cancelled, never, false) and no "no" right before → `.emergency`. `travelVerbPhrases` is now only the explicit navigation verbs (show me the way, take / bring / get / navigate / walk / guide / lead / route me, directions, navigate) — the only 14b verbs whose destination may be a MapKit search; `gazetteerOnlyVerbPhrases` adds go, get, walk, travel; `gazetteerOnlyPrefixes` ("go to ", "walk to ") make rule 14 and rule 13b use `campusAction(_:)` (campus place or nil). `nonPlaceWords` += you, emergency, emergencies, 911 (and eat, drink). Tests: `politeStopPhrasesStillStop`, `ambiguousTravelPhrasesRouteOnlyToCampusPlaces`, `emergencyWordsReachTheConfirmationNeverARoute`, `CampusPlacesTests.compoundGraingerMishearingsMatch` (aliases Granger Engineering / Granger Building / Grainger Building).
- **`WalkMarker`**: Voice-dropped GPS breadcrumb pin — `public struct: Sendable, Equatable, Codable, Identifiable` with `id: UUID`, `name`, `coordinate`, `timestamp` (no altitude). Its `Codable` form is the on-disk shape of `PostStore` (`walkMarkerJSONRoundTrip`).
...
- **`FastPathIntentClassifier`**: Sub-millisecond deterministic intent matcher resolving settings, status aspects, campus gazetteer destinations (`CampusPlaces`), marker drops, and trip metrics with zero LLM tokens. **Rule 0 (Step 56)** is the voice shell: `VoiceMenu.match` / `verb` / `confirmation`, whole utterance, before every other rule → `action(for:)`; rule 12 (distance walked) now runs before rule 8 ("how far"). Voice-shell `ConversationAction` cases: `startDefaultRoute`, `describeScene`, `speakStatus`, `repeatInstruction`, `nextWaypoint`, `setCueLevel(CueLevel)`, `help`, `emergency`, `confirm(Bool)`. Tests add `ivrRuleRunsBeforeEverythingElse`, `stopPhrasesStillStopBehindRuleZero`, `howFarHaveIWalkedIsTheDistanceWalked`. **Step 62:** rule **1b** — whole utterance `outsideForms` ("I'm outside", "I am outside", "we're outside", "outside now", …; curly apostrophe folded) → `.indoorOutside`; rule **13b** (before rule 14) — `routeFromTo(_:)`: "take me from / go from / navigate from / from A to B" (split at the first " to ") and "take me to B from A" (split at the last " from ") → `.routeFromTo(from:to:)`, both ends in the speaker's case with edge punctuation trimmed; either end empty → nil (later rules run); a `hereOrigins` origin ("here", "my location", …) → `destinationAction(to)`, the same `.startRoute` rule 14 gives. Rule 14 now calls `destinationAction(_:)` (`CampusPlaces.match` → place `name`, else `target.capitalized`). Review round (Muse M3): `public static destinationPrefixes` (rule 14's nine prefixes); rule 13b's "B from A" split applies after every one of them (test `everyDestinationPrefixTakesAFromOrigin`). `ConversationCoordinator.executeAction` wires them to `AppModel.routeFromTo(from:to:)` and `AppModel.indoorOutside()` (AppModel+Indoor.swift). Tests: `fromAToBIsARouteFromTo`, `fromHereOrHalfARouteIsNotARouteFromTo`, `imOutsideIsTheIndoorHandover`.
...
`enum CueLevel: String` (`quiet`, `standard`, `detailed`; raw values persisted — `rawValuesAreStable`) with `spokenLine` ("Quiet cues." …) and `title`; `enum CuePlace: String` (`outdoors`, `indoors`) with "Outdoor mode." / "Indoor mode." and `title`. `struct CueRules { level, place, requireOverhangSignature }` (Step 52: `init(level:place:requireOverhangSignature: = true)`, ON by default — owner decision 2026-09-13; valve `Settings.bool("overhangSignature")` in `AppModel.cueRules`; `defaultRulesRequireTheOverhangSignature`): `static default` = Detailed + Outdoors (today's behaviour, owner decision until a mounted log tunes it); `headEnterM` (outdoors `CueThresholds().head` 1.5 m, indoors 1.2 m [H]); `allowsName(_ cls:, navigating:)` (speakable classes only; indoors or Quiet never; Standard door only while navigating; Detailed all but wall); `namesLimitLine` (nil for Detailed outdoors, else "Quiet cues name nothing." / "Standard cues name only doors, on a route." / "Indoor mode names nothing." — appended by `AppModel.setOption` when names are turned on by voice); `allowedSignPhrases` (Quiet or Indoors → `safetySignPhrases`: closures, danger, caution, wet floor, push button, closed; otherwise nil = all); `static safetySignPhrases`, `static allSpokenLines` (in `AppModel.commonLines`). Owner: `AppModel.cueLevel` / `cuePlace` / `cueRules`. Tests: `CueProfileTests.swift` (12; `defaultRulesRenderTodaysHaptics` pins that the default renders today's haptics through `TorsoHapticPolicy`) + `SignPhraseFilterTests` (4).

### `TorsoHapticPolicy.swift` — torso haptics by level × place, over the decider (Step 41, cue design v2 §3.3 / §4 row 9)

`CueDecider` is untouched (its 14 tests pin today's decisions); this value type says which of its **torso** decisions the walker's level renders. `enum TorsoSuppressReason: String, CaseIterable` — `quiet`, `indoors`, `crossing_settle`, `standard_side`, `standard_center_hold`, `shoreline` (⚠ raw values are the `suppressed` field of the trip log's `cue` record, counted by `cue_audit.py`; never rename). `enum TorsoHapticAction` — `render(HapticCue)` (play as the decider fired it: the head cue always, torso in Detailed), `centerOnset(strong: Bool, distance: Float)` (Standard: `HapticPlayer.playCenterOnset(strong:)`; mirrored / spoken like a centre cue), `updateCenter(distance:)` (Detailed loop update), `stop` (the decider's `.stop`, every level), `suppressed(HapticCue, reason:)` (no pattern, no wrist mirror, no spoken fallback; the app calls `stopAll()` and logs). `struct TorsoHapticThresholds` (metres / seconds, pinned by `thresholdDefaultsAreTheDesign`):

| Field | Default | Meaning |
|---|---|---|
| `onsetNearM` | 1.5 m | Standard: single onset tap once the centre distance is under this **and closing** |
| `onsetStrongM` | 0.6 m | Standard: strong triple once under this — proximity only, closing or not (Muse review: a walker inching in at < 0.1 m/s must still be told); fires alone if the near tap was skipped |
| `closingDeltaM` | 0.1 m | "closing" = at least this much nearer than the reference sample |
| `closingWindowS` | 1.0 s | the reference is the newest *trusted* centre sample at least this old; none, or a non-finite one, is not closing |
| `rearmClearS` | 1.5 s | both onsets re-arm after this long with the centre no longer the decider's cue (a `.stop`, or a side fire) |
| `shorelineBandM` | 0.1 m | Detailed: every side sample in the window must be finite and within ± this of the current reading |
| `shorelineWindowS` | 2.0 s | Detailed: the side history must cover at least this long for a tap to be a shoreline |

`struct TorsoHapticPolicy: Sendable, Equatable` — `thresholds`; `reset()`; `update(_ output: CueOutput?, report: LaneReport, rules: CueRules, crossingSettle: Bool, now: TimeInterval) -> TorsoHapticAction?` (nil = nothing to do). Order inside: trusted frames append the three torso readings to per-lane histories (pruned past 2.5 s; untrusted frames add nothing, mirroring the decider's freeze) → `.stop` passes through and starts the re-arm clock → `.fire(.head)` → `.render(.head)` unconditionally → side fires: hold check (Quiet → `quiet`, Indoors → `indoors`, crossing → `crossing_settle`, in that order), Standard → `standard_side`, Detailed → `shoreline` or `.render` → centre fire / update: re-arm if the clear lasted ≥ `rearmClearS`; under a hold the fire (or the first update of a loop that was rendered) comes back `.suppressed` so the app stops the loop, later updates nil; Detailed → `.render` / `.updateCenter`; Standard → `.centerOnset(strong: true)` if < 0.6 m and not yet fired (proximity only; sets both flags), else `.centerOnset(strong: false)` if closing and < 1.5 m and not yet fired, else `.suppressed(standard_center_hold)` for a fire (once per decider fire) and nil for an update. Owner: `AppModel.torsoPolicy`, stepped in `handle(_:)` right after `decider.update` with `rules: cueRules` and `crossingSettle: nav.isCrossingSettle`; `reset()` wherever the decider is reset and in `applyCueRules` (which also calls `haptics.stopAll()`). Tests: `TorsoHapticPolicyTests.swift` (23, suite "Torso haptics by level"). `torsoIsHeld(rules:crossingSettle:)` = `holdReason != nil`, read by `AppModel.handle` to stop a running centre loop before rendering a head cue under a hold (`torsoIsHeldNamesEveryHold`); a Standard onset carries `max(d, CueThresholds().centerNear)` (`standardOnsetDistanceIsFloored`). Side-lane decisions live in `side(_:report:rules:crossingSettle:now:)`; every `HapticCue` case is named in `update` (no catch-all). History is pruned with `removeAll` by age (a clock gap with no reset must not leave stale references).
...
### `TripLogRecord.swift` — the JSON object behind every trip-log line

`public enum TripLogRecord` (pure, stateless, nonisolated). `reservedKeys = ["t", "kind"]`; `make(t:kind:fields:) -> [String: Any]` builds `["t": t, "kind": kind]` plus every field, and a field named `t` or `kind` is stored as `field_t` / `field_kind` instead of replacing the record's own value (the 2026-09-11 phone log had `"hazard"` records renamed to `"sign"` by a colliding field). Caller: `TripLogger.event(_:_:)` (main actor; `t` = seconds since the logger started, one JSONL line per record). ⚠ A `field_kind` / `field_t` key in a real log is an app bug: `e2e.py` fails the run on it and `cue_audit.py` reports `field_collisions` — that is why cue events name their field `cue` and hazards `type`. Tests: `TripLogRecordTests` (2).

### `SpokenPhrases.swift` — every templated warning line, enumerated for the voice cache

`public enum SpokenPhrases` (nonisolated; lazily initialised `static let` sets). Why: `SpeechQueue` never lets a warning wait on the network, so a warning that missed the mp3 cache came out in the system voice while route lines came out in the natural voice ("it's switching back and forth"). The warning phrase space is finite (`noun + bucketed distance`), so it is enumerated here and prefetched at launch. The templates **live here** and production calls them — `obstacleLine(name:distance:)` (`ObstacleNamer`), `approachLine(distance:)` (`CueSpeechPolicy`) — so the strings are byte-identical by construction; `obstacleMaxDistance = 3.0`, `wallMaxDistance = 1.5`, `bucketSamples(from:below:)`, `obstacleNameLines`, `approachLines` (range from `CueThresholds`), `groundHazardLines` (`GroundHazard.spokenLine` over the detector's bin geometry), `signLines` (`SignPolicy.phrases`), `warningLines` (74 lines, 1,718 characters today), `warningCharacterBudget = 2_000` (one ElevenLabs synthesis per line on a 10,000-character/month tier; `spokenPhrasesStayInsideTheCharacterBudget` fails the build past it), `characterCount(_:)`. Callers: `AppModel.start()` sets `SpeechQueue.backgroundLines = warningLines`; `SpeechQueue.prefetch` appends them to every batch; `ElevenLabsVoice.prefetch` synthesizes what `VoicePrefetch.queue` says is missing. Fixed cue lines ("Head height.", "Left.", "Right.") are `AppModel.commonLines`, not here. Step 56: `shellLines` — every fixed line the voice shell can speak (help — the launch menus were removed in Step 67 and are no longer prefetched — item confirmations, the three cue-level lines, filler, timeout, `EmergencyConfirm.fixedLines`, `StatusSummary.fixedLines`, `notHeardLine` "I did not catch that.", `describerBusyText` "Still describing the previous scene."; `HeadCoverNotice.line` is not in it since Step 68 — pinned by `replacedWaitingLinesAreNotPrefetched`), deduplicated, built from the production constants; `shellCharacterBudget = 1_500`; `AppModel.commonLines` appends `shellLines`. Tests: `SpokenPhrasesTests` (12 + `shellLinesStayInsideTheirBudget`, `everyLineTheShellCanSpeakIsPrefetched`) — completeness is proven by sweeping the production functions over the production ranges.

### `VoicePrefetch.swift` — what the natural voice synthesizes ahead, in what order, how many at once

`public enum VoicePrefetch` (stateless, nonisolated). `maxConcurrent = 2` (the ElevenLabs free tier allows two concurrent requests and the app also makes a *live* request on a cache miss — a third slot would earn a 429 on the line a walker is waiting for; `prefetchConcurrencyLeavesRoomForALiveRequest`); `isFatal(status:) -> Bool` — true for 401, 403 and 422 only (a bad key / voice fails every remaining line identically; 429 / 5xx / transport are per-request bad luck; `aBadKeyStopsThePrefetchInsteadOfRepeatingItselfTwentyTimes`); `queue(_ lines:isCached:) -> [String]` — keeps order, drops cached lines (which is also what lets an interrupted warm-up resume instead of paying twice); `chunk = maxConcurrent` (lines per worker pass, `prefetchChunkIsTheConcurrencyLimit`); `merge(new:backlog:tail:isCached:) -> [String]` (Step 54, additive prefetch: `queue(new + backlog + tail)` — `aNewBatchGoesAheadOfTheBacklogAndTheTailFollows`, `mergeDropsCachedAndRepeatedLinesButNeverAnUncachedOne`). Callers: `ElevenLabsVoice.prefetch` and its error type's fatal check; `SpeechQueue.prefetchPass` (backlog, then `routeLines + backgroundLines`). Tests: `VoicePrefetchTests` (8) + `SpokenPhrasesTests.aCancelledWarmUpResumesInsteadOfStartingOver`.

### `VoiceEngineChoice.swift` — which voice speaks a line, decided once (Step 53)

`public enum VoiceEngine: String` (`natural = "elevenlabs"`, `system`, `race`, `muted` — ⚠ raw values are `speech_dispatch.engine`), `public enum VoiceEngineReason: String, CaseIterable` (`muted`, `no_key`, `natural_off`, `cached`, `warning_miss`, `breaker_open`, `race`; resolutions `race_won`, `race_timeout`, `race_failed`, `playback_failed` — ⚠ log contract), `public struct VoiceEngineDecision {engine, reason}`, `public enum VoiceEngineChoice`: `raceDeadline = 2.5` s (moved from `SpeechQueue`), `decide(muted:hasKey:naturalEnabled:cached:isWarning:breakerOpen:naturalUnavailable:)` (first match: muted → no key → natural off → **refused key (`naturalUnavailable`, Step 61/63, before the cache)** → cached → warning miss → breaker open → race; there is no `urgent` / `immediate` input — owner decision 2026-09-13 one voice), `resolvedEngine(_:)` (only `race_won` / `cached` are natural; `naturalUnavailable` is system), `describe(engine:reason:)` (words for the Scene engine card's Voice row and the Haptics voice pill label, never a raw value). Owner: `SpeechQueue.speakNow` (before `onDispatch`), `startRace` / `resolveRace`, `reportPlaybackFailure`. Tests: `VoiceEngineChoiceTests` (the table + 5 breaker tests).

### `NaturalVoiceLatch.swift` — refused key survives a restart (Step 63)

`public enum NaturalVoiceLatch` — `settingsKey = "naturalVoiceUnavailable"` (⚠ not in `LaunchRecovery.optionalFeatureKeys`: a crash recovery must not restore two-voice mixing), `persistedError` (same words as `StatusSummary.voiceLine` when unavailable), `shouldClearVoiceError(unavailable:)` (false when latched — a later prefetch must not wipe the Haptics-card 401). Owner: `SpeechQueue.applyPersistedNaturalVoiceLatch` / `markNaturalVoiceUnavailable` / `retryNaturalVoice` / `prefetch`. Tests: `NaturalVoiceLatchTests`, `LaunchRecoveryTests.recoveryNeverClearsTheRefusedVoiceLatch`.

### `SceneDescribePolicy.swift` — a lock drops a pre-lock "Where am I" (Step 63)

`public enum SceneDescribePolicy` — `maySpeak(started:current:)` is `started == current`. Owner: `SceneDescriber.run` / `cancelForBackground`. Tests: `SceneDescribePolicyTests.aLockGenerationDropsThePreLockScene`.

### `VoiceBreaker.swift` — the natural voice's session-sticky breaker (Step 54)

`public struct VoiceBreaker: Sendable, Equatable` — `probeInterval = 60` s; `Reason: String` (`race_timeout`, `race_failed`, `prefetch_succeeded` — `voice_breaker.reason`); `isOpen`, `lastProbeAt`; `mutating trip(_:now:) -> Bool` (opens; true on a state change), `mutating prefetchSucceeded(requested:) -> Bool` (closes only when open and `requested > 0`), `probeDue(now:)`, `mutating probed(now:)`. Time alone never closes it (replaces the 60 s `naturalVoiceFailedAt` timestamp). Owner: `SpeechQueue` (`tripBreaker`, `prefetchPass`, `armBreakerProbe`). Tests: `VoiceEngineChoiceTests` `breakerOpensOnARaceTimeoutOrFailure`, `breakerNeverClosesOnTimeAlone`, `breakerClosesOnlyWhenAPrefetchActuallyFetchedSomething`, `probeIsDueEverySixtySecondsWhileOpenAndNeverWhileClosed`, `breakerProbeIntervalAndReasonsArePinned`.

### `SelfHearFilter.swift` — OpenCane never answers its own voice (Step 55)

`public struct SelfHearFilter: Sendable, Equatable` — `window = 3` s, `tailSeconds = 0.3` (the tap stays paused after `isSpeaking` goes false; read by `VoiceInputEngine`), `minTranscriptCharacters = 3`; `Entry {line, at}`, `history`; `static normalize(_:)` (lowercase, punctuation / symbols → space, whitespace collapsed, digits kept), `static clauses(of:)` (split at `SpeechResume.clauseEnders` followed by whitespace or the end, normalised), `mutating record(line:at:)` (prunes past the window, ignores blanks), `mutating reset()`, `shouldDrop(transcript:now:) -> String?` (the matched line when the normalised transcript equals a recorded line or one whole clause of it within the window; never a substring match — the menu line contains "route"). Owner: `VoiceInputEngine` (`noteDispatched` records only while starting / listening; `stopListeningAndSubmit` drops → `voice_self_hear {action: dropped, transcript, matched_line}`). Tests: `SelfHearFilterTests` (8).

### `UtteranceEnd.swift` — when push-to-talk listening ends on its own

`public struct UtteranceEndDetector: Sendable, Equatable` with `enum Verdict { listening, endOfUtterance, timeout }`; `silenceAfterSpeech = 1.5` s, `maxListen = 10` s, `checkInterval = 0.25` s; `init(startedAt:maxListen:)` (the cap is per listen — default `maxListen`, a follow-up window passes `VoiceShellPolicy.followUpSeconds`, Step 58); `mutating update(transcript:now:) -> Verdict`. Never submits before any words (silence before the first word is thinking time); submits once a non-empty transcript has been unchanged for `silenceAfterSpeech`; the cap is a submit with words, a timeout without; deterministic (`now` is passed). Transcript stability rather than audio energy: `SFSpeechRecognizer` already runs a voice-activity detector and only emits a new partial when it heard more words. Owner: `VoiceInputEngine` creates one per listening session (`startEngine`), polls it every `checkInterval` and on every partial (`recognitionUpdate` → `checkEnd`); `.endOfUtterance` → `stopListeningAndSubmit(reason: "silence")`, `.timeout` → `"timeout"` (written to the `voice_end` record). Tests: `UtteranceEndTests` (8, incl. `followUpWindowUsesAShorterCap`).

### `VoiceMenu.swift` — the voice shell's eight words (Step 56)

`public enum VoiceMenu` (pure, nonisolated). `enum Item: String, CaseIterable` — `route`, `whereAmI`, `describe`, `status`, `repeatLast`, `quiet`, `help`, `emergency` (raw value = `conv_turn.ivr`); `word(_:)` ("route" … "emergency"), `digit(_:)` (1…8 in that order), `digitWord(_:)` ("one" … "eight"); `helpLine` = "One, route. Two, where am I. … Eight, emergency. Or say stop to end the route." (built from the items); `Item.confirmationLine` (short cached line per item; `quiet` reuses `CueLevel.quiet.spokenLine`, `help` is `helpLine`, `emergency` is `EmergencyConfirm.noContactLine`); `match(_:) -> Item?` (whole utterance after `normalize` — lower-case, punctuation dropped, whitespace collapsed; words, digit words, numerals, a few narrow aliases; **no homophones**: "won", "to", "for" are nil); `enum Verb { next, standard, detailed }` + `extraVerbs` table + `verb(_:)`; `confirmation(_:) -> Bool?` (yes / yeah / yep / confirm … → true; no / nope / cancel / don't call … → false). "stop" is deliberately in none of the tables (it stays the classifier's exact-phrase rule 1). Callers: `FastPathIntentClassifier` rule 0, `ConversationCoordinator`, `SpokenPhrases.shellLines`. Tests: `VoiceMenuTests` (9).

Step 67: `menuLine`, `shortMenuLine` and `launchMenuLine(firstLaunch:)` (Step 65) are deleted — launch says only "OpenCane ready." (`VoiceShellPolicy.launchLine`). The only list the shell speaks is `helpLine`, on "help" / "menu" / "options" / "seven" (`theOnlyMenuIsTheOneTheWalkerAsksFor`). The `heardFullVoiceMenu` UserDefaults key is no longer read or written (a stale value on a phone is harmless).

### `ConversationBudget.swift` — a cloud turn's latency budget; latest wins (Step 57)

`public struct ConversationBudget: Sendable, Equatable`: `fillerAfter = 1.5` s, `budget = 8.0` s (Step 67; was 4.0 — the cloud answered in 6–17 s on every logged turn), `thinkingRepeatAfter = 4.0` s (was 3.0), `maxThinkingTicks = 2`, `timeoutLine` "No answer." (Step 65; `fillerLine` removed, `Event.speakFiller` → `.thinking`, `fillerSpoken` is now `thinkingTicks > 0`); `enum Event { none, speakFiller, timeout }`; `begin(now:) -> Int` (new turn id; a turn in flight is superseded), `tick(now:) -> Event` (filler once, timeout once — timeout wins on a stalled tick), `finished(id:) -> Bool` (true only for the live turn; a superseded / timed-out / unknown id must not speak), `cancel()` (a fast-path query drops the live turn without starting a clock), `inFlightID`, `fillerSpoken`, `elapsedMs(now:)`. Caller: `ConversationCoordinator` (cloud path only). Tests: `ConversationBudgetTests` (8).

### `Earcon.swift` — calm feedback: seven tones and the tone-vs-words policy (Step 65)
`public enum Earcon: String, CaseIterable, Sendable` — `listening`, `heard`, `nothing`, `thinking`, `busy`, `done`, `error`; `struct Tone {hz, durationMs, gapBeforeMs, gainDBFS, partialRatio}`; `tones`, `durationMs`; `maxDurationMs` 180, `maxGainDBFS` −12, `sampleRate` 44 100, `attackMs` 4; `samples(gainOffsetDB:)` (linear attack, quadratic decay to 0, offset clamped ≤ 0), `wavData(gainOffsetDB:)` (16-bit mono PCM WAV). `public enum EarconPolicy` — `Feedback {earcon, gainOffsetDB, line}` (`.silent`), `ListenKind` press / launch / followUp / question, `Event` (listenOpened, listenHeardWords, listenSelfHeard, listenEmpty(kind, emptyPressCount), thinking, timedOut, cloudFailed, superseded, describerBusy, answer(characters), routeWarming, routeReady), `feedback(for:)`, `followUpGainOffsetDB` −8, `emptyPressesBeforeWords` 2, `emptyPressCount(previous:heardWords:)` (1, 2, 1, 2 …), `longAnswerCharacters` 100, `warmupTickInterval` 2 s, `warmupMaxTicks` 3, `warmupTickDue(elapsed:ticksSoFar:)`, `routeReadyLine` "Starting.", `gate(muted:safetySpeaking:)` → `.play` / `.skip(reason)`. Callers: `SpeechQueue.playEarcon` / `perform`, `VoiceInputEngine`, `ConversationCoordinator`, `SceneDescriber`, `AppModel.queueRouteStart` / `depthReadinessChanged`. Tests: `EarconTests` (14).

### `EmergencyConfirm.swift` — emergency by voice, confirmation-gated (Step 59)

`public struct EmergencyConfirm: Sendable, Equatable`: `confirmWindow = 8` s; `promptLine(name:number:)` "Say yes to call <name> at <number>." ("your emergency contact" when the name is blank; the number as stored); `canceledLine`, `callingLine`, `noContactLine`, `nothingPendingLine` and `fixedLines` (all four, prefetched); `telDigits(_:)` (digits + a leading plus — moved from `ProfilePage`, which now calls it); `enum Outcome { prompt(String), call(tel:), cancel, none, noContact }`; `emergency(now:name:number:)` (never `.call`; restarts the window; blank number → `.noContact`), `confirm(_:now:)` (`.call` only for a yes inside the window; clears the prompt), `isPending(now:)`, `expire(now:)` (true once when the window lapses). Callers: `ConversationCoordinator` (`.emergency` / `.confirm`), `ProfilePage`. Tests: `EmergencyConfirmTests` (9).

### `VoiceShellPolicy.swift` — when the shell may open the microphone by itself (Step 58) + `ScenePhaseReason` (Step 60)

`public enum VoiceShellPolicy`: `followUpSeconds = 5` (idle), `followUpSecondsNavigating = 3` (opt-in only), `speechDrainCap = 15` (named `menuWaitCap` until Step 67); `readyLine` "OpenCane ready.", `noLidarLine` "OpenCane. This phone has no LiDAR.", `launchLine(lidarSupported:)` — the whole launch announcement (Step 67, `launchSaysOnlyOpenCaneReady`); `launchListen(enabled:muted:speechAuthorized:micGranted:micOwnedElsewhere:recoveredLaunch:cameraDenied:) -> LaunchListen` (`.listen` / `.skip(reason:)` — first refusal wins: disabled, muted, permission_prompt, mic_owned, recovered_launch, camera_denied); `followUp(enabled:navigating:navigatingEnabled:queuedLines:answerWasQuestion:) -> FollowUp` (`.open(seconds:)` / `.skip(reason:)`; ON idle, OFF navigating unless opted in, never with lines queued, always after a question such as the emergency prompt). `public enum ScenePhaseReason { enum Phase { active, inactive, background }; classify(phase:protectedDataAvailable:sawProtectedDataWillBecomeUnavailable:) -> String }` → "foreground" / "inactive" / "locked" / "backgrounded". Pure; wired into `AppModel` by the voice-shell integration (part 2). Tests: `VoiceShellPolicyTests` (8), `ScenePhaseReasonTests` (3).

### `VoiceInputGuard.swift` — push-to-talk microphone safety

`VoiceInputGuard` is the Foundation-only lifecycle policy used by `VoiceInputEngine`. It generation-fences
permission callbacks, rejects HFP or missing input routes, stops on route replacement, interruption,
recognizer/engine failure or permission revocation, and ignores callbacks from a canceled request.
`SpeechQueue` remains the sole audio-session owner; the app adapter supplies route and analyzer events
and uses the existing speech/watch failure cues. Tests: `VoiceInputGuardTests`.

### `SensorModeInterlock.swift` — route-time AR configuration policy

`SensorModeInterlock` is the pure policy for settings that would re-run ARKit. It reserves the sensor
configuration while a route start is queued, refuses face-tracking or high-frame-rate writes while a
route starts or guides, defers thermal mesh restarts, and releases/apply-deferred decisions only when
navigation ends. `AppModel` owns effects and `DepthEngine` is the thin ARKit adapter. Tests:
`SensorModeInterlockTests`.

### `NavigationHealth.swift` — retained GPS freshness gate

`NavigationHealth.isFresh(timestamp:now:)` is the Foundation-only rule used by
`NavigationEngine.update(fix:)` and `tick(now:)`. A fix is usable only when both clocks are finite,
the timestamp is not in the future, and its age is **less than 5 seconds**. The app adapter reports
Core Location stream termination or authorization revocation through `LocationService.onFailure`;
the engine then clears distance/bearing/course evidence until a fresh fix arrives (since Step 68 the
spoken "GPS weak." is `GPSAnnouncer`'s, 10 s after its bad onset — review round Steps 67–68). Tests: `NavigationHealthTests.swift` (fresh,
boundary, invalid/future clocks).

### `QuietRouteSpeech.swift` — which route status lines are spoken (Step 68)

- **`struct GPSAnnouncer: Sendable, Equatable`** (retuned in the review round Steps 67–68) — `static weakLine = "GPS weak."`, `backLine = "GPS back."`; `static maxAccuracyM = 20`, `maxFixAge = 12 s`, `noFixGrace = 10 s`; vars `weakAfter = 10 s`, `backAfter = 10 s`, `weakRepeatInterval = 60 s`, `backInterval = 120 s`; private `badSince`, `goodSince`, `weakSpoken`, `lastWeakAt`, `lastBackAt`. `static badOnset(lastFixAt:accuracy:routeStartedAt:now:) -> TimeInterval?` — nil while good; `now` for accuracy < 0 or > 20 m or a backwards clock; `lastFixAt + NavigationHealth.maxFixAge` (the guidance pause) for a fix older than 12 s; `routeStartedAt + 10` for no fix once passed. `mutating update(badOnset:outdoors:now:) -> String?` — the bad stretch starts at the earliest onset and keeps running while not `outdoors` (nothing spoken then); weak after 10 s while outdoors, not within 60 s of the last weak, not while a back is pending; back after 10 s good, only after weak, not within 120 s of the last back (stays pending); non-finite `now` → nil. Owner `NavigationEngine.gpsAnnouncer` (fresh per route) via `announceGPS(now:)` (`lastFix`, `startedAt`); outdoors = `!isIndoorActive()` and not after `markLocationUnavailable(speak: false)`. The engine's own 5 s pause is unchanged. ⚠ Pinned by `theFlappingLogSaysNoGPSLines` (log 15-48-34Z fix times), `gpsAnnouncerNumbersArePinned`, `badOnsetFollowsAccuracyAgeAndNoFix`, `aStationaryWalkerWithGoodAccuracyIsSilent`, `trueOutdoorLossByAccuracyIsSpokenWithinTenSeconds`, `aRealOutageIsOnePairTenSecondsAfterGuidancePaused`, `repeatedDropoutsSayWeakAgainAfterSixtySeconds`, `aPendingBackIsNotFollowedByASecondWeak`, `backIsNeverSaidWithoutWeak`, `neverWhileIndoorsButTheBadClockKeepsRunning`, `noFixAtAllIsAnnouncedOnceAtTwentySeconds`.
- **`enum RouteStatusLines`** — `hapticsOnWatchLine`, `hapticsSpokenLine`, `screenLockedLine` "Screen locked. Obstacle warnings off."; `routeStartLine(headphonesConnected:watchPaired:watchReachable:hapticsHealthy:) -> String?` (only the haptics line; caller `AppModel.announceChannels`); `allSpokenLines` (GPS ×2, screen lock, haptics ×2, headphone ×2, "Close." — appended to `AppModel.commonLines`). ⚠ Pinned by `routeStartSaysOnlyTheHapticsLine`, `step68LinesAreShortAndPrefetched`.
- **`struct HeadphoneNotice: Sendable, Equatable`** — `routeDisconnectLine` "AirPods disconnected.", `idleDisconnectLine` "Headphones disconnected. Beacon paused."; `routeStarted()`; `mutating disconnected(navigating:) -> String?` (once per route while navigating; the idle line every time otherwise). Owner `AppModel.headphoneNotice` (`wireAudioRoute`, `startRouteNow`). ⚠ Pinned by `aMidRouteDisconnectIsSaidOnce`.
- Tests: `QuietRouteSpeechTests.swift` (10).

### `EmergencyContactSeed.swift` — emergency contact from Secrets (Step 68)

`enum EmergencyContactSeed` — `minDigits = 7`; `seeded(storedName:storedPhone:secretName:secretPhone:) -> (name, phone)?` — non-nil only when the stored phone is blank and the Secrets phone has ≥ 7 digits (`EmergencyConfirm.telDigits`); trimmed; a blank Secrets name keeps the stored name. Review round Steps 67–68: `struct Contact { name, phone, fromSetup }` and `effective(storedName:storedPhone:secretName:secretPhone:) -> Contact` (the seed marked `fromSetup`, else the stored contact). Caller `MedicalProfileStore.effectiveEmergencyContact` (computed on read with `Secrets.string("EMERGENCY_CONTACT_NAME" / "EMERGENCY_CONTACT_PHONE")`; never written into `profile`, so UserDefaults and the cloud mirror never see the seed). Tests: `EmergencyContactSeedTests.swift` (6, incl. `logRecordsMaskThePhoneNumber` for `EmergencyConfirm.logSafe`: runs of ≥ 7 digits → "[number]", caller `TripLogger.event` for `text` / `response` / `matched_line` / `transcript`).

### `PeopleDetection.swift` — validation gate for the optional people detector

`PeopleDetection.defaultEnabled` is `false`. `PeopleDetection.state(enabled:)` returns `.off` or
`.experimentalUnverified` (`"experimental_unverified"`); there is intentionally no validated state
without measured cane walks. `AppModel` logs the state at launch and on changes, and `HazardsCard`
keeps the caption visible to VoiceOver. Tests: `PeopleDetectionTests.swift`.

### `StopRouteConfirmation.swift` — accidental-stop guard

`StopRouteConfirmation` is a two-tap, **3-second** pure state machine. The first tap arms and the
second confirms; expiry or any unrelated action resets it. `GuideCard` keeps the exact
`"Stop route"` accessibility label, speaks/shows the armed state, and calls `AppModel.stopRoute()`
only on confirmation. Tests: `StopRouteConfirmationTests.swift`.

### `TripRefreshGeneration.swift` — HealthKit result fencing

`TripRefreshGeneration` issues a monotonically increasing token for each active trip. `TripTracker`
captures it before every asynchronous HealthKit authorization/statistics request and accepts a
result only if the token still belongs to the active trip. `cancel()` invalidates the token. Tests:
`TripRefreshGenerationTests.swift`.

### `HeadNodDetector.swift` — the AirPods "double nod to talk" gesture

`public struct HeadNodDetector: Sendable, Equatable` — `minAmplitudeDeg = 15`, `maxNodSeconds = 0.8`, `pairWindowSeconds = 2.5`, `refractorySeconds = 3`, `turnBandDeg = 3`; `mutating update(pitchDeg:now:) -> Bool`, `reset()`. Tracks pitch turning points with a hysteresis band; a nod is rest → ≥ `minAmplitudeDeg` away → ≥ `minAmplitudeDeg` back, all within `maxNodSeconds` of the head *leaving* rest (a slow tilt never counts); a double nod is two non-overlapping nods completing within `pairWindowSeconds`; fires **exactly once** per pair, then silent for `refractorySeconds`. Direction-agnostic on purpose (the sign of `CMAttitude.pitch` for "chin down" is unmeasured). ⚠ Every number is an **untuned placeholder** — re-measure from `head_nod` trip-log events (logged by `AppModel` with `pitch_deg`, including ignored nods). Owner: `HeadPoseTracker` (passes `CMDeviceMotion.timestamp` and pitch in degrees) → `AppModel.startVoiceInput()`, behind the off-by-default "Nod to talk". Tests: `HeadNodDetectorTests` (7).

### `HeadYawSources.swift` — head yaw without AirPods: face-anchor geometry, tracker, source selector

Where the beacon's listener orientation comes from when the walker has no AirPods (owners: `FaceHeadPose`, `AppModel.startTicker`, `AppModel.recenter`).
- `public enum FaceYawGeometry` — `minimumHorizontalMagnitude = 0.15`; `worldYawDegrees(forwardX:forwardZ:) -> Double?` — the `ARFaceAnchor.transform` third column (the face's +Z, the direction the walker looks) projected onto the horizontal plane, `atan2(x, −z)` clockwise from the world's −Z under `worldAlignment = .gravity`, so **positive = turned right**, the same sign as `HeadPoseTracker.headYawDeg`; nil for a head tipped too far.
- `public struct FaceYawTracker: Sendable, Equatable` — `smoothing = 0.35`, `maxAge = 0.7` s, `maxJump = 45`°, `maxConsecutiveRejections = 3`, `publishInterval = 1/15` s (the anchor relay hops to main at most this often); `smoothedWorldYaw`, `referenceYaw`, `lastSampleTime`; `ingest(worldYawDeg:now:)`, `isFresh(now:)`, `yawDeg(now:)` (relative to the reference; nil until one exists or when stale), `recenter()`, `recenterWhenReady(now:)`, `reset()`.
- `public enum HeadYawSource { none, airPods, face }`, `public struct HeadYawChoice { degrees, source }`, `public enum HeadYawSelector.choose(airPodsYaw:faceYaw:…) -> HeadYawChoice` — AirPods beat the camera whenever they have data (on the head, work in the dark, no face-in-cone dependency); a stale face is no face and the selector falls back to 0 rather than steering the beacon with an old pose.
- Tests: `HeadYawSourcesTests` (18).

### `DepthSnapshot.swift` — a small scene-space LiDAR grid so Vision detections get a real distance

`public struct NormalizedBox: Sendable, Equatable` (Vision's convention — origin **bottom-left**, 0…1 on the upright portrait image; `minX`, `minY`, `width`, `height`, `midX`, `midY`) and `public struct DepthSnapshot: Sendable, Equatable` — `defaultCols = 16`, `defaultRows = 24`, `cols`, `rows`, `values`, `empty`, `isEmpty`, `value(col:row:)`, `distance(inVisionBox:sampleFraction: 0.4) -> Float?` (flips y itself; `trusted = 0.3...5.0` m; **nil when unsure — the caller then says the direction alone**, never a borrowed distance), two `static make(...)` overloads (the raw one reads caller-locked pointer memory). Geometry identical to `LaneMath` (scene space after the portrait remap; `DepthFrameProcessor.jpegSnapshot` rotates the camera buffer the same way, so a Vision box on that JPEG and this grid describe the same pixels); built **unmirrored**, `mirrorLeftRight` is applied to the spoken side (`PeopleAhead.bearing`). Cost, measured: `make` 19.3 µs per published depth frame vs `LaneMath.computeLanes` 47.0 µs; `distance` 0.1 µs, once per "Where am I". Flow: `DepthFrameProcessor.computeGrid` per published frame → `jpegSnapshotWithDepth` hands it out only when its frame time matches the retained image (`maxDepthPairingSkew`), else `.empty` → `SceneDescriber` → `SceneContext.setDepth` → `OnDeviceVLMClient` calls `distance(inVisionBox:)` per `Sighting`. Tests: `DepthSnapshotTests` (14).

### `MultiCamDepth.swift` — the verdict on "both cameras plus LiDAR depth", and `MultiCamCost`

`public struct MultiCamDepthFindings: Sendable, Equatable` (`multiCamSupported`, `frontPlusDepthDeviceSets`, `depthCapableMultiCamFormats`, `bestDepthWidth`, `bestDepthHeight`; filled by `MultiCamDepthProbe.measure()` in the app), `public enum MultiCamDepthVerdict: String, CaseIterable` (`multiCamUnsupported`, `noDepthInMultiCam`, `depthButNotWithTheFrontCamera`, `depthAtOrBelowARKitResolution`, `depthAboveARKitResolution`; `keepsDepth`), `public enum MultiCamDepth` (`arkitSceneDepthWidth = 256`, `arkitSceneDepthHeight = 192`; `verdict(_:)` — "supported" needs a *pairing* of a front camera with a depth-capable back camera in one device set; resolution compared to ARKit's `sceneDepth` by pixel count; `sentence(_:)`). Decides nothing at run time: `AppModel.logMultiCamDepthProbe()` writes one `multicam_depth` record per launch so a real walk's log holds a defensible yes/no. `public enum MultiCamCost` is the part that **does** run: `maximumHardwareCost = 1.0`, `reducedFramesPerSecond = 24`, `needsFrameRateReduction(hardwareCost:)` — read by `DualCameraSession.reduceFrameRateIfOverBudget(_:)`. Nonisolated `Sendable` throughout. Tests: `MultiCamDepthTests` (8).

### `PeopleAhead.swift` — Vision body / animal detections → "About 3 meters ahead, two people."

Why: the 1,303-class scene classifier fails silently (real log: `vision_error: "no labels (1303 raw)"` with people in front of the walker); `DetectHumanRectanglesRequest` / `RecognizeAnimalsRequest` are narrow models that answer *where* as well as *what*. `public enum SightingKind: String, Codable, CaseIterable { person, dog, cat }` (`vocabularyNoun`, singular / plural, priority, per-kind `minConfidence`), `public enum SightingBearing { left, ahead, right }` (`spoken`), `public struct Sighting: Sendable, Equatable` (`kind`, `box: NormalizedBox`, `confidence`, `distance: Float?`), `public enum PeopleAhead` — `aheadHalfWidth = 0.12`, `maxCounted = 3`, `maxGroups = 2`; `bearing(midX:mirrored:)`, `countPhrase(_:count:)`, `distancePhrase(_:)`, `line(_:mirrored:) -> String?`, `nouns(_:)` (feeds `SceneVocabulary.isFaithful` so a sentence naming people passes the gate), `needsSpeaking(_:given:…)`, `summary(_:)` (→ `describe_result.people` via `OnDeviceVision.lastPeople`). A sighting is a **fact** like the LiDAR distance: `line` is spoken whether or not the language model produces anything. Callers: `OnDeviceVLMClient.describe`, `SceneDescriber` (cloud path prepends `line` unless the model sentence already carries the fact); the detectors run only for a scene answer while `AppModel.namePeopleEnabled` (never for the hazard watch or signs); `DepthSnapshot.distance(inVisionBox:)` attaches the distance. Stateless, nonisolated. Tests: `PeopleAheadTests` (28).

### `CloudSceneGate.swift` — the faithfulness gate for a cloud "Where am I" sentence

`public enum CloudSceneGate` (pure, stateless) — `struct Verdict: Sendable, Equatable` (with a `note` logged as `gate` in `describe_result`); `check(_ sentence:lidar:ocr:…) -> Verdict` and `sanitized(...)`. Rules, each backed by a measurement (file header): **counts** are stripped (`stripCounts`; small VLMs count at ~53 %), **distances** are refused (`units`; below chance — only the LiDAR number is ever spoken), **unread street names** are refused (`unreadName(in:ocr:detectedNouns:)`, `streetAbbreviations`, `titleCaseRatio = 0.75`; the model produced "S Grand Ave" where Vision read no text), **reassurance** is refused (`reassuranceWords` / `reassurancePhrases` / `reassurance(in:)`; a solid white frame got "The path ahead is clear" 4 of 4 — only the sensors may clear a path); `firstSentence`, `cap(maxWords: 20)`, `bare`; Step 49: `tooDark = "It is too dark to see."` (what `ScenePrompt` asks for on a black frame) and `isTooDark(_:)` (token-equal, so a dropped full stop or quotes still match) — `check` returns the canonical sentence with note "spoken" before any other rule; `SceneDescriber` speaks it as the answer (LiDAR line in front as usual) and never adds its own dark caveat to it (`tooDarkPassesTheGateUnchanged`). Owner: `SceneDescriber` calls it between the cloud reply and speech (`grounded(_:jpeg:lidar:)` for "Where am I", `groundedAnswer` for "Ask OpenCane") after re-running `OnDeviceVision.detect` on the same frame; nil → the on-device description is spoken instead (a question says "I can't answer that." first). `reassurance(in:)` is also the shared refusal table for `HazardWatchPolicy.line` and `ConversationResponseParser.sanitizeSpoken`. Tests: `CloudSceneGateTests` (16; fixtures are the measured hallucinations), plus `hazardWatchReassuranceIsRejected` (HazardTests) and the parser tests in `ConversationLogicTests`.

### `SpeechLoadPolicy.swift` — rationing the one optional, unsolicited speech channel (Step 30)

`public enum SpeechLoadClass { normal, ambientObstacleName }`, `public enum SpeechSuppressionReason { calmWindow, busy, invalidTime }`, `public enum SpeechLoadDecision { speak, suppress(reason:) }`, `public struct SpeechLoadPolicy: Sendable, Equatable` with `Configuration(minimumAmbientGap: 7.0)` (negative → 0, non-finite → 7; **[H] a calibration hypothesis, not a measurement** — tune from `speech_suppressed` with a blind / O&M-trained tester), `mutating admit(_:now:isBusy:…) -> SpeechLoadDecision`, `reset()`. Only `.ambientObstacleName` (the `ObstacleNamer` line, TTL 4 s, from `AppModel`) is ever rationed; route, safety, haptic-fallback and user-requested speech are `.normal` and always speak. Owner: `SpeechQueue` holds one `loadPolicy` and calls `admit` inside `say` (after trimming, before queue insertion; `isBusy` = speaking, incl. the pause between bands, or interrupted); `stopAll()` → `reset()`; a suppressed line goes to `SpeechQueue.onSuppressed` → trip-log `speech_suppressed {text, load, reason}`. Research: `docs/auditory-load.md`. Tests: `SpeechLoadPolicyTests` (7).

### `CameraControlGate.swift` — when a Camera Control / volume press may describe (Step 67)

`public struct CameraControlGate: Sendable, Equatable` — `launchGraceSeconds = 5`, `debounceSeconds = 2`, `burstWindowSeconds = 1.5`, `burstPresses = 3`, `pendingMaxAgeSeconds = 30`, `pendingSettleSeconds = 0.5`; `enum Verdict { describe, skip(reason:) }`; `enum PendingOutcome { none, wait, fire, drop(reason:) }`; `launchedAt`; `hasPendingDescribe`; `init(launchedAt:)`; `mutating press(now:listening:launchLinePending:commandGeneration:) -> Verdict` — first refusal wins: `launch_grace` (< 5 s after launch), `listening` (voice input listening or starting), `launch_line` (the launch line is pending) — each of these arms one pending describe unless the press came < 2 s after the previous press (a grip) — then `grip_burst` (≥ 3 presses within 1.5 s), `debounce` (< 2 s after the last **accepted** press; review round Steps 67–68 — it used to restart on refused presses and could lock the button out indefinitely); a backwards clock refuses. `mutating resolvePending(now:listening:launchLinePending:commandGeneration:) -> PendingOutcome` — `.drop("other_command")` when the generation changed, `.drop("expired")` after 30 s, `.wait` while a launch state holds or for 0.5 s after they clear, then `.fire` (counts as the accepted press). Evidence: `canekit-2026-09-13T15-48-34Z.jsonl` — three `describe {trigger: cameraControl}` at 1.71 / 2.89 / 3.56 s while "OpenCane ready." spoke, two busy earcons, an unasked description at 14.9 s. Owner: `AppModel.cameraControlPressed` (re-created in `start()`; `.skip` → `describe_skipped {reason, trigger: cameraControl, pending}`) and `AppModel.watchPendingCameraControl` (0.25 s poll → `describe_deferred {action: fired | dropped, reason}`; generation = `ConversationCoordinator.queryGeneration`). Tests: `CameraControlGateTests` (14).

### `LaunchRecovery.swift` — no optional feature may ever keep the app from starting

Evidence: the 2026-09-12 phone logs — "Head tracking without AirPods" was turned on and persisted; the next launch wrote three records and died in the ARKit warm-up, before the walker could reach the switch. Rule: *an optional feature must never be able to prevent the app from starting; a persisted optional feature that fails degrades to off, says so, and the app runs.* `public enum LaunchMode: String { normal, recovered }`; `public enum LaunchRecovery` — `healthySeconds = 10`, `markerName = "launch-in-progress"` (an empty **file** in Application Support, not `UserDefaults`), `optionalFeatureKeys = ["faceHeadTrackingEnabled", "groundHazardsEnabled", "hazardWatchEnabled", "signsEnabled", "highFrameRateCamera", "familyAlertsEnabled", "fallDetectionEnabled"]`, `mode(previousLaunchCompleted:)`, `spokenLine(for:)` (Step 67: one short sentence, "Extra camera features are off after a crash, but guidance still works." — no card names). Owners: `Settings.launchMode` in AppModel.swift (a `static let` evaluated before the first setting is read: checks the marker and, when recovering, removes every key in `optionalFeatureKeys`), `AppModel.start()` (`Settings.armLaunchMarker()` before any engine starts; `Settings.markLaunchHealthy()` after `healthySeconds` or when the walker deliberately backgrounds the app), `AppModel.announceLaunchRecovery()` (speaks `spokenLine`, logs `launch_recovery`); the `start` record carries `launch`. Tests: `LaunchRecoveryTests` (6).
...
### `SceneEngineSummary.swift` — the words on the Details tab's "Scene engine" card (Step 47)

Why: the owner asked for the Details tab to be specific about *when Muse answers and when the on-device model does*; until Step 47 the only evidence on screen was the "Muse + On-device" pill. Pure wording, so every string is tested and the two numbers in it come from the app's constants, never a second literal.

- `public enum DescribeTrigger: String, CaseIterable, Codable` — `button`, `watch`, `actionButton`, `cameraControl`, `waypoint`, `question` (no separate Siri case: `WhereAmIIntent` cannot tell Siri from the Action button, and a spoken question is `.question`); raw values are a trip-log contract (`describe {trigger}`, `describe_result {trigger}`); `spoken` ("the Guide button", "the watch", "the Action button or Siri", "Camera Control", "a waypoint", "a question"). Used by `SceneDescriber.describe(trigger:)` / `AppModel.describeScene(trigger:)`.
- `public struct SceneEngineFacts: Sendable, Equatable` — everything the card may know (memberwise init, every field required): `cloudName: String?` ("Muse", nil on-device only), `onDeviceName`, `lastSource`, `lastCloudMs`, `lastLatencyMs`, `lastFallbackReason`, `lastGate`, `lastError`, `secondsSinceLast`, `lastTrigger`, `hazardWatchOn`, `hazardWatchIntervalS` (`HazardWatchPolicy.interval`, 8), `hazardCloudDeadlineS` (`FallbackVLMClient.hazardDeadline`, 2.5; nil without a cloud), `lastWatchSource`, `lastWatchMs`, `lastWatchReason`, `secondsSinceWatch`, `lastWatchError` (defaulted nil; `HazardScanner.lastError` after a check that threw — `hazardWatch` then says "Hazard watch: last check failed — <reason> · 30 s ago", `hazardWatchFailureIsSaidAsAFailure`), `cueLevelTitle`, `cuePlaceTitle`, `namesOn`; Step 49 (all defaulted): `lightState: LowLightPolicy.State` (`.unknown`), `ambientLux: Float?`, `torchOn`, `torchByApp`.
- `public struct SceneEngineLine { text, spoken }` — the screen string (short units "1.9 s", "2 min ago") and the VoiceOver sentence (full words; VoiceOver reads "s" as a letter).
- `public enum SceneEngineSummary` — `justNowWindow: TimeInterval = 10` (the "just now" age window and `SceneEngineCard.ageRefresh`); `latency(ms, spoken:)` ("850 ms" / "1.9 s"), `seconds(_:spoken:)` ("2.5 s", "8 s"), `age(_:spoken:)` (< 10 s "just now", "45 s ago", "2 min ago", "1 h ago"); rows: `chain` ("Muse → On-device" / "On-device only"), `lastAnswer` ("Not asked yet." / "No camera frame." / "Failed — <error>" — the row caption already says WHERE AM I / "Muse answered in 1.9 s" / "On-device answered — Muse: The request timed out. (after 18 s)" / "On-device answered in 420 ms"), `when` ("2 min ago · from the watch", nil before the first run), `gate` (only when the cloud answered: "Muse's sentence passed the gate." / "… was edited: dropped count." / "… was refused: invented distance. On-device spoke instead."), `hazardWatch` ("Hazard watch off — when on, asks Muse every 8 s while a route guides, on-device after 2.5 s" / "Hazard watch on — …. Nothing asked yet." / "Hazard watch: Muse answered in 1.2 s · 30 s ago" / "Hazard watch: On-device answered — Muse: The request timed out. · 30 s ago"), `light` (Step 49: "Light: unknown" / "Light: lit (640 lux)" / "Light: dark (12 lux) · flashlight on (by OpenCane)" / "… · flashlight on" / "Light: dark (12 lux) · flashlight off — cameras may miss things"; spoken "Light: dark, 12 lux. Flashlight off, so the cameras may miss things. Obstacle detection still works."; `lightRowSaysWhatTheCamerasHave`, `lightRowNamesWhoLitTheTorch`, `lightRowUnknownBeforeAnEstimate`), `cues` ("Detailed · Outdoors · names off"), `spokenSummary` (all rows in that fixed order, light before cues). Owner: `SceneEngineCard` (ContentView.swift). Tests: `SceneEngineSummaryTests.swift` (20).
...
### `LowLight.swift` — is it dark, and what to do about it (Step 49)

Why: the owner asked how the app copes with low light. LiDAR depth, the gyro gate, GPS, the compass and haptics do not care; ARKit's visual tracking, sign reading, the on-device scene words / people detection, the cloud "Where am I" and the hazard watch degrade silently — and a blind walker cannot tell that it is dark. ⚠ Every number is a research hypothesis [H] until a dark-room walk (`light` records) tunes it; `defaultsAreTheDesign` pins them.

- **`struct LowLightPolicy: Sendable, Equatable`** — `Configuration(darkLux: 40, litLux: 120, enterSeconds: 3, exitSeconds: 5, smoothingSeconds: 0.3, minTorchOnSeconds: 60, torchBackoffSeconds: 60, litWithTorchLux: 400)` (clamped / repaired in `init`; `litLux` kept above `darkLux`, `litWithTorchLux` above `litLux`); `static minBatteryPct = FamilyAlertLimits().lowBatteryPct` (20). `State: String` — `unknown` (before the first estimate; always in the simulator), `lit`, `dark` (raw values are the `light {state}` log contract). `Step { state, didEnterDark, didExitDark, smoothedLux }`. `update(lux: Float?, now: TimeInterval, torchByApp: Bool = false) -> Step` — time-based EMA (α = 1 − e^(−dt/τ)) on the ARKit clock; nil changes nothing; dark needs the smoothed lux under `darkLux` for `enterSeconds`, lit needs it over `litLux` for `exitSeconds`; from `unknown` a not-dark first estimate resolves to `lit` at once, a dark one waits the full dwell; a clock jump back restarts the EMA and dwells but keeps the state; while `torchByApp` the exit also waits out `minTorchOnSeconds` from the first frame that saw the torch AND the exit threshold becomes `litWithTorchLux` (400 lux) instead of `litLux` — the torch is an auto-exposure proxy's worst enemy, it raises the reading, and with the plain exit it switched itself off by its own glow every minute (`appLitTorchGlowDoesNotEndTheEpisode`). `probeDue(now:)` / `beginProbe(now:)` / `endProbe(now:) -> Bool` (Step 50 review): dark, app-lit past the minimum on-time, reading in the dead zone (`litLux` < lux ≤ `litWithTorchLux`), last probe ≥ `probeIntervalSeconds` (60) ago → the app switches its torch off for `probeSeconds` (1.0), `update` runs with `torchByApp: false` without clearing `appTorchSince`, and `endProbe` returns true (state `.lit`, torch stays off) when the ambient reading is over `litLux`, false otherwise (the app re-lights). `AppModel.beginTorchProbe` / `setTorchQuietly` / `torchProbing` drive it with both torch confirmations muted; `light {action: probe_begin | probe_lit | probe_dark}`. `torchCutByDevice(now:)` (a thermal cut-out) starts a `torchBackoffSeconds` backoff; `canAutoLight(now:batteryPct:) -> Bool` (not backed off, battery > `minBatteryPct`; nil / negative battery = unknown = allowed). Owner: `AppModel.lowLight`, stepped once per report in `updateLowLight`.
- **`struct LowLightAdvice: Sendable, Equatable`** — `decide(state:entered:torchOn:torchAvailable:navigating:autoTorch:autoTorchAllowed:)` → `turnTorchOn` (only on the enter edge, only while navigating, only with the setting on and `autoTorchAllowed`, only a torch that exists and is off), `spokenLine` (enter edge only: `darkLine` "Low light. Obstacle detection still works." or `darkLineWithTorch` "… Flashlight on."; both in `AppModel.commonLines`), `visionCaveat` (dark and no torch lit or being lit). `allSpokenLines`.
- Tests: `LowLightTests.swift` (11, suite "Low light": `darkNeedsThreeSecondsUnderForty`, `litNeedsFiveSecondsOverOneTwenty`, `flickerDoesNotToggle`, `unknownUntilFirstEstimate`, `adviceTurnsTorchOnOnlyOnARoute`, `adviceSpeaksOncePerEpisode`, `defaultsAreTheDesign`, `appTorchStaysOnAMinute`, `thermalCutOutBacksOffAMinute`, `lowBatteryNeverAutoLights`, `clockJumpBackResetsDwellOnly`).

### `IndoorRoute.swift` — indoor step script → outdoor route handover (Step 62)

The indoor leg of "take me from the lab to CIF": a human-recorded step script advanced by the pedometer, then a GPS-gated handover to the outdoor route. Script lines are route facts spoken at `.nav` like a waypoint `say` (never the scene / obstacle path, never a `speech` log record). Pure Foundation, no clock (callers pass `now` / `t`). Callers: `IndoorGuide` (app), the Settings "Record indoor route" card, `ConversationCoordinator` (`fromAliases` for `.routeFromTo`).
- **`enum IndoorTurn: String, Codable`** — `none`, `left` (positive yaw), `right`, `around`.
- **`struct IndoorStep: Codable, Sendable, Equatable`** — `say` (≤ 140 chars), `steps: Int?` (nil = only "next" advances), `turn` (JSON optional → `.none`), `landmark: String?`. `init(say:steps:turn:landmark:)`.
- **`struct IndoorExit`** — `say`, `lat`, `lon`, `radiusM` (JSON optional → 25); `coordinate`.
- **`struct IndoorScript`** — `id`, `name`, `fromAliases`, `outdoorPlaceID` (a `CampusPlaces` id), `walked` (false = floor-plan draft), `recordedAt?`, `strideM?`, `steps`, `exit`. JSON keys are the property names (camelCase, unlike the route file). Constants `defaultStrideM` 0.7, `defaultRadiusM` 25, `radiusRangeM` 10…60, `maxSayCharacters` 140. `stride` (strideM or 0.7); `static load(from:) throws`; `validate() -> [String]` (no / blank aliases, empty steps, count ≤ 0, blank say, a say / landmark / exit say over 140, radius outside 10–60).
- **`enum IndoorEvent`** — `.say(String)`, `.advanced(Int)`, `.reachedExit`.
- **`struct IndoorProgress`** — `advanceFraction` 0.85, `landmarkFraction` 0.7, `maxJumpLines` 2, `notWalkedLine` ("Draft route. Use your cane.", Step 65). `init(script:)`; `start()` (caveat when `!walked`, then step 0's say; once); `update(stepsWalked:)` (cumulative, lower values ignored) — a step of n advances at ceil(0.85·n) walked in it, its landmark once at ≥ 0.7·n; step boundaries are the **nominal** counts (no drift after early advances); the passed step's unsaid landmark goes out with the advance; a jump keeps every `.advanced` but at most 2 lines (first and last step / exit line; with one step line, it and the landmark after it); last step → exit say + `.reachedExit`, then everything is ignored; `next()` advances regardless and re-bases the count at the latest pedometer value; `index`, `atExit`, `started`, `currentSay` (step say, or exit say — for "repeat"), `walkedInStep`. update / next call `start()` first if needed.
- **`struct IndoorHandover`** — `goodAccuracyM` 15, `consecutiveFixes` 3, `forcedRecentAccuracyM` 30, `forcedRecentWindowS` 20, `waitingAccuracyM` 20, `gpsOnlyAccuracyM` 10, `gpsOnlyRadiusM` 15, `gpsOnlyFixes` 3 (review round). `init(exit:)`; `reachedExit()` arms; `fix(lat:lon:accuracyM:now:) -> Bool` — after arming, 3 consecutive fixes ≤ 15 m within `radiusM` hand over; worse than 15 m neither counts nor resets; a good fix outside resets; before arming the armed rule never counts (fixes are remembered 20 s for `forced`), but the **GPS-only** run hands over after 3 consecutive fixes ≤ 10 m within 15 m of the exit (a locked phone pauses the pedometer; any other fix resets); invalid (non-finite / negative) ignored. `forced(now:) -> Bool` ("I'm outside") — at once if a fix ≤ 30 m accuracy within the exit's `radiusM` arrived in the last 20 s (Muse M4; pre-exit fixes may count because the walker's words are the evidence), else `waitingForGPS` and the first fix ≤ 20 m within `radiusM` hands over. True exactly once (`handedOver`).
- **`struct IndoorRecorder`** — `turnThresholdDeg` 60, `turnHoldS` 1.5, `aroundThresholdDeg` 150, `defaultExitSay`. `enum Event { stepCount(Int, t:), yaw(Double, t:), landmark(String, t:), finish(lat:lon:t:) }`; `handle(_:)` (ignored after finish). A new step starts when |yaw − step heading| ≥ 60° is held ≥ 1.5 s; before the first pedometer step the start heading follows the walker (a turn there is no step); while a new turn step has no steps its angle grows with a turn that keeps going (a slow U-turn still ends `around`); a second turn before walking re-measures the same step. `script(id:name:fromAliases:outdoorPlaceID:recordedAt:exitSay:radiusM:distanceM:) -> IndoorScript?` — nil before finish / with no steps; zero-step steps dropped (landmarks move back); says "Walk straight about N steps." (first), "Turn left|right|around. Walk about N steps." ("1 step"); landmarks joined with terminal punctuation; `strideM` = distanceM / total steps when a distance is given; `walked: true`. `say(turn:steps:)`, `turn(for:)`, `stepCountsSoFar`, `isFinished`, `exit`. `static isLandmarkText(_:) -> Bool` (review round) — false for blank text and for `FastPathIntentClassifier` `.stopRoute` / `.indoorOutside` / `.emergency` / `.confirm`, so the landmark hook never swallows a command (`landmarkHookLetsCommandsThrough`).
- Tests: `IndoorRouteTests.swift`.

- **App support (Step 62 app half, pinned by the `catalog…`, `exitAverager…`, `indoorStatus…`, `statusRouteClause…`, `yawUnwrapper…`, `simSteps…`, `recorderTimings…` tests):**
  - `enum IndoorOutdoorLeg` — `.demoRoute` / `.navigate(String)`.
  - `enum IndoorScriptCatalog` — `defaultRecordingID = "isr_townsend_to_front_doors"` (a recording replaces the ISR draft); `merged(bundled:recorded:)` (recorded replaces by id in place, new ids appended); `script(forOrigin:in:)` (`CampusPlaces.normalize` whole-alias match, walked beats draft, "" matches nothing); `outdoorLeg(destination:)` (nil / blank / any `cif` alias → `.demoRoute`, else `.navigate(trimmed)`); `sanitizedID(_:)` (lower case, `[a-z0-9_-]`, others `_`, blank → default).
  - `struct IndoorExitAverager` — "Finish at the exit": `windowS` 8, `goodAccuracyM` 15 (= `IndoorHandover.goodAccuracyM`), `enoughGoodFixes` 5; `add(lat:lon:accuracyM:t:)` (invalid or out-of-window ignored), `isDone(now:)`, `result()` (mean of good fixes, else the last fix ≤ `fallbackAccuracyM` 30 m, else nil), `goodFixCount`; review round: `fallbackAccuracyM` 30 (= `IndoorHandover.forcedRecentAccuracyM`), `fallbackMaxAgeS` 20 (= `forcedRecentWindowS`), `static usableFallback(accuracyM:ageS:)` for the last-known fix (`exitAveragerFallbackNeedsAUsableFix`).
  - `enum IndoorStatus` — `statusClause(index:count:atExit:)` "Indoors: step 3 of 5." / "Indoors: at the exit, waiting for GPS."; `guideLine(...)` "Indoors · step 3 of 5" / "Indoors · at the exit" (steps count from 1).
  - `struct IndoorYawUnwrapper` — `unwrap(radians:) -> Double?`: CMAttitude yaw → unwrapped degrees (shortest-way delta per sample; non-finite → nil).
  - `enum IndoorSimSteps` — `defaultRate` 1.8, `maxRate` 5; `rate(from:)` (the `CANEKIT_INDOOR_SIM_STEPS_PER_S` text: positive, clamped, else nil); `steps(rate:elapsedS:)` = floor(rate·t), ≥ 0.
  - `extension IndoorRecorder` — `yawSampleIntervalS` 0.1 (10 Hz), `landmarkWindowS` 20 (the one-shot landmark hook's life).
  - `StatusFacts.indoorClause: String?` (StatusSummary.swift) — while set and no outdoor route runs, `StatusSummary.routeLine` returns it instead of "No route running."
### Tests — `ios/Logic/Tests/CaneKitLogicTests/` (Swift Testing, `@testable import CaneKitLogic`) — 626 tests passing in 10 suites at Step 49 (`make test`); recount with `grep -rhoE '^\s*@Test' ios/Logic/Tests | wc -l`
**CueDeciderTests.swift** (26; helper `report(head:torso:trusted:headCoverage:torsoCoverage:)` builds a depth-available report; coverage defaults all true)
...
**GeoMathTests.swift** (26; fixtures `wps` = 2-waypoint ISR→CIF, `line` = 4 waypoints ~100 m apart due north, radii 15/15/15/20)
...
**TorsoHapticPolicyTests.swift** (23, suite "Torso haptics by level"; `Rig` steps one `CueDecider` and one `TorsoHapticPolicy` together, as `AppModel.handle` does) — `quietRendersNoTorsoHaptics`, `quietStillRendersHead`, `standardCenterTapsOnceOnClosingOnset`, `standardNeedsClosing`, `standardStrongTripleFiresOnASlowCreep`, `torsoIsHeldNamesEveryHold`, `standardOnsetDistanceIsFloored`, `staleHistoryIsPrunedAfterAGap`, `standardStrongTripleBelowSixtyCentimetres`, `standardStrongTripleAloneWhenNearIsSkipped`, `standardDoesNotRepeatWhileDistanceHolds`, `standardRearmsAfterClear`, `standardSuppressesSideTaps`, `untrustedFramesDoNotFeedTheClosingHistory`, `detailedIsTodayForCenterAndSides`, `steadySideDistanceIsShoreline`, `dropoutBreaksShoreline`, `indoorsSuppressesTorsoAtEveryLevel`, `crossingSettleHoldsTorsoTaps`, `aRunningLoopStopsWhenTorsoBecomesHeld`, `headIsNeverSuppressed`, `resetForgetsHistoryAndArming`, `thresholdDefaultsAreTheDesign`. Plus `CueProfileTests.defaultRulesRenderTodaysHaptics`.
...
- CueSpeechPolicy (rewritten Step 52 for the onset payload): `headHeightIsSpokenOncePerEpisode` (onset spoken, 1.0 m band silent, under 0.6 m spoken, never a third), `headEpisodesAreRateLimitedAcrossEpisodes` (onset at 0, onset at 2 → nil, onset at 4.5 → spoken), `secondHeadLineNeedsUnderSixtyCentimetres`, `sideCuesAreSpokenOnlyWhenThePhoneCannotBuzz` (incl. "One meter ahead.", per-kind limiter), `aBuzzedSideCueDoesNotSplitAHeadEpisode` (a buzzed side cue between the onset and a band re-fire does not re-speak "Head height.").
...
**LaneGeometryTests.swift** (16, suite "Lane geometry and metric bands", Step 51; fixture `syntheticDepth(pitchDeg:rollDeg:camH:surfaces:)` renders floor / wall / hanging sign / person / box through a 256 × 192 pinhole, z-depth, sky = 6 m at confidence 0) — `metricBandsDropTheFloorAtFortyFiveDegrees`, `metricBandsDropTheFloorAtFiveDegrees`, `aWallIsTorsoAndHeadAtMountTilt`, `aWallAtFortyFiveDegreesIsTorsoOnly`, `aHangingSignIsHeadOnlyAtMountTilt`, `aHangingSignIsInvisibleAtFortyFiveDegrees`, `aPersonIsTorsoAndHead`, `kneeHighBoxAtFortyFiveDegreesIsTorsoNotHead`, `headCoverageFallsOffPastTheGeometryLimit`, `headCoverLimitFollowsTheGeometry`, `blindSamplesCountTowardTheirNominalBand`, `rollDoesNotBreakTheBands`, `pitchOnlyGeometryMatchesTheTransformRow`, `heightIsLinearInDepth`, `mountTiltStatusSaysTooSteepForHeadCover`, `tileNoCoverIsNotClear`, `headCoverNoticeSpeaksOncePerRouteAfterTheHold`, `headCoverNoticeIgnoresRowsModeAndTransients`.

**LaneMathTests.swift** (16; helper `portraitBuffer(bufW: 256, bufH: 192, f)` builds a landscape buffer from a scene function; scene = 192 wide × 256 tall; lanes 64 px; usable height 192 → bands of 96)
- `uniformWallReadsSameEverywhere`, `leftWallOnlyHitsLeftLanes`, `mirrorSwapsLeftAndRight`, `headRowIsTopBandWithoutGeometry`, `groundBandIsSkippedWithoutGeometry` (rows mode, renamed Step 51; bottom 25 % at 0.3 m ignored, centre window too), `lowConfidencePixelsAreIgnored` (confidence 0 → `.infinity`), `tenthPercentileNeedsMoreThanTenPercentOfCell` (20 % coverage → 1 m; 5 % → 4 m), `zeroAndNaNDepthsAreInvalid`, `landscapeModeUsesBufferAsScene` (`rotateForPortrait = false`), `rawEntrypointHonoursPaddedRowStrides` (depth bpr 1088, conf bpr 320, confidence 1 accepted, padding 0xFF/0 never read), `tileLevels`, `mountTiltStatus` (renamed from `mountTiltWindow`, Step 51: 12° is good now, 25° too steep; 5° → ok "Camera tilt 5° down, good"; 12° → "…: tilt the phone up"; −2° → "Camera tilt 2° up: tilt the phone down"; 0.3° → "Camera level: tilt the phone down").
...
**VLMCodecTests.swift** (11) — `geminiRequestCarriesImageAndPrompt` (thinkingBudget 0), `openAIRequestUsesDataURI`, `anthropicRequestShape` (`max_tokens == 1024`, image block first), `geminiResponseParses` (join + clean, SAFETY block → `emptyResponse("SAFETY")`, garbage throws), `openAIResponseParsesStringAndPartsAndRefusal`, `anthropicResponseParsesAndDetectsRefusal`, `httpErrorsCarryProviderMessage`, `spokenDistances`.

**SceneEngineSummaryTests.swift** (17, Step 47; fixture `idle(cloud:)` = a Muse key, nothing asked, hazard watch off, Detailed · Outdoors · names off) — `chainPillNamesTheOrder`, `neverAskedYet`, `cloudAnswerNamesTheModelAndLatency` ("Muse answered in 1.9 s"), `onDeviceOnlyAnswerHasNoCloudStory`, `fallbackSaysWhy` ("On-device answered — Muse: The request timed out." + "(after 18 s)"), `gateRefusalIsExplained`, `gateLineOnlyWhenTheCloudAnswered`, `aFailureIsSaidAsAFailure`, `ageIsRelative` ("just now" / "45 s ago" / "2 min ago" / "1 h ago"), `latencyReadsInSecondsAboveOne`, `whenLineJoinsAgeAndTrigger`, `triggerWordsAreSpoken`, `hazardWatchOffSaysWhenItWouldRun` (8 s / 2.5 s follow the facts: 12 s / 3 s rewords), `hazardWatchOnNamesTheLastAnswer`, `cuesLineNamesLevelPlaceAndNames`, `summaryReadsEveryRowInOrder`.
...
**Other test files** (one line each: what it pins; `@Test` count at Step 47):
- `ActionRateLimitTests.swift` (5) — `ActionRateLimit`: repeats spaced by the interval (default 10 s), refused attempts do not extend the wait, actions are independent, `secondsRemaining` counts down.
- `AlertContextTests.swift` (11) — `AlertContext`: `extra` fields carry the context (unknown / invalid / blank omitted, long text truncated), the summarizer prompt carries the facts, bans false reassurance and invention and states its length budget; request bodies and output / reasoning budgets.
- `CloudSceneGateTests.swift` (15) — `CloudSceneGate.check` / `sanitized`: counts stripped, distances refused, unread street names refused, reassurance refused; fixtures are measured hallucinations.
- `DepthSnapshotTests.swift` (14) — `DepthSnapshot.make` (portrait remap, confidence), `distance(inVisionBox:)` (bottom-left origin flip, `trusted` 0.3…5 m, nil when unsure), `NormalizedBox`.
- `FallDetectorTests.swift` (10) — `FallDetector`: a real fall is reported once per episode; impact without free fall, a caught drop, a quick pick-up, one noisy sample and non-finite samples are not falls; free fall without impact expires; picking the cane up re-arms; `reset` abandons the episode.
- `FamilyContactsTests.swift` (14) — `FamilyContacts`: plausible addresses accepted, malformed rejected (with the typos listed); `normalize` trims, lowercases, de-duplicates, drops invalid entries, caps the list, is idempotent; the registration event matches the contract, omits `send_test` unless asked, round-trips, and ordinary events carry no addresses.
- `HeadNodDetectorTests.swift` (7) — `HeadNodDetector`: two nods within `pairWindowSeconds`, one fire per pair, `refractorySeconds`, a slow tilt is silent.
- `HeadYawSourcesTests.swift` (18) — `FaceYawGeometry.worldYawDegrees`, `FaceYawTracker` (smoothing, `maxAge`, `maxJump`, recenter) and `HeadYawSelector` (AirPods beat the camera; nil → 0).
- `LaunchRecoveryTests.swift` (7) — `LaunchRecovery.mode`, `optionalFeatureKeys`, `spokenLine`, `healthySeconds`; `recoveryNeverClearsTheRefusedVoiceLatch` (Step 63).
- `NaturalVoiceLatchTests.swift` (3, Step 63) — persisted key, prefetch must not wipe the 401, restored card line matches `StatusSummary.voiceLine`.
- `SceneDescribePolicyTests.swift` (1, Step 63) — a lock generation drops the pre-lock scene.
- `MultiCamDepthTests.swift` (8) — `MultiCamDepth.verdict` / `sentence` from `MultiCamDepthFindings`, and `MultiCamCost`.
- `IndoorRouteTests.swift` (Step 62) — `IndoorScript` decode defaults / validation, `IndoorProgress` (85% advance, 70% landmark, no drift, exit, next, never backwards, jumps ≤ 2 lines), `IndoorHandover` (3 good fixes, poor fixes ignored, outside resets, forced 30 m / radius / 20 s, waiting ≤ 20 m, GPS-only 3 × ≤ 10 m within 15 m), `IndoorExitAverager` fallback, `IndoorRecorder.isLandmarkText`, `IndoorRecorder` (straight with jitter, L, U-turn fast and slow, 1.5 s hold edge, turn before walking, landmarks, stride). `shippedIndoorDraftIsValid` pins the bundled `indoor_isr.json` (decodes, validates, `walked: false`, exit = its `outdoorPlaceID` entrance).
- `NodToTalkFastPathTests.swift` (2) — the "nod to talk" switch through `FastPathIntentClassifier`: switches by voice; without a verb it is not a setting.
- `CameraControlGateTests.swift` (8, Step 67) — numbers; the log's three presses inside the grace; the 5 s edge; blocked while listening and while the launch line is pending; a burst collapses to one; a refused press still starts the debounce; a backwards clock refuses.
- `StressTests.swift` (8, Step 66 stress campaign) — seeded property tests with an in-file `SplitMix64`; every suite runs each seed twice and asserts identical output, and prints `STRESS-DIGEST <suite> <hash>` (diffed across processes, `ios/scripts/stress/README.md`) plus `STRESS-STAT` measurements. `geofenceSurvivesHostileGps` (1,000 walks on `route_isr_cif.json`: AR(1) noise σ 3–25 m with honest accuracy, dropouts, 20 s gaps, backwards jumps, outliers, a door dwell → indices strictly ascending, arrival only on the last waypoint, nothing after it, arrival whenever σ ≤ 8 m; ⚠ arrival > 25 m early is a `withKnownIssue` — 12 / 1,000 at ρ 0.7, worst 38 m; 16 at ρ 0.95, worst 58 m); `cueDeciderSurvivesRandomLaneStreams` (400 streams: onset never held beyond the 400 ms gate, no head fire without a covered candidate or the latch, "Head height." ≥ 1.5 s apart, onset lines ≥ 4 s after the previous, ≤ 2 in 4 s); `indoorScriptSurvivesPedometerChaos` (2,000 walks: never backwards, ≤ 2 lines per call, one exit, one handover, each handover justified by an oracle of the IndoorRoute.swift rules, liveness at the door); `islandNeverClaimsClearWithoutLiveSensing` (20,000 inputs); `islandAlertThrottleHoldsUnderChurn` (200 × 300 changes); `fastPathFuzz` (5,000 decorated utterances: stop phrases always `.stopRoute`, yes forms always `.confirm(true)`, pure, whitespace / trailing-period / case invariant); regressions `headOnsetFiresAfterAGatedHandoffAndALongSweep`, `noHeadOnsetOnANonFiniteDistance`.
- `PeopleAheadTests.swift` (28) — `PeopleAhead.bearing` / `countPhrase` / `distancePhrase` / `line` / `nouns` / `needsSpeaking` / `summary`, `SightingKind` confidence floors.
- `SpeechLoadPolicyTests.swift` (7) — `SpeechLoadPolicy.admit`: the 7 s calm window for `.ambientObstacleName`, busy suppression, `reset()`, `.normal` always speaks.
- `SpokenPhrasesTests.swift` (12) — every production warning line (`obstacleLine`, `approachLine`, ground-hazard and sign lines) is a member of `SpokenPhrases.warningLines`; `warningCharacterBudget`; a cancelled warm-up resumes.
- `ThreatWatchTests.swift` (10) — `ThreatWatch`: a weapon in a vision reply is reported (event quotes the camera, critical); negations (covering a whole list, reset by a contrast), benign collocations, substrings, ordinary scenes and empty text do not alert; punctuation and case do not hide it.
- `TripLogRecordTests.swift` (2) — `TripLogRecord.make`: fields never overwrite `t` / `kind` (kept as `field_*`), ordinary fields pass through.
- `UtteranceEndTests.swift` (7) — `UtteranceEndDetector`: no submit before words, 1.5 s silence after speech submits, 10 s cap, 0.25 s tick.
- `VoicePrefetchTests.swift` (8) — `VoicePrefetch.maxConcurrent` (2), `isFatal` (401 / 403 / 422 only), `queue` keeps order and drops cached lines; Step 54 `merge` order and completeness, `chunk`.
- `VoiceEngineChoiceTests.swift` (16, Steps 53–54, 61, 63) — the engine decision table, pinned raw values, the 2.5 s deadline, `resolvedEngine` (including `naturalUnavailable` → system), `describe`, and the session-sticky `VoiceBreaker`.
- `SelfHearFilterTests.swift` (8, Step 55) — numbers, exact line, case / punctuation, clauses, the walker's own words kept, age-out, short fragments, explicit recording.
...
`uploadObject` / `deleteObject` for the `hazard-photos` bucket. `fromSecrets()` accepts only a
real HTTPS URL with a host; plaintext HTTP is rejected before any medical, contact or location
data can leave the phone. 15 s request timeout — a cane that cannot reach the
...
ids; existing remote rows are not implicitly deleted. Each async writer captures a consent
generation and checks it before and after suspension, so an opt-out fences delayed registration,
trip, hazard, alert, medical and family-contact continuations; a photo that finishes after opt-out
is deleted when it has no row to own it.
...
- `audit(records) -> dict`: `tilt` (median / range / share inside `MountTilt.aim` 3–8°, `mounted` = ≥ 50 % inside — a handheld log must not tune distances); `head_band` and `head_band_mounted_frames` (the same over frames inside 3–8° only) — every `lanes` cell with head < 1.5 m = `CueThresholds.head`: `torso_also_near` when the same lane's torso is within 0.5 m → wall / furniture / person, `overhang_signature` when torso is −1 (with other torso data in the frame) or ≥ head + 0.5 m, `torso_dropout` when the whole torso row is −1 (not judged), `overhang_share` over judged cells, `by_lane`; `tilt` is over every `lanes` record carrying one (None → "mount state unknown"); `cues_per_min`; `speech_per_min` by priority; `unsolicited_per_min` and `unsolicited_non_route_per_min`; `top_lines`; `suppressed` by reason and by load; `dispatch` (`speech_dispatch` sorted by `t`, records without `t` skipped: replays and 0 < gap < 1 s); `voice` (Step 53: `engine_flips` from `engine_flips_per_minute(records, minutes)` — flips between adjacent settled lines, per minute, lines per engine, unsettled races — and `engine_flips_inside_route_speech(records)` — flips between consecutive `nav` lines inside one `route` start…stop window, want 0; both over `resolved_engines(records)`, which settles a `race` dispatch with its `speech_engine` record and a `playback_failed` as system); `field_collisions` (`field_kind` / `field_t`, an app bug). Step 51 / 52 (agent A): `head_cover` (`limit_deg` from `head_cover_limit_deg` — the z-depth mirror of `MountTilt.headCoverLimitDeg`, overridden by a `depth_geometry` record; `share_head_cover` from `lanes.head_cover` or tilt ≤ limit; `bands`; `cells` / `could_not_be_head` — rows-mode head cells whose top-ray height is under 140 cm); `head_gate_replay` (`head_gate` = Python `HeadGate`, and a 2 Hz mirror of the head episode + `CueSpeechPolicy` → `onsets`, `band_refires`, `lines_would_speak` for `signature_only`, `signature_and_estimated_cover`, `no_signature`, beside `logged_head_cues` / `logged_head_onsets` / `logged_head_lines`). Fixtures: `selftest_head()`. Step 68 `--speech-load`: `route_window(records)` (first route-start sign → stop / arrival / last record), `dispatched`, `load_of`, `speech_load(records)` (lines, characters, per minute, by text), `step68_intro`, `gps_announcer_replay` (Python `GPSAnnouncer` at 10 Hz over `gps` records), `close_replay` (2 Hz `lanes` estimate), `step68_replay(records)` (before vs after, `chars_per_min_change_pct`), `human_speech_load`; fixture `selftest_speech_load()`.
...
- `AppModel` (MainActor) owns `CueDecider`; feeds each `LaneReport` with `now`; routes `CueOutput` to `HapticPlayer` (which uses `GeigerRate.hertz`), mirrors `CueKind` to the watch via `PhoneToWatch.obstacle`, and asks `CueSpeechPolicy` which cues to speak (from the `.head(distance:onset:)` payload; no `cleared()` since Step 52).
...
#### Engine wiring (all `let`, created in the property initialisers, except `describer`, `sceneContext`, `hazards` and `sounds`, which `init` builds, and `conversation` / `voiceInput`, which are `private(set) var …!` set at the end of `init`)
...
| `speech` | `SpeechQueue` | 4 | Single voice. `say(_:_:ttl:load:)` (default ttl 8; since Step 54 no `immediate` — every line takes cache → 2.5 s race → system voice only on failure, `VoiceEngineChoice`), `sayAgain(_:_:ttl:)` (default ttl 12; bypasses coalescing so a line still playing is re-spoken), `setVoiceHold(_:)` (holds sub-`.safety` lines while dictating; `.safety` breaks through), `prefetch`, `stopAll`, `isSpeaking`, `configureAudioSession`. |
...
| `faceHead` | `FaceHeadPose` (`ios/CaneKit/Audio/`) | 14 | Head yaw from the **front** camera's `ARFaceAnchor`, beside LiDAR — the fallback that makes the AirPods optional. Fed by `DepthEngine.onFaceYaw`, aged by `handle` (`refresh`), chosen against AirPods by `HeadYawSelector` in `startTicker`. Off unless `faceHeadTrackingEnabled`. |
| `sounds` | `SoundWatcher` | 14 | Danger-sound recognition on the microphone (sirens, horns, vehicles); off by default. Built in `init` as `SoundWatcher(speech: speech)` (it needs the audio-session owner). `start/stop` from `dangerSoundsEnabled`. |
| `bothCameras` | `DualCameraSession` | 14 | Front **and** back previews at once (`AVCaptureMultiCamSession`) for a spotter / the demo video; off by default, ARKit paused while it runs. |
| `medicalProfile` | `MedicalProfileStore` (`ios/CaneKit/Trip/`) | 43 | Medical ID card, emergency profile and mobility fitness tracking (Profile tab). |
| `fallWatcher` | `FallWatcher` (`ios/CaneKit/Alerts/`) | 43 | Cane-went-over detector (CoreMotion → CaneKitLogic `FallDetector`); `portraitMount` follows `portraitMode`; armed by `applyFallWatcher()` when both `fallDetectionEnabled` and `familyAlertsEnabled`. ⚠ Unvalidated thresholds. |
| `family` | `FamilyAlerts` (`ios/CaneKit/Alerts/`) | 39 | Cane detections → the Grok Bot routine. `enabled` ← `familyAlertsEnabled`, `aiContextEnabled` ← `familyAlertsAIContext`; a missing webhook key is a no-op that says so. |
...
| `torsoPolicy` | `TorsoHapticPolicy` (`@ObservationIgnored private var`, CaneKitLogic) | 41 | Which decider torso decisions the level × place × crossing settle renders; `update(_:report:rules:crossingSettle:now:) -> TorsoHapticAction?`, `reset()` (with every decider reset and in `applyCueRules`). |
...
Logic state held as `@ObservationIgnored private var` value types (CaneKitLogic): `cueSpeech: CueSpeechPolicy` (which obstacle cues are spoken; replaced with a fresh value in `beginRoute`), `straightWalk: StraightWalkDetector` (auto-recenter trigger) and `groundPolicy: GroundHazardPolicy` (when a confirmed ground hazard is spoken again; `reset()` in `beginRoute`). Other private flags: `wasHot` (thermal notice on transitions only), `lastNavKind`, `recenterPending`, `ticker`, observer tokens.
...
- `activeCue: CueKind` — the kind *rendered* (`.clear` when nothing in range, or when the level / place / crossing settle suppressed the decider's torso cue; `.center` after a Standard onset tap until the decider's `.stop`).
- `lastGroundHazard: String?` — the last ground-hazard line spoken ("Two meters ahead, drop-off."), shown as the `detection("LiDAR", …)` row of `HazardsCard` (the source literals are `"LiDAR"`, `"Sign"`, `"Watch"`, uppercased for display); cleared by `startRouteNow`.
...
| `namePeopleEnabled` | `false` on a fresh install (explicit opt-in; persisted legacy opt-ins remain visible as experimental/unverified) | `sceneContext.setPeopleEnabled(_:)`. HazardsCard "Name people ahead" |
| `familyAlertsEnabled` | `false` (opt-in twice: this switch and a webhook key in Secrets.plist) | `family.enabled`, `applyFallWatcher()`. Settings "Send cane events to family" |
| `fallDetectionEnabled` | `true` (sends only while family alerts are on; thresholds unvalidated) | `applyFallWatcher()`. Settings "Detect the cane falling" |
| `familyAlertsAIContext` | `true` | `family.aiContextEnabled`. Settings "Add AI context" |
| `familyContactEmails` | `[]` (`Settings.strings`, normalised by `FamilyContacts.normalize`) | `familyEmails` (`private(set)`); registered only by `saveFamilyContacts()` |
...
**Low light (Step 49).** `setTorch(_ on:, byApp: Bool = false)` — `byApp` marks an app-lit torch (`torchLitByApp = on && byApp`, `torchConfirmationMuted = on && byApp`; the request record gains `by_app`); the walker's switch passes the default and thereby takes an app-lit torch over. `applyTorch` mutes the `.confirmed(on: true)` line while `torchConfirmationMuted` (the low-light line already said "Flashlight on."; logged `muted: true`), forgets `torchLitByApp` on `.failed(requested: true)`, and on `.changedByDevice(on: false)` of an app-lit torch calls `lowLight.torchCutByDevice(now: lastReportTime)` (60 s backoff). Members: `lowLight: LowLightPolicy` (`@ObservationIgnored`, 30 Hz), `lightState: LowLightPolicy.State` (published, written only on change — the Guide card's DARK pill and the Scene engine Light row), `ambientLux: Float?` (`@ObservationIgnored`; read inside the card's 10 s `TimelineView`), `torchLitByApp` (published), `torchConfirmationMuted`, `lastReportTime` (AR clock for off-clock events), computed `torchAvailable` and `camerasInTheDark` (`lightState == .dark && !torchEnabled` — what `describer.isDark` / `hazards.isDark` return). `updateLowLight(_ report:)` (from `handle`, after `sceneContext.set`): `lowLight.update(lux: report.ambientLux, now: report.timestamp, torchByApp: torchLitByApp)`; on a state change publishes `lightState`, `releaseAppTorch()` on `didExitDark`, `lowLightAct(entered: true)` on `didEnterDark`, `logLight()`. `lowLightAct(entered:routeStart:)` — `LowLightAdvice.decide(... autoTorch: autoTorchInDark, autoTorchAllowed: lowLight.canAutoLight(now: lastReportTime, batteryPct: batteryPercent))` → `setTorch(true, byApp: true)` and the line at `.nav`, 10 s (+ `speech` record); with `routeStart` it acts only when it would light the torch (`startRouteNow` calls it when `lightState == .dark`, after `announceChannels()`). `releaseAppTorch()` — `setTorch(false, byApp: true)` only if `torchLitByApp`; called on `didExitDark`, in `stopRoute` (after `nav.stop()`), `endRouteQuietly`, and `nav.onArrived`. `logLight()` → `light {state, lux (−1 before the first estimate), torch, torch_by_app}`. `trackingLimitedLine` — see `failQueuedRouteStart`. `faceHeadTrackingEnabled`'s `didSet` now asks `FaceTrackingChange.decide` first; a refusal (route guiding / starting) writes `oldValue` back under `applyingFaceTracking`, speaks why at `.nav` (ttl 10) and logs `face_tracking {action: refused_route|refused_route_start, requested}` — it does **not** set `routeError` (the Guide card's route-build line, which nothing would clear). `startFaceTrackingSelfTest` refuses the same way (`selfTestStatus` "Not while a route is guiding you" / "… starting"), and its 15 s restore — itself a session re-run — waits, polling once a second (`face_head_selftest {action: restore_deferred_route}`), while a route started during the test is guiding or starting. `start()` wires `speech.onDispatch` → trip-log `speech_dispatch {text, priority, replays, resume_from}` for every line handed to a voice backend, and `speech.onLineEnd` → `speech_end {priority}` for every natural line end (Step 37).
...
|---|---|---|
| `CueSpeechPolicy.headInterval` | 4 s | Min gap between onset "Head height." lines across episodes (AR clock). |
| `CueSpeechPolicy.headSecondLineBelowM` | 0.6 m | A band re-fire closer than this speaks "Head height." once more in the episode (Step 52). |
...
| hazard frame JPEG | 768 px (hazard map) | `frame(_:maxDimension:)` (the 480 px live-view JPEG is gone — `LiveCameraView` draws the AR session directly) |
...
- **`start()`** — once (`guard !started`). Order matters:
  1. `observeThermalAndBattery()`; 2. `logger.start()`; 3. `speech.configureAudioSession()` **before ARKit and before the haptic engine**; 4. `wireAudioRoute()`; 5. `haptics.start()`; 6. `watch.onCommand = handleWatchCommand`; `watch.activate()`; 7. `wireNavigation()`; 8. `location.requestAuthorization()` **unless env `CANEKIT_UITEST == "1"`** (the three-choice alert races the first XCUITest tap) — Location prompts at launch, Motion/HealthKit at route start to avoid a three-alert pile-up; 9. `depth.onReport = handle`, `depth.onReadinessChanged = depthReadinessChanged`; `depth.start()`; 10. `wireHazards()` (after the depth engine: the scanner reads its camera frames; starts the scanner); 11. log `start` event (`lidar`, `mesh`, `haptics`, `vision` = `describer.providerName ?? "none"`); 12. `announceCameraDenied()`; 13. `speech.applyPersistedNaturalVoiceLatch()` then `speech.prefetch(commonLines)`; `speech.say("OpenCane ready." | "OpenCane. This phone has no LiDAR.", .nav)`; 14. if `CommandLine.arguments` contains `--demo-route` **or** env `CANEKIT_DEMO_ROUTE == "1"` → `startDemoRoute()` (simulator GPS replay / UI-test / `e2e.py` hook).
...
- **`scenePhaseChanged(_ phase: ScenePhase)`** — no-op until `started`.
  - `.active`: `haptics.resume()`, `depth.resume()` (no tracking reset), `beacon.resumeIfNeeded()` (the audio engine can die across a screen lock without an interruption notification), `hazards.start()` (idempotent).
  - `.inactive`: nothing.
  - `.background`: `hazards.stop()` (no scanning a frozen last frame), `voiceInput.cancel()`, `voiceShellGeneration` bump, `conversation.cancelForBackground()`, **`describer.cancelForBackground()`** (Step 63: a pre-lock "Where am I" JPEG must not speak after the unlock), **`sceneContext.set("")`** (LiDAR facts are stale once we come back), `depth.pause()` (stops gyro and drops the retained camera frame), `haptics.stopAll()`, `decider.reset()`, `namer.reset()`, `activeCue = .clear`, `logger.flush()`.
...
1. `decider.update(report, now: report.timestamp)` (AR clock, seconds) on every report, then `torsoPolicy.update(output, report:, rules: cueRules, crossingSettle: nav.isCrossingSettle, now:)` (Step 41) →
   - `.render(cue)`: `activeCue = cue.kind`; `haptics.play(cue)`; `mirrorAndSpeak(cue, now:)` (`phoneCannotBuzz = !haptics.isHealthy || haptics.silenced`; if `phoneCannotBuzz || fallbackToWatch` → `watch.send(obstacle: cue.kind, now:)`; `speakCueIfNeeded(cue, phoneCannotBuzz:, now:)`); log `cue` (`cue`, `ar_t`, plus `distance` for `.centerApproach`).
   - `.centerOnset(strong, d)` (Standard): `activeCue = .center`; `haptics.playCenterOnset(strong:)`; `mirrorAndSpeak(.centerApproach(distance: d))`; log `cue` (`cue: center`, `ar_t`, `distance`, `render: center_onset | center_strong`).
   - `.updateCenter(d)` (Detailed): `activeCue = .center`; `haptics.setApproach(distance: d)` (no watch/speech — continuous ramp).
   - `.stop`: `activeCue = .clear`; `haptics.stopAll()`; `cueSpeech.cleared()` (the next head cue is a new episode); log `cue: clear`.
   - `.suppressed(cue, reason)`: `activeCue = .clear`; `haptics.stopAll()` (ends a loop the hold interrupted); **no** watch mirror, **no** speech; log `cue` (`cue`, `ar_t`, `suppressed: <reason>`). The head cue never arrives here.
...
- **`wireHazards()`** (private, once from `start()` after `depth.start()`) — `hazards.isNavigating = { nav.isNavigating }`, `hazards.isDark = { camerasInTheDark }` (Step 49; `wireDescriber` sets `describer.isDark` the same way), `hazards.currentSpeed = { location.fix?.speed ?? 0 }`, `hazards.onHazard = { text, source, jpeg in speech.say(text, .obstacle, ttl: 6); recordHazard(kind: source.rawValue, text:, source:, jpeg:) }` (signs and vision cautions: below route lines, above scene), then `hazards.start()`.
...
- Live camera view — no model method any more: `liveFrameJPEG()` (and `HazardsCard.refreshLoop()`) were removed. `HazardsCard.liveView` asks `LiveView.state(enabled:hot:cameraRunning:highFrameRate:foreground:)` (CaneKitLogic) and, when `.live(fps)`, draws `LiveCameraView(session: depth.arSession, framesPerSecond: fps)` on the GPU; `hazards.paused` (thermal) still hides it, because the view is optional and the lanes are not.
...
- **`wireAudioRoute()`** (private, called once in `start`) — installs `audioRoute.onChange { connected, name }` (debounced 2 s in the monitor): `beacon.headphonesConnected = connected`; log `audioroute` (`connected`, `name`); connected → `speech.say("\(name) connected.", .nav, ttl: 5)` and, if `nav.isNavigating`, `head.start()` + `recenterPending = true`; disconnected → `headphoneNotice.disconnected(navigating:)` (Step 68: "AirPods disconnected." once per route while navigating, else "Headphones disconnected. Beacon paused.") at `.nav`, ttl 5, and **`head.stop()`** (no AirPods, no motion). Also installs `audioRoute.onImmediateChange { connected in beacon.headphonesConnected = connected }` — undebounced, so the click stops the moment the AirPods drop instead of playing from the cane speaker for 2 s. Then `audioRoute.start()` and seeds `beacon.headphonesConnected = audioRoute.headphonesConnected` (the initial read fires neither callback). The beacon only renders into headphones; a click out of the cane speaker carries no direction.
- **`announceChannels()`** (private, near-last step of `startRouteNow`) — Step 68: at most one line, `RouteStatusLines.routeStartLine(...)` at `.nav`, ttl 20: only `!haptics.isHealthy` → "Haptics unavailable. Obstacle cues on the watch." (watch reachable) / "… will be spoken.". No headphones and an unreachable watch are no longer spoken (Guide card, "status").
...
- Step 65: `queueRouteStart` no longer speaks the warm-up sentence; `routeWarmupTickTask` plays `Earcon.thinking` at 0 / 2 / 4 s while the start is pending; `depthReadinessChanged(.ready)` cancels it, plays `done` and says "Starting." (`.nav`, 8 s) before `startRouteNow`. ~~`speakMenuThenListen` speaks `VoiceMenu.launchMenuLine(firstLaunch:)`~~ (removed in Step 67); launch / follow-up listens pass `kind` `.launch` / `.followUp` / `.question`.
- Step 67: `start()` says `VoiceShellPolicy.launchLine(lidarSupported:)` and nothing else, then `listenAfterLaunchLine()` (was `speakMenuThenListen`): sets `launchLinePending`, waits `waitForSpeechToDrain` (cap `VoiceShellPolicy.speechDrainCap`), applies `VoiceShellPolicy.launchListen`, clears `launchLinePending` on every exit (`defer`); logs `voice_launch {action: skipped, reason}` (was `voice_menu`) and `voice_listen {mode: launch}`. `commonLines` references `VoiceShellPolicy.readyLine`. `cameraControlGate` (`CameraControlGate`, re-created at the top of `start()`) and `launchLinePending` are `@ObservationIgnored`; `cameraControlPressed()` asks the gate with `voiceInput.isListening || isStarting` and `launchLinePending`.
...
- **`beginRoute(_ route: Route)`** (private) — ends a currently running route as before, refuses denied Location, turns off two-camera mode (serialized), then on a LiDAR phone with camera access queues the route in `pendingRouteStart` and says/sets `routeStartStatus` while `DepthReadiness` waits. `DepthEngine` reports same-frame `.normal` tracking + valid scene depth + sweep trust; after 3 consecutive reports the queued route enters `startRouteNow`. A 7 s request deadline (including a two-camera transition that never drains) clears the request, logs `route_readiness {state: timed_out_fallback_gps}`, speaks `"Obstacle detection warming up. Guiding with GPS."` (.safety, 15 s TTL), sends watch status `"Guiding with GPS"` and immediately starts GPS guidance via `startRouteNow` (AGENTS.md Rule 6: never trade guidance away for a stricter check). Stop or a newer destination cancels the queue. `startRouteNow` then performs the existing prefetch → location → `nav.start` → beacon/head → recenter → ticker/trip/Live Activity → log/watch/channel sequence.
- **`startSimulatedWalk(speedMps:)`** / **`stopSimulatedWalk()`** — on-device walk simulation for in-room testing. Stepping iterates through route waypoints at 1 Hz with linear coordinate interpolation and bearing calculation, resumes from `nav.waypointIndex` if invoked mid-route, and injects synthetic `GeoFix` and heading updates into `location.ingest`. Protected by `simulatedWalkGeneration` against task cancellation races.
- **`stopRoute()`** — `stopSimulatedWalk()` → `nav.stop()` → `location.stop()` → `beacon.stop()` → `head.stop()` → `stopTicker()` → `Task { await trip.stop() }` → `liveActivity.end()` → `speech.stopAll()` (**queued waypoint lines must not play after Stop**) → `speech.say("Route stopped.", .nav)` → log `route {action: stop}` → `pushStatusToWatch()`.
...
- **`endRouteQuietly()`** (private; only from `beginRoute` when a route is already running) — increments the route generation, stops simulated walking/navigation/location consumers, stops beacon/head tracking and the ticker, **`trip.cancel()`** synchronously (an async `stop()` would still be tracking when the new `trip.start()` runs), ends the Live Activity through its serialized operation, cancels voice input, then clears speech before logging `route {action: restart}`. Speaks nothing and leaves location running for the replacement route.
...
- `static let commonLines: [String]` (Step 65: "I did not catch that." and the warm-up sentence removed — the first lives in `SpokenPhrases.shellLines`, the second is gone) (Step 49 appends `"Camera tracking is limited, probably low light. Obstacle detection is running on LiDAR."` after the GPS-fallback line and `LowLightAdvice.allSpokenLines` after `TorchSwitch.allSpokenLines`) — `"OpenCane ready."`, `"Route started."`, `"Route stopped."`, `"Next."`, `"Recentered."`, `"Veer left."`, `"Veer right."`, `"No route running."`, `"No GPS fix yet. Try again outside."`, `"Head height."`, `"Left."`, `"Right."`, `"Passed one waypoint."`, `"Obstacle detection warming up. Route will start when it is ready."`, `"Route start canceled."`, `"Obstacle detection warming up. Guiding with GPS."`, `"Stop route is armed. Tap Stop route again within 3 seconds to end guidance."`, `"Siren. Do not start crossing."`, `"Horn nearby."`, `"Vehicle sound nearby."`. Must stay byte-identical to the strings spoken elsewhere (`NavigationEngine`, `CueSpeechPolicy`, `DangerSound`, `GuideCard`, this file) or the prefetch cache misses. (`"Route started."` and `"Next."` are not currently spoken standalone — the intro is `"Route to <destination>. <first say>"`, prefetched separately.) Step 68: `+ RouteStatusLines.allSpokenLines` (GPS weak / back, "Screen locked. Obstacle warnings off.", haptics, headphone and "Close." lines); the head-cover literal is gone.
...
Other triggers into the same paths: `describeScene(trigger: DescribeTrigger = .button)` (logs `describe {provider, trigger}` then `describer.describe(trigger:)`; `.button` from `GuideCard`, `.watch` from `handleWatchCommand`, `.actionButton` from `WhereAmIIntent`, `.cameraControl` from `cameraControlPressed`, `.waypoint` from the `describeEveryWaypoint` hook — Step 47), `cameraControlPressed()` (Step 67: `CameraControlGate.press` first — a refused press writes `describe_skipped {reason, trigger: cameraControl}` and makes no sound; an accepted one is `describeScene(trigger: .cameraControl)` — one `describe {trigger: cameraControl}` record per press since the Step 47 review; the old separate `source` record is gone), debug `watchTest(_ cue: NavCue)` (`watch.send(nav:)`), `speechTest()` (a `.scene` line then an `.obstacle` line to prove interrupt ordering).
...
**Purpose.** Exactly seven `AppIntent`s — `WhereAmIIntent`, `StartDemoRouteIntent`, `NavigateToCIFIntent`, `TakeMeToIntent`, `RepeatInstructionIntent`, `NextWaypointIntent`, `StopRouteIntent` (plus the `CampusDestination` `AppEnum`; there is no "Take me somewhere" intent) — that open the app (ARKit needs the foreground) and call into `AppModel.shared`, plus the `AppShortcutsProvider`. Spoken name is OpenCane via `\(.applicationName)` (display name — no edit needed on rename); code names stay CaneKit.

| Type | Kind | Role |
|---|---|---|
| `WhereAmIIntent` | `struct: AppIntent` | title "Where am I"; `perform()` → `model.describeScene(trigger: .actionButton)` (the intent cannot tell the Action button from Siri; the card says "from the Action button or Siri"). |
| `TakeMeToIntent` | `struct: AppIntent` | title "Take me to a place"; a `CampusDestination` (`AppEnum`, gazetteer) or the free-text `place: String?` parameter (Siri asks "Where do you want to go?" when neither is given) → `model.navigate(to:)`. |
...
### `App/AppModel+Indoor.swift` — the indoor leg's voice actions and hooks (Step 62)

**Purpose.** `extension AppModel` holding everything the indoor leg needs from the model, so `AppModel.swift` carries one hook line per entry point (the Live Activity step edits AppModel in parallel). The stored property `let indoor = IndoorGuide()` and `indoor.attach(self)` (after `wireNavigation()` in `start()`) are in AppModel.swift.

- `routeFromTo(from:to:) -> String` — `.routeFromTo`: `IndoorScriptCatalog.script(forOrigin:in: indoor.scripts)` → `startIndoor(script, destination: to, source: "voice")`, returns "Indoors: <name>, then <to>."; no match → `indoor {action: no_script, origin, destination}` + `navigate(to:)`, returns "Routing to <to>.". Both already spoken.
- `startIndoorRouteForTesting()` — Settings "Start indoor route" and `CANEKIT_INDOOR_ROUTE=1`: the `defaultRecordingID` script (else the first) with no destination (→ CIF demo route); empty catalog → "No indoor route saved."
- `private startIndoor(_:destination:source:)` — stops an outdoor route running / queued / building first (`stopRoute()`, "Route stopped."), then `indoor.start`.
- `indoorOutside() -> (response, alreadySpoken)` — `.indoorOutside`: `.waiting` speaks "Waiting for GPS outside." (`.nav`, ttl 20); `.handedOver` speaks nothing (the route intro does); `.notActive` → ("No indoor route running.", false) for the coordinator to speak.
- `indoorHandlesNext()` / `indoorHandlesRepeat()` / `indoorHandlesSimulate()` — first line of `nextWaypoint()` (GuideCard Next, watch Next / crown, Siri, "next"), `repeatInstruction()` (GuideCard Repeat, watch Repeat, Siri, "repeat"; `IndoorGuide.repeatLine` logs `indoor {action: repeat}` — the duplicate `repeat` record was removed in the review round) and `startSimulatedWalk(speedMps:)`; true when indoors and handled.
- Other hook lines in AppModel.swift: `location.onFix` → `indoor.ingest(fix)`; `voiceInput.onTranscriptionFinalized` → `if indoor.takeTranscript(transcript) { voiceInput.finishProcessing(); return }` (the landmark hook, before the conversation); `stopRoute()` → `indoor.stop(reason: "stop_route")` first; `stopSimulatedWalk()` → `indoor.stopSimulation()`. Review round: `scenePhaseChanged` → `indoor.sceneChanged(active:)` (log only) and, on `.active`, `liveActivity.flushPendingRequest()`; `start()` sets `liveActivity.onLog` (→ `logger.event("live_activity", …)`); `buildRoute`'s no-fix and error branches call `liveActivity.endIfIdle()`. `indoorOutside()` returns `alreadySpoken: true` on `.handedOver` because the outdoor leg speaks on every path (Muse M6 rejected). `speakStatus` (HandsFreeIntents.swift) and `ConversationCoordinator.currentStatusFacts` set `facts.indoorClause = indoor.statusClause`; the coordinator's "route" answers the route clause (indoor clause) while `indoor.isActive`.
- ⚠ Known gap: `scenePhaseChanged(.background)` stops GPS when no *outdoor* route runs, and CMPedometer live updates pause in the background — an indoor walk with the screen locked hands over only after unlocking.
...
- Step 49: every report carries `ambientLux` (`frame.lightEstimate.map { Float($0.ambientIntensity) }`, both yields), and a frame whose `trackingNormal` is false publishes `centerHit = nil` (`lastMeshHit` cleared before the throttled lookup) — no mesh *name* from a world-anchored mesh under a drifting pose; distances are LiDAR and unaffected. File header "Low light" paragraph.
- Step 48: `nearHold: NearHold` (queue-only) is applied to every grid right after `computeGrid` (`let grid = nearHold.apply(rawGrid)`) and reset by a frame without depth, so the tiles, `CueDecider`, the watch and the Live Activity all see a held 0.1 m for a blind-after-near cell. `TripLogger.lanes` writes `head_blind` / `torso_blind` (2 dp) and `held` (count of substituted cells).
...
Published (all `private(set)`): `report: LaneReport` (~30 Hz), `status: String` (header text via `AppModel.status`: "Depth idle" / "Waiting for depth…" / "Depth OK" / "Depth paused" / "Depth resuming…" / "AR error: …" / "AR interrupted" / "AR resumed" / "Mesh classification on" / "Mesh classification off (thermal)" / "No LiDAR / sceneDepth on this device"), `fps: Double` (rolling over a 2 s window of report timestamps), `framesProcessed: Int`, `isRunning: Bool`, `meshEnabled: Bool` (default `true`), `tracking: String` ("normal", "limited (motion)", "limited (features)", "initializing", "relocalizing", "not available", "limited", "—"), `readinessState: DepthReadinessState` (route-start gate state); computed `readinessTimeoutReason: DepthReadiness.TimeoutReason?` (Step 49; read by `AppModel.failQueuedRouteStart` before `cancelReadiness`). `ContentView` feeds `report` to `LaneGridView` and shows `status`; `fps`, `framesProcessed`, `tracking`, `meshEnabled` and `readinessState` currently have no standalone reader.
...
- `setFaceTracking(_ on: Bool)` / `setMeshClassification(_ on: Bool)` — update their configuration flag, drain the old delegate queue, invalidate readiness and, when running, re-run the session without reset options. Re-running costs ~1–2 s of depth; `AppModel` refuses face/high-rate writes through `SensorModeInterlock` while a route starts or guides, and thermal changes call `setMeshClassification` without a restart until the route ends.
...
- `fileprivate sessionFailed(code:message:)` — maps `ARError.Code` (`.cameraUnauthorized` → "Camera access denied — enable it in Settings"; `.sensorUnavailable`/`.sensorFailed` → "LiDAR sensor unavailable"; `.unsupportedConfiguration` → "Unsupported AR configuration"; else raw message) into "AR error: …", drains the processor, stops motion, drops the retained image/report, resets FPS/face state, sets `isRunning = false` and notifies `AppModel` so stale obstacle cues/scanner work are cleared while GPS guidance remains explicit.
...
### `ios/CaneKit/Depth/MultiCamDepthProbe.swift` — could both cameras keep LiDAR depth? (capability read, one trip-log record)

Purpose: answers, into the trip log, whether a future CaneKit could show the front and back camera at once **and still have a depth stream** — the shipped "Both cameras" mode pauses ARKit (ARKit never delivers two camera images; Apple DTS forums 677731), and the possible escape is AVFoundation (`builtInLiDARDepthCamera` inside an `AVCaptureMultiCamSession`, forums 702875; LiDAR depth up to 320×240 per WWDC22 110429). Those are claims about other devices, so this file measures the phone in hand.

- `nonisolated enum MultiCamDepthProbe` — `struct Result: Sendable` (`findings: MultiCamDepthFindings` from CaneKitLogic plus a human-readable summary of each supported device set); `static func measure() -> Result` reads `AVCaptureDevice.DiscoverySession.supportedMultiCamDeviceSets` and the devices' format lists — pure introspection, a few milliseconds, no camera permission, **no session started and nothing changed at run time**; private helpers `bestDepthFormat(of:)`, `largestDepthFormat(of:)`, `dimensionText`, `positionName`. Nonisolated because enumerating every camera has no business on the main thread at launch.
- Caller: `AppModel.start()` → `logMultiCamDepthProbe()`, in a detached `.utility` task once per launch; the record is written back on the main actor as one `multicam_depth` record (`verdict`, `keeps_depth`, `answer`, …). ⚠ Field names avoid `kind` and `t` (`TripLogRecord` renames a collision to `field_*` and `e2e.py` fails the run on one).
- Tests: `MultiCamDepthTests` pins the verdict, the sentence and `MultiCamCost`; the AVFoundation reads have no simulator equivalent (multi-cam unsupported there, so the simulator logs `multicam_supported: false`). Measured on the iPhone 17 Pro Max: 12 multi-cam sets pair the front camera with rear LiDAR depth at 320×240.

### `ios/CaneKit/Depth/SensorProbe.swift` — debug-only "what can run beside LiDAR" measurement (`CANEKIT_SENSOR_PROBE=1`)

Purpose: a one-shot measurement of what this phone actually lets run at the same time as LiDAR depth, written as trip-log records so hardware claims are proven by a command's output (AGENTS.md "How we engineer", rule 1) rather than asserted from memory. Cases, in order: `probe_capabilities` (static flags + the complete Objective-C property list of `ARFrame` — no second `CVPixelBuffer` besides `capturedImage`), `probe_a0_world_baseline`, `probe_a_world_plus_face` (`userFaceTrackingEnabled`: depth survives, face anchors arrive, `capturedImage` stays the rear 1920×1440), `probe_b_face_plus_world`, `probe_c_multicam` (an `AVCaptureMultiCamSession` beside ARKit), `probe_d_front_capture`, `probe_e_audio_session` (`.playAndRecord` vs the app's single `.playback` session), `probe_f_sound_labels` (which `SoundAlerts.candidateLabels` the built-in classifier has), `probe_warning` (app never active within 60 s).

- `@MainActor final class SensorProbe` — `static var isEnabled` (`CANEKIT_SENSOR_PROBE=1` / `--sensor-probe`), `var onEvent: ((String, [String: Any]) -> Void)?` (→ `TripLogger.event`, kinds `probe_*`), `func run() async`; private per-case methods (`caseA0Baseline` … `caseFSoundLabels`), `measure(_:options:…)`, `runDwell`, `waitForForeground`, `startRunning` / `stopRunning` (blocking AVFoundation calls pushed to a global queue through `CaptureHandoff` from DualCameraSession.swift), `emit(_:_:)`. Own `ARSession` (paused, delegate cleared at the end); restores the `.playback` audio session before returning. ≈ 24–36 s plus capture start/stop (five 4 s dwells, two 2 s settles, one retry dwell each for a0 / a / b when a case saw no frames).
- `enum ProbeIntrospection` (`frameProperties`, `framePixelBufferProperties`, `properties(of:)` via the ObjC runtime); private nonisolated relays `ProbeSessionWatcher: ARSessionDelegate` (`Counts` behind a `Mutex`), `ProbeCaptureNotes` (session interruptions / runtime errors), `ProbeCaptureCounter`.
- Caller: `AppModel.start()`, only before `DepthEngine.start()`; `AppModel` speaks "Sensor probe running…" / "Sensor probe finished…" because obstacle detection is off for the whole run. Never runs on a normal launch. Findings quoted elsewhere (re-run before changing them): face tracking beside LiDAR costs nothing measurable (60.0 fps either way) → `DepthEngine.setFaceTracking`; a multi-cam session beside ARKit starves ARKit to 8 frames per 4 s → `DualCameraSession` pauses ARKit. Tests: none (hardware measurement); `SoundAlertsTests` pins the label-list case.
...
| `centerOnsetPlayer` | Standard centre onset (`playCenterOnset(strong: false)`, Step 41) | 1 | 0 | 0.8 | 0.6 |
| `centerStrongPlayer` | Standard strong centre onset (`playCenterOnset(strong: true)`) | 3 | 0.08 s | 1.0 | 0.6 |
...
- `playCenterOnset(strong:)` (Step 41) — `stopApproachLoop()` first (Standard has no loop; one still running is left over from a level change mid-approach), `rendering = .center`, then `centerStrongPlayer` / `centerOnsetPlayer` once unless silenced / unhealthy. Called for `TorsoHapticAction.centerOnset`. Which level gets which torso pattern is decided upstream by `TorsoHapticPolicy`; this class has no level logic.
- `setApproach(distance:)` — updates `approachDistance`, `rendering = .center`, (re)starts loop if `!silenced && isHealthy`. Called for `TorsoHapticAction.updateCenter` (Detailed).
- `stopAll()` — `rendering = .clear`, cancels loop. Called for `CueOutput.stop`, on `.background`, and on `silenced = true`.
- `private fire(_:)` — `player.start(atTime: CHHapticTimeImmediate)`, errors to `lastError`, marks `isHealthy = false` and stops the approach loop so `AppModel` immediately mirrors cues to the watch/speech; a nil player is a no-op.
...
- `buildPlayers()` now builds the four obstacle players, the two Standard centre onsets, `groundPlayer` and the four `navPlayers`; a media-server reset rebuilds all of them.
...
Cross-module contract (`AppModel.handle(_:)`, ~30 Hz; since Step 41 every decider output passes through `TorsoHapticPolicy` first — a torso cue it suppresses reaches neither the player, the watch nor speech, and a Standard `.centerOnset` goes to `playCenterOnset(strong:)` with the mirror / speech of a centre cue): `.render(cue)` → `haptics.play(cue)`; `phoneCannotBuzz = !haptics.isHealthy || haptics.silenced`; if `phoneCannotBuzz || fallbackToWatch` → `watch.send(obstacle: cue.kind, now:)` (the link drops the same kind within 1 s); then `speakCueIfNeeded` → `CueSpeechPolicy.line(for:phoneCannotBuzz:now:)` ("Head height." at `.safety` on the head onset, ≥ 4 s between onset lines, and once more under 0.6 m; "Left." / "Right." / "<distance> ahead." only when `phoneCannotBuzz`, per kind ≥ 4 s, at `.obstacle`; spoken with ttl 6 s). `.updateCenter(d)` → `setApproach`; `.stop` → `stopAll` (the head episode is the decider's since Step 52; the head `cue` record carries `distance` and `onset`). `cueSpeech` is reset at route start. `AppModel.hapticsSilenced` (UserDefaults) mirrors into `haptics.silenced`. `announceChannels()` at route start says "Haptics unavailable. Obstacle cues will be spoken." when `!haptics.isHealthy && !watch.isReachable`. Outside the cue router: `nav.onNavCue` → `playNav(cue)` (every cue but `.obstacle`), and `AppModel.groundHazardFound` → `playGroundHazard()` (with the same `phoneCannotBuzz || fallbackToWatch` wrist mirror, sent as `.obstacle(.center)`).
...
- `naturalVoice: ElevenLabsVoice?` = `ElevenLabsVoice.fromSecrets()` (nil without a key); `useNaturalVoice = true` (pushed from the persisted `AppModel.naturalVoiceEnabled`, Step 53; switching back on resumes the prefetch backlog).
- `lastEngine: VoiceEngineDecision?` (Step 53) — the last dispatched line's engine and reason, re-set when its race / playback resolves; Scene engine card "Voice" row, Haptics voice pill label. `naturalVoiceOffline: Bool` (Step 54) — mirror of the breaker for the UI and `AppModel.voiceFacts()`.
- Hooks (set by `AppModel.start()`): `onDispatch: ((String, SpeechPriority, Int, Int, VoiceEngineDecision) -> Void)?` → `speech_dispatch {…, engine, engine_reason}`; `onEngineResolved: ((String, VoiceEngineReason, Int) -> Void)?` → `speech_engine {text, engine, engine_reason, wait_ms}`; `onBreakerChanged: ((Bool, String) -> Void)?` → `voice_breaker {open, reason}`; `onSpeakingChanged: ((Bool) -> Void)?` (Step 55, installed by `VoiceInputEngine.init`).
- `cachedShare(of lines:) -> Double` (Step 54) — share of unique non-blank lines on disk; 0 without a key, 1 for an empty list.
...
Private state beyond the backends: `queue`, `sequence`, `generation`, `currentPriority`, `currentText`, `currentExpires`, `currentReplays`, `interrupted: Bool` (between an interruption's `.began` and resume), `interruptionFallback: Task`, `breaker: VoiceBreaker` (Step 54, session-sticky; replaced `naturalVoiceFailedAt`), `probeTask` (60 s probe while open), `prefetchBacklog: [String]` (lines asked for and not yet fetched, newest batch first), `prefetchTask` (the one worker), `voicePending: Bool` (true while a natural-voice fetch is racing its 2.5 s deadline; whichever of the two claims it first clears it, so they are mutually exclusive; `stopCurrent()` clears it).
...
| session reactivation | 3 tries (`attempt` 0…2), 1 s apart; after the last failure it drains pending speech **only if the system posted `.ended`** (`fromEnded`); otherwise it re-arms the 15 s fallback and remains interrupted | `resumeAfterInterruption(attempt:fromEnded:)` |
| natural-voice total deadline | `VoiceEngineChoice.raceDeadline` 2.5 s from the race start to a started mp3; then system voice | `startRace` (URLRequest's timeout is an *idle* timeout, so a slow trickle could stall longer) |
| breaker | `VoiceBreaker`: opens on a race timeout / failure; closes only after a prefetch chunk that fetched ≥ 1 line; probe every `probeInterval` 60 s while open | `resolveRace`, `prefetchPass`, `armBreakerProbe` |
| prefetch chunk | `VoicePrefetch.chunk` = 2 lines per worker pass (merge off main) | `prefetchPass` |
...
#### Functions
- Step 65: `playEarcon(_:gainOffsetDB:) -> Bool` (applies `EarconPolicy.gate` — muted, or a `.safety` line playing — then `EarconPlayer.play`, soft haptic except `.listening`; reports through `onEarcon`, logged `earcon {name, played, reason}`), `perform(_ feedback:_ priority:ttl:)` (tone, then `say` of `feedback.line`). Not queued, no ducking, `isSpeaking` untouched.
...
- `prefetch(_ lines: [String])` — no-op unless natural voice active. **Additive** (Step 54): `lines` go to the front of `prefetchBacklog` (repeats dropped), `voiceError` cleared **only when the key has not been refused** (`NaturalVoiceLatch.shouldClearVoiceError`, Step 63), `runPrefetchWorker()`. Nothing is cancelled. Called by `AppModel.start()` (`voiceReadyLines`: safety lines first, then `commonLines`), `buildRoute` / `queueRouteStart` / `startRouteNow` (`routeStartLines`: announce, `WalkingIntro.routeStarted`, every `waypoint.say`), and by `speakNow` / `startRace` for a `warning_miss`, `breaker_open`, timed-out or failed line. `applyPersistedNaturalVoiceLatch()` restores a previous launch's refused-key latch before the first `say`.
- `runPrefetchWorker()` / `prefetchPass() async -> Bool` (private, Step 54) — one worker: each pass snapshots the backlog and `routeLines + backgroundLines`, on a detached `.utility` task runs `VoicePrefetch.merge` and `ElevenLabsVoice.prefetch` on the first `chunk` lines; back on main a fetched chunk leaves the backlog and may close the breaker (`onBreakerChanged(false, "prefetch_succeeded")`), a failed chunk stops the worker with the backlog kept (`voiceError`, not while backgrounded), an empty merge stops it (or runs once more if lines arrived meanwhile).
- `tripBreaker(_:)` / `armBreakerProbe()` (private, Step 54) — open + log + start the 60 s probe (`probed(now:)` then `runPrefetchWorker()`), which ends when the breaker closes.
...
- `speakNow(_ text: String, _ priority: SpeechPriority, expires: TimeInterval = .infinity, replays: Int = 0, resumeFrom: Int = 0, lastResumeOffset: Int? = nil)` (private) — `generation += 1`, ends any pause, records current text/priority/expires/replays/resume state, `isSpeaking = true`, `lastSpoken = text` (whole line), `spokenText = SpeechResume.remainder(of: text, from: resumeFrom)` (what the system voice says and the watchdog / mute timing measure; a cached or fetched mp3 plays the whole file from `SpeechResume.clipTime`, 0.25 s early), arms the watchdog, then (Step 53) `decision = VoiceEngineChoice.decide(muted:hasKey:naturalEnabled:cached:isWarning:breakerOpen:naturalUnavailable:)` — the cache `stat` only when it could matter — sets `lastEngine`, fires `onDispatch(…, decision)`, and acts on it:
  - **`muted`** → no audio at all; a `Task` sleeps `0.4 + text.count/15` s then `lineEnded(gen:)` (queue timing, coalescing and logging behave as on a device).
  - `elevenlabs` (`cached`, any band, breaker or not) → `playFile`;
  - `system` → `speakSystem`; for `warning_miss` (**warnings never wait for the network**) and `breaker_open` also `prefetch([text])`;
  - `race` → `startRace(text:spokenText:gen:voice:)`: `voicePending = true`, then a **2.5 s total-deadline** `Task` racing `fetchTask` (awaiting `naturalVoice.audio(for:)`). Each side acts only if `voicePending` is still set, `generation == gen`, **`isSpeaking`**, and (deadline) neither `player` nor `currentUtterance` exists / (fetch) `currentUtterance == nil`; the winner clears `voicePending`, so the deadline and the fetch completion are **mutually exclusive**. Deadline → cancel `fetchTask`, `resolveRace(.raceTimeout)` (trips the breaker), `speakSystem`, `prefetch([text])`. Fetch success → `resolveRace(.raceWon)`, `playFile`; failure → `voiceError`, `resolveRace(.raceFailed)`, `speakSystem`, `prefetch([text])`.
- `resolveRace(_:_:since:)` (private) — `lastEngine`, `onEngineResolved(text, outcome, wait_ms)`, `tripBreaker` on timeout / failure. `reportPlaybackFailure()` (private) — `speech_engine {engine_reason: playback_failed}` from `playFile`'s failure paths (breaker untouched).
...
- `interruption(_:)` (private) — `.began`: `requeueCurrent(fromClause: false)` if speaking, `stopCurrent()`, `isSpeaking = false`, `currentPriority = nil`, `currentText = ""`, `interrupted = true`, and arms `interruptionFallback` (15 s, then `resumeAfterInterruption(attempt: 0, fromEnded: false)` if still interrupted — `.ended` is not guaranteed). `.ended`: cancel the fallback, `resumeAfterInterruption(attempt: 0, fromEnded: true)`; both paths carry the interruption generation so stale retries cannot reactivate a newer interruption.
- `resumeAfterInterruption(attempt: Int, fromEnded: Bool)` (private) — `setActive(true)`; on failure `audioSessionError = "Audio resume: …"` and, if `attempt < 2`, retry after 1 s (same interruption generation and `fromEnded`) and return. After the third failure: **without `.ended`** (`fromEnded == false`) the call is probably still on, so it **re-arms the 15 s `interruptionFallback`** and returns without draining (never talk over a call); with `.ended` it drains anyway. On success, or `fromEnded` after three failures: `interrupted = false` and, if not speaking, `lineEnded(gen: generation)` drains only speech that is still valid in priority order; failed activation never drains pending safety speech.

Invariants a future editor must keep:
- **Generation token**: every backend callback (`didFinish`, `didCancel`, `audioPlayerDidFinishPlaying`, decode error, fetch completion, watchdog) is accepted only when its `gen == generation`. `stopCurrent()` must keep incrementing `generation`; `speakNow` must keep incrementing it before dispatch.
...
- Resume cap: an interrupted line resumes at most `SpeechResume.maxResumes` (3) times, never from an earlier point than its last resume, then it is dropped (Repeat recovers) — a head-height branch every few seconds must not loop the first words of a crossing line.
- While `interrupted`, nothing may start a backend: `say`/`sayAgain` only queue, `lineEnded` does not pop.
- No line waits more than 2.5 s for the natural voice, and after a failure none waits at all until a background prefetch has succeeded (session-sticky breaker, Step 54). No caller may bring back a per-line "skip the fetch" flag (owner decision: one voice). `muted` must stay env-only (`CANEKIT_MUTE` / `CANEKIT_UITEST`).
- ⚠ Do not change priorities, the re-queue/replay rule, the obstacle/safety no-network rule, the 2.5 s deadline / sticky breaker, the interruption re-arm or the watchdog without the CHANGELOG Step 10 device checks ("Speech that is never lost or looped": Repeat on the watch mid-line, "Head height." once with haptics silenced, phone call mid-route) and `docs/design.md` §5 (crossing/arrival/head = P0, obstacle names = P1).

---

### `ios/CaneKit/Speech/EarconPlayer.swift` — plays the calm-feedback tones (Step 65)
`@MainActor final class EarconPlayer` — `play(_ earcon:gainOffsetDB:haptic:) -> Bool`: builds (and caches per earcon + gain) `Earcon.wavData`, plays it with `AVAudioPlayer(data:)` in the session `SpeechQueue` owns (no `setCategory` / `setActive`, hard rule 7), keeps players alive in `playing` until they finish, and fires `UIImpactFeedbackGenerator(style: .soft)` when `haptic`. Owned by `SpeechQueue` (`earconPlayer`); never called directly by anything else. No unit test (numbers pinned by `EarconTests`).
...
- `prefetch(_ lines:) async -> String?` — `VoicePrefetch.queue` (order kept, cached dropped), a `TaskGroup` with at most `VoicePrefetch.maxConcurrent` (2) in flight, each with `prefetchTimeout` (15 s); stops at the first fatal HTTP status; returns the first failure's description or nil. Since Step 54 `SpeechQueue.prefetchPass` hands it one `chunk` at a time.
...
### `ios/CaneKit/Audio/FaceHeadPose.swift` — head yaw from the front camera, no AirPods (Step 14)

Purpose: head yaw from the **front** (TrueDepth) camera's `ARFaceAnchor`, delivered by `ARWorldTrackingConfiguration.userFaceTrackingEnabled` while the back camera's LiDAR keeps running, so the beacon can be head-tracked with no AirPods. It publishes **numbers only** (face found, yaw in degrees) — ARKit delivers one `capturedImage` per frame and it is the rear camera's, so there is no front-camera picture and the Hazards card never implies one.

- `@MainActor @Observable final class FaceHeadPose` — published `private(set) var yawDeg: Double?` (positive = head turned right of the recentred direction, range (−180, 180]; nil when no fresh face or no reference yet — **the same contract as `HeadPoseTracker.headYawDeg`**, so `BeaconEngine` needs no second code path), `isTracking`, `samples`; `@ObservationIgnored` `tracker: FaceYawTracker` (CaneKitLogic — smoothing, staleness, jump rejection and the reference all live there), `active`, `lastNow`.
- `start()` / `stop()` (clears everything: the UI must never show a head pose from a previous walk) / `ingest(worldYawDeg:now:)` (from `DepthEngine.onFaceYaw`, main actor, off the AR anchor delegate) / `refresh(now:)` (ages the value; called by `AppModel.handle` on every depth report — a face not seen for `FaceYawTracker.maxAge` reports nil, never a stale direction) / `recenter()` / `var readout: String` (the Hazards card's "Front camera … detecting:" row and the front-camera self test).
- Owner: `AppModel.faceHead` (one instance). Read by `AppModel.startTicker` and `autoRecenterIfWalkingStraight` through `HeadYawSelector` (AirPods first, this second); `isTracking` feeds the status summary (`HandsFreeIntents`, `ConversationCoordinator`). Off by default: Hazards card "Head tracking without AirPods" (`AppModel.faceHeadTrackingEnabled`, refused while a route guides or starts because the change re-runs the AR session — `FaceTrackingChange`). Measured on the phone (`SensorProbe`, `probe_a_world_plus_face`): face tracking beside LiDAR costs nothing measurable.
- Tests: `HeadYawSourcesTests` (the `FaceYawTracker` rules and `HeadYawSelector`); this wrapper is device-only.
...
- `nonisolated extension VLMClient` — default `describe(jpeg: Data)` = `describe(jpeg:prompt: ScenePrompt.text)`; default `describeScene(jpeg:)` times the call and returns `VLMAnswer(text:, source:, answeredBy: name, cloudMs: <ms, nil on-device>)`; defaults `lastHazardOutcome = nil`, `hazardCloudDeadline = nil`. Caller: `SceneDescriber.describe()`.
- `VLMAnswer` (`nonisolated struct: Sendable`) — `text`, `source: VLMAnswerSource` (`.cloud` → gate it, `.onDevice` → speak as is) and, Step 47, defaulted provenance: `answeredBy: String` (display name of the client that wrote it), `cloudMs: Int?` (cloud round trip when the cloud was tried — answered, or failed after this long), `fallbackReason: String?` (the primary's `localizedDescription` when `FallbackVLMClient` fell back: "The request timed out.", "HTTP 429: …"). Read by `SceneDescriber.recordOutcome`.
- `VLMHazardOutcome` (`nonisolated struct`) — `answeredBy`, `cloudMs`, `fallbackReason?` for the hazard path, which returns a bare `String`; kept in `VLMHazardOutcomeBox` (`nonisolated final class: Sendable`, a `Mutex` slot: `last`, `record(_:)`). Protocol members `lastHazardOutcome: VLMHazardOutcome?` and `hazardCloudDeadline: Duration?` (nil except on `FallbackVLMClient`) let `HazardScanner` read who answered and the deadline without casting.
- `FallbackVLMClient` (`nonisolated struct: VLMClient`) — `primary`, `fallback: any VLMClient`, **`hazardDeadline: Duration = .seconds(2.5)`**; `name = "\(primary.name) + \(fallback.name)"`. `describe(jpeg:prompt:)`:
  ...
  - Provenance (Step 47): `hazardOutcomes = VLMHazardOutcomeBox()` (a `let` with a default, so it stays out of the memberwise init and every copy shares the factory's one box); the hazard path records `VLMHazardOutcome(answeredBy: primary.name | fallback.name, cloudMs, fallbackReason)` after every reply; `describeScene(jpeg:)` returns the fallback's answer with `cloudMs` (how long the cloud was given) and `fallbackReason = error.localizedDescription` when it fell back. `lastHazardOutcome` / `hazardCloudDeadline` expose them.
...
- `SceneDescriber` — `@MainActor @Observable final class`. Published: `isDescribing`, `lastDescription`, `lastError`, `lastLatencyMs: Int`, `providerName: String?` (still declared optional but always `client.name` now — e.g. "Muse + On-device" or "On-device"; logged by `AppModel.describeScene()` and the `start` event). `GuideCard` shows "Describing…" / disables the button while `isDescribing`, and renders `lastDescription` (a11y label "Scene: …") and `lastError`. Private: `client: any VLMClient` (**injected, never nil**), `processor: DepthFrameProcessor`, `speech: SpeechQueue`. Step 63: `cancelForBackground()` bumps `describeGeneration` and cancels the live `Task` so a pre-lock JPEG never reaches `speech.say` (`SceneDescribePolicy.maySpeak`; caller `AppModel.scenePhaseChanged(.background)`). A cancelled run is silent (no "Scene description failed.", no `lastError`).
- Provenance of the last run (Step 47, `private(set)`, read by `SceneEngineCard` and `AppModel.wireDescriber`): `lastSource: String?` (who wrote the spoken sentence — "Muse" / "On-device"; a question's is the cloud's name; nil after a failure), `lastCloudMs: Int?` (`VLMAnswer.cloudMs`), `lastFallbackReason: String?` (`VLMAnswer.fallbackReason`), `lastGate: String?` (the `describe_result.gate` string: "spoken" / "edited: …" / "refused: …" / "on-device" / "error" / "no frame"), `lastAt: Date?`, `lastTrigger: DescribeTrigger?`; computed `cloudName: String?` (`client.cloudPrimary?.name`) and `onDeviceName` (`client.onDeviceFallback?.name ?? client.name`). All six are written by `private recordOutcome(source:cloudMs:reason:gate:)` **before** `onResult` fires, so the log and the card show the same values. `describe(trigger: DescribeTrigger = .button)` and `ask(_:)` (→ `.question`) pass the trigger into `run(question:trigger:)`.
...
- `isDark: () -> Bool = { false }` (`@ObservationIgnored`, Step 49; `AppModel.wireDescriber` sets it to `camerasInTheDark`) and `static darkCaveat = "It is dark, so this may miss things. "`: read once per accepted run when the result is spoken — a description (20 s) and a question's answer (10 s) are prefixed with the caveat, after the gate and after the people line (it is a fact about the camera, not a model claim), unless the sentence already contains `CloudSceneGate.tooDark`; `lastDescription` carries the prefix too.
- `onResult: ((String?, String?, Int?, String, String, String) -> Void)?` (`@ObservationIgnored`; text, error, ms, frame, gate, cloudText) — every outcome on the main actor: no frame, success, failure. `AppModel.wireDescriber` logs it as `describe_result` (with `trigger`, `source`, `cloud_ms`, `fallback_reason` read from the describer's provenance state, Step 47), so a walk log shows what "Where am I" actually said and who said it.
...
**`nonisolated struct VisionDetections: Sendable, Equatable`** — `labels: [(name: String, confidence: Float)]` (most confident first), `texts: [(text: String, confidence: Float)]`, `textHeights: [Float]` (each text's line-box height as a fraction of the image height, same order; filled by `detect`), `textBoxes: [SignPolicy.SeenText.Box?]` (line-box position per text, so stacked lines on one sign can be joined), `sightings: [Sighting]` (people / dogs / cats from the body and animal detectors, CaneKitLogic `PeopleAhead`), and the computed `seenTexts: [SignPolicy.SeenText]` (texts + heights + boxes for `SignPolicy`). `==` compares label names, text strings and sightings — confidences, heights and boxes are ignored. `boringLabels` deliberately does **not** contain "people" / "adult" (people are said, via `SceneVocabulary`). `OnDeviceVision.lastPeople: Mutex<String>` — the last "Where am I" body / animal summary ("2 person, 1 dog"), written by `OnDeviceVLMClient.describe` (not by `detect`, which the sign scanner calls every 3 s) and read into `describe_result.people`.
...
| `boringLabels` | outdoor, structure, material, blue_sky, sky, daytime, night_sky, land (8 entries) | too generic to say; `people` / `adult` are kept on purpose |
| label cap | none | no top-N cut on the labels themselves; only the `lastClassify` trip-log copy is `prefix(12)` |
...
- `static func detect(jpeg: Data, readText: Bool = true, classify: Bool = true, minTextHeight: Float? = nil, detectPeople: Bool = false) async -> VisionDetections` — runs off the caller's actor; the label pass and the body / animal pass (`detectPeople`, on only for a scene answer while `AppModel.namePeopleEnabled`, never for the hazard watch or signs) run concurrently (`async let`). `minTextHeight` = smallest text to read as a fraction of the image height (nil = Vision's default 1/32). Each request is `try?`: a Vision failure yields empty arrays, never a throw. Callers: `OnDeviceVLMClient.describe` (classify always; text only outside hazard mode; default text height) and `HazardScanner.scanSigns` (`classify: false` — classification there was wasted CPU every scan — and `minTextHeight: 1/128`, so 7.5 cm sign letters read from ≈ 7 m, not 1.7 m, measured by `ios/scripts/sign_probe.swift`).
...
- **`HazardScanner`** — `@MainActor @Observable final class`. Owned by `AppModel.hazards`, built in `AppModel.init` as `HazardScanner(processor: depth.processor, watchClient: client)`.
  - Published (`private(set)`): `isRunning`, `lastSign: String?`, `lastCaution: String?`, `watchProvider: String` (= `watchClient.name`, shown on `HazardsCard`), `lastWatchMs: Int?` (round trip of the last watch request), `lastError: String?`; Step 47 for the Scene engine card: `lastWatchSource: String?` (who answered — `watchClient.lastHazardOutcome?.answeredBy`, or the client's own name for a bare client; nil after a failure), `lastWatchReason: String?` (the cloud's error when on-device answered), `lastWatchAt: Date?`; computed `watchInterval` (= `watchPolicy.interval`, 8 s) and `hazardCloudDeadlineS: TimeInterval?` (= `watchClient.hazardCloudDeadline` in seconds, 2.5; nil without a cloud). The `hazard_watch` record gained `source`, `cloud_ms` (−1 without a cloud) and `fallback_reason` ("" when the cloud answered).
  ...
  - Inputs/outputs (`@ObservationIgnored`, set by `AppModel.wireHazards`): `onHazard: ((String, HazardSource, Data?) -> Void)?` (line, source, the JPEG it came from), `isNavigating: () -> Bool`, `isDark: () -> Bool` (Step 49; `AppModel.camerasInTheDark` — changes no behaviour, only stamps `light: "dark"` on `scan` and `hazard_watch` records), `currentSpeed: () -> Double` (m/s of the last fix; 0 without one), `onDiagnostic: ((String, [String: Any]) -> Void)?` (→ `logger.event`: a `scan` record per sign scan and a `hazard_watch` record per reply, including `dropped: "stale"` and errors — what the camera saw, not only what was spoken). `AppModel.wireHazards` also turns the watch on without persisting it when launched with `CANEKIT_HAZARD_WATCH=1` (Street View e2e).
...
- Attached once in `ContentView`: `.background(CameraControlInteraction { model.cameraControlPressed() })`; `AppModel.cameraControlPressed()` asks `CameraControlGate` (Step 67) and, when it allows, calls `describeScene(trigger: .cameraControl)`, whose `describe {trigger: cameraControl}` record is how the trip log shows whether presses reach the app under ARKit.

---

## Module `navigation-trip` — GPS, waypoint engine, turn settling, route sources, trip log/tracker, Live Activity

Files: `ios/CaneKit/Navigation/{LocationService,NavigationEngine,RouteSource,DestinationSearch,IndoorGuide}.swift`, `ios/CaneKit/Trip/{TripLogger,TripTracker,LiveActivityController,HazardLog,MedicalProfileStore,SupabaseClient}.swift` (the last two are documented under ui-tests-build next to `ProfilePage`), `ios/CaneKit/Resources/route_isr_cif.json`, `ios/CaneKit/Resources/indoor_isr.json` (Step 62 draft, `walked: false`), `docs/route_isr_cif.md`, plus `TurnSettle` in `ios/Logic/Sources/CaneKitLogic/NavSupport.swift`. All app classes are `@MainActor @Observable final class` (app target default `SWIFT_DEFAULT_ACTOR_ISOLATION: MainActor` in `ios/project.yml`); the pure decision logic they wrap (`GeofenceTracker`, `OffCourseDetector`, `GeoMath`, `Route`/`Waypoint`, `RouteBuilder`, `TurnSettle`, `StraightWalkDetector`, `CourseSmoother`, `NavigationHealth`, `TripRefreshGeneration`, `HazardRecord`/`HazardGeoJSON`) lives in `ios/Logic/Sources/CaneKitLogic/{GeoMath,Waypoint,NavSupport,CourseSmoother,NavigationHealth,TripRefreshGeneration,Hazards}.swift` (SwiftPM package, no default isolation: value types are `Sendable`, `GeofenceTracker`/`OffCourseDetector` are non-Sendable classes owned by the engine) and is tested by `ios/Logic/Tests/CaneKitLogicTests/{GeoMathTests,RouteTests,NavSupportTests,CourseSmootherTests,NavigationHealthTests,TripRefreshGenerationTests,HazardTests}.swift` (this module's tests are part of the 654-annotation suite at Step 51; `make test` / `ios/scripts/test.sh`; `logic-tests` job in `.github/workflows/ci.yml`, manual for now). Everything here is wired together by `AppModel.wireNavigation()` / `beginRoute()` / `stopRoute()` / `autoRecenterIfWalkingStraight(_:)` in `ios/CaneKit/App/AppModel.swift`; `LocationService` reports terminal/authorization failures and `NavigationEngine` rejects stale fixes via `NavigationHealth`.
...
| Location activity type | `.otherNavigation` (navigating) / `.default` (idle) | `startUpdatesLoop()`, `setNavigating` |

- `init()` — sets delegate, filter, orientation.
- `requestAlwaysAuthorization()` (Step 47) — sets `alwaysWanted`, then `manager.requestAlwaysAuthorization()` if the status is already `.authorizedWhenInUse`; otherwise `locationManagerDidChangeAuthorization` finishes the request the moment When In Use is granted (a first route right after install). Called by `AppModel.startRouteNow` (not under `CANEKIT_UITEST=1`). Why: with When In Use, a route that runs through the screen lock needs a `CLBackgroundActivitySession`, and that session puts iOS's blue location pill in the Dynamic Island, demoting the Live Activity to the minimal bubble; with Always no session is needed and OpenCane owns the island like Apple / Google Maps.
- `authorizationName: String` — "always" / "whenInUse" / "denied" / "restricted" / "notDetermined"; `backgroundSessionArmed: Bool` (`private(set)`) — a session exists right now. `onAuthorizationChange: ((String, Bool) -> Void)?` fires from `locationManagerDidChangeAuthorization` (AppModel logs `location_auth {status, background_session, at}`).
- `private reconcileAlwaysSession()` — while navigating holds an explicit `CLServiceSession(authorization: .always)` (`alwaysSession`, iOS 18+; the modern API's implicit session is When In Use, so this is what makes background delivery certain under Always) and logs its `diagnostics` through `onDiagnostic("always_session", flags)`; invalidated when the route ends. `onDiagnostic: ((String, [String]) -> Void)?` also carries the fix stream's flags (`authorizationDenied`, `authorizationRestricted`, `insufficientlyInUse`, `serviceSessionRequired`, `locationUnavailable`, `accuracyLimited`; deduplicated) — `AppModel` logs `location_diag {source, flags}`.
- `private reconcileBackgroundSession()` — arms `CLBackgroundActivitySession` iff `isNavigating` and the status is `.authorizedWhenInUse` or `.notDetermined` (never for Always — no pill — nor for denied / restricted, where it could deliver nothing), else invalidates it; called from `setNavigating` and on every authorization change (an Always grant mid-route drops the pill at once; a downgrade re-arms so the walk keeps GPS).
...
- `start()` — idempotent (`guard !isRunning`). Requests when-in-use, `startUpdatingHeading()` if `CLLocationManager.headingAvailable()`, spawns `updatesTask` iterating `liveUpdates()` (default configuration while idle so no persistent blue navigation indicator is held in the Dynamic Island). Sets `denied`/`authorized` from the update flags; skips updates with `authorizationRequestInProgress`; calls `ingest` for each `update.location`. Errors → `lastError`.
- `setNavigating(_:)` — arms or invalidates `CLBackgroundActivitySession` and switches `liveUpdates` configuration between `.otherNavigation` (during active route) and default (idle), preventing the system navigation indicator from anchoring 24/7. `manager.showsBackgroundLocationIndicator` stays `false` in both states (Step 47): the blue pill iOS draws around the Dynamic Island while a route runs in the background comes from the background session itself (it is what lets a When-In-Use app keep receiving fixes) and no flag removes it; the legacy manager flag is kept off so there is one system pill, never two.
- `stop()` — cancels task, stops heading, invalidates background session, `isRunning = false`, **`fix = nil`** (a stale fix must not seed the next MapKit route's origin). Called by `AppModel.scenePhaseChanged(.background)` when no route is running.
...
- `ingest(fix:course:)` — directly ingests an injected `GeoFix` and optional heading course into the navigation pipeline, bypassing CoreLocation. Used by on-device walk simulation and deterministic replay testing.
...
Published (`private(set)`): `route: Route?`, `isNavigating`, `arrived`, `instruction: String` (next waypoint's `say`, default `"No route"`), `distanceToNext: Int?` (metres, rounded), `targetBearing: Double?` (degrees true; **held previous-leg bearing while settling; nil = beacon silent** — crossing still settling, or curved leg), `bearingError: Double?` (target − heading, (−180, 180], positive = target is to the right), `waypointIndex: Int` (= number of waypoints reached), `startedAt: Date?`, `gpsWeak: Bool`, `isSettling: Bool`, `lastReached: Waypoint?` (most recently reached waypoint; auto-recenter checks `.crossing`). Computed (Step 41): `isCrossingSettle: Bool` = `isSettling && (lastReached?.crossing ?? false)` — the walker is standing at a crossing waiting for the curb / turn release; read by the cue router per report as `TorsoHapticPolicy`'s `crossingSettle` (no torso taps over "Listen for traffic"; the head cue is unaffected).
...
| `gpsWeakAfter` | 10 s | continuous bad accuracy before `gpsWeak` (pill, fences) turns on; the spoken line is `GPSAnnouncer` (10 s after its bad onset, review round Steps 67–68) |
...
| Arrival gate (last waypoint) | speed-exempt; `accuracy ∈ [0, maxArrivalAccuracy = 30]` **and plausibly inside**: `distance + accuracy ≤ radius` on `arrivalHits = 2` consecutive fixes (the full reported uncertainty must fit inside the fence; streak resets on any fix that is not an arrival candidate, and on every index move) |
...
- `update(fix: GeoFix)` — guard navigating and tracker. Order matters:
  ...
  2. **GPS-weak:** if `accuracy < 0 || accuracy > veerMaxAccuracy`: start `weakSince`; after `gpsWeakAfter` s set `gpsWeak = true`. Else clear `weakSince`, `gpsWeak = false`, `gpsLineSuppressed = false`. Then `announceGPS(now:)` (Step 68: `GPSAnnouncer` decides "GPS weak." / "GPS back."; also called first in `tick` and in `markLocationUnavailable`, which no longer speaks itself).
...
| `curbSlackM` | 4 m | extra metres beyond `nearM` that still count as "at the curb" for a crossing (`nearM + curbSlackM` = 10 m) |
...
Other types in the file (documented with their modules): `CueSpeechPolicy` ("Head height." on the head onset, ≥ 4 s between onset lines, once more under 0.6 m; "Left."/"Right."/"Ahead, …" only when the phone cannot buzz, ≤ 1 per kind per 4 s), `CrownAccumulator` (3 detents within 1 s of the first, 0.8 s debounce).
...
### `ios/CaneKit/Navigation/IndoorGuide.swift` (Step 62)

Purpose: walks an indoor step script from a room to the exit door, hands over to the outdoor route once GPS says the walker is outside, and runs the Settings recording mode. `@MainActor @Observable final class IndoorGuide`, one instance `AppModel.indoor`; decisions in CaneKitLogic `IndoorRoute.swift`.

- Observable walking state: `isActive`, `stepIndex` (0-based), `stepCount`, `atExit`, `waitingForGPS`, `currentSay`, `scriptName`, `isSimulating`, `scripts`; computed `guideLine` ("Indoors · step 3 of 5") and `statusClause` ("Indoors: step 3 of 5.", nil idle).
- Observable recording state: `isRecording`, `recordedStepCounts`, `recordedLandmarkCount`, `recordingFinished`, `isFinishingExit`, `awaitingLandmark`, `recordingStatus`, `recordingID` (default `IndoorScriptCatalog.defaultRecordingID`).
- `init()` → `reloadScripts()`: bundled `indoor_*.json` + `Documents/indoor/*.json` → `IndoorScriptCatalog.merged` (recorded wins by id); a file that fails `IndoorScript.load` or `validate()` is skipped and logged `indoor {action: load_failed, file, problems}`. `attach(_:)` (from `AppModel.start()`) sets the weak model, logs `indoor {action: catalog, scripts, walked, sim_rate}` and honours `CANEKIT_INDOOR_ROUTE=1` (starts the ISR script after 2 s).
- Review round: `sceneChanged(active:)` (from `AppModel.scenePhaseChanged`) logs `paused_background` / `resumed` while walking, speaks nothing; `start` calls `liveActivity.beginIndoor(name:say:stepIndex:stepCount:)` (trip name: `AppModel.bundledRouteName` for the demo leg, "To <destination>" otherwise) before the first `setIndoor`; `handOver` calls `liveActivity.endIfIdle()` when no outdoor route runs, waits or builds; a nil pedometer update logs `pedometer_nil {source}` ≤ once per 10 s (`ActionRateLimit`); the `CANEKIT_INDOOR_ROUTE` auto-start logs `auto_start_skipped` while navigating / waiting / indoors.
- `start(script:destination:source:)` — new generation, `IndoorProgress` + `IndoorHandover`, prefetches every line (`speech.prefetch`), logs `start`, performs `progress.start()` (draft caveat + step 0), then `simulate(rate:)` when `CANEKIT_INDOOR_SIM_STEPS_PER_S` is set, else CMPedometer (`pedometer_unavailable` logged when absent; "next" still works). ⚠ The pedometer handler is explicitly `@Sendable` and hops to main (TripTracker crash note).
- `perform(_:)` — mirrors `index` / `atExit` / `currentSay`, then per event: `.say` → `speech.say(line, .nav, ttl: 20)` + `indoor {action: say, text}` (never a `speech` record — e2e.py asserts on those); `.advanced` → `advanced`; `.reachedExit` → `handover.reachedExit()` + `exit`.
- `next()` (at the exit re-says the exit line), `repeatLine()` (`sayAgain`, `.nav`), `stop(reason:)` (silent, logs `stop`), `walkerIsOutside() -> OutsideResult` (`.notActive` / `.handedOver` / `.waiting`; `forced` / `forced_waiting`), `ingest(_ fix:)` (exit window while recording; handover gate with `now = fix.timestamp`, wall clock).
- `simulate(rate:)` / `stopSimulation()` — synthetic steps every 0.5 s from the last count (`IndoorSimSteps.steps`); at the exit a 5 m fix at the exit coordinate every 1 s through `LocationService.ingest(fix:course:)`, so the handover runs in the simulator. `sim_start` / `sim_stop`.
- Handover (`handOver(by:)`) — logs `handover {by, destination}`, tears down silently, then `IndoorScriptCatalog.outdoorLeg`: `.demoRoute` → `startDemoRoute()` (or `startSimulatedWalk()` when the indoor leg was simulated), `.navigate(text)` → `navigate(to:)`.
- Recording: `startRecording()` (refused while walking; CMPedometer `@Sendable` + `CMMotionManager.startDeviceMotionUpdates(using: .xArbitraryZVertical, to: .main)` at `IndoorRecorder.yawSampleIntervalS`, yaw through `IndoorYawUnwrapper`, `MainActor.assumeIsolated` because delivery is on main; warms GPS), `addLandmark()` (arms the hook for `landmarkWindowS`, `AppModel.startVoiceInput()`), `takeTranscript(_:) -> Bool` (the one-shot hook; a command per `IndoorRecorder.isLandmarkText` is not consumed and logs `record_landmark_command`), `finishAtExit()` (`IndoorExitAverager` window polled every 0.25 s; mean of good fixes → last fix ≤ 30 m of the window → `location.fix` if `usableFallback` (≤ 30 m, ≤ 20 s old); none → "No GPS fix. Step outside the door and tap Finish at the exit again."; a tap after the exit is recorded or while the window is open speaks a refusal, Muse M11 — so does `addLandmark` after the exit), `saveRecording()` (`recorder.script` with name / aliases / outdoor place / exit line / radius copied from the catalog script of the same id, `validate()`, pretty JSON → `Documents/indoor/<id>.json` atomically, `reloadScripts()`), `cancelRecording()`. Every recording line is shown (`recordingStatus`) and spoken at `.nav` (ttl 10). Logs `record_start`, `record_landmark_listen`, `record_landmark {text}`, `record_landmark_expired`, `record_finish {good_fixes, fallback, lat, lon}`, `record_finish_no_fix`, `record_save {id, steps, landmarks}`, `record_save_invalid`, `record_save_failed`, `record_cancel`.
- Every log is `indoor {action, index, count, steps, walked, script, …}` — never `t` / `kind`.
- Tests: the pure halves in `IndoorRouteTests`; the class is device-tested (CHANGELOG Step 62).
...
- `static func bundled() throws -> Route` — Step 66 automation hook first: with env `CANEKIT_ROUTE_FILE=<absolute path>` it loads that route JSON instead (same schema; the simulator stress campaign walks OSM-built routes through the demo-route hook; unset on every normal launch; `AppModel.bundledRouteName` reads it too, so the cloud upload label says `bundled`). Otherwise loads `route_isr_cif.json` from `Bundle.main` via `Route.load(from:)` (`JSONDecoder`, snake_case keys via `CodingKeys`). Throws `RouteError.missingBundledRoute` if absent. Called by `AppModel.startDemoRoute()` (GuideCard "Start route to CIF", `StartDemoRouteIntent`, and at launch when `--demo-route` / `CANEKIT_DEMO_ROUTE=1`).
...
- Step 50: `locality: Locality` (default `.campus`) and `localityOrigin` — `refreshLocality()` runs from `update(text:fix:)` and reverse-geocodes the fix with iOS 26 `MKReverseGeocodingRequest` (one request at a time) when it moved ≥ 500 m from the last geocoded point; the reply is applied only if `localityOrigin` still equals the request's coordinate (a late reply for an older fix is dropped), a failure clears `localityOrigin` so the next fix retries, and `republish(announce: false)` re-filters without bumping `revision` (no VoiceOver re-announce). `republish(announce:)` passes `locality:` to `DestinationSuggestions.suggestions` so rows from another country are dropped.
...
|---|---|---|---|
| `name` | string | `Route.name` | spoken in intro ("Route to <destination>…") |
...
| `lanes` | `ar_t` (ARKit monotonic clock, 3 dp), `head: [Float×N]`, `torso: [Float×N]` (2 dp, −1 = invalid), `trusted: Bool`, `depth: Bool`, `tracking_normal: Bool` (same-frame AR tracking state), `frame_seq` (published report sequence), `omega` (rad/s), `cue` (`CueKind` raw: `clear/center/left/right/head`), `thermal: String`, `battery: Int`, `mesh` (centre-hit classification or ""), `tilt` (`cameraTiltDownDeg`, degrees below the horizon, 2 dp, or JSON `null` before the first trusted frame), `fps` (`DepthEngine.fps`, published depth rate) — the last two for tuning the mount from the log; Step 51: `bands` (`"rows"` before a pose / `"metric"`), `head_cover`, `torso_cover` (three Bools each, L/C/R: can the camera see the band at 150 cm; a NO COVER cell logs −1 like a clear one) | `AppModel.handle(report)` ≤ 2 Hz |
| `depth_geometry` | Step 51, once per distinct set of intrinsics: `fx`, `fy`, `cx`, `cy` (depth-map px), `up` (world-up components of the camera axes at that frame), `pitch_deg`, `cam_h_cm`, `floor_max_cm`, `head_min_cm`, `cover_range_cm`, `half_fov_long_deg`, `head_cover_limit_deg` | `AppModel.noteDepthGeometry` |
| `head_cover` | Step 51: `state: "none"`, `text` (the spoken "Camera too steep for head-height cover. Torso obstacles only."), `tilt` — once per route (`HeadCoverNotice`); not a `speech` record on purpose (e2e.py asserts on `speech`) | `AppModel.handle` |
| `cue` | `cue` (`CueKind` raw or `"clear"`), `ar_t`, optional `distance` (m, centre approach / onset / Step 52 gated head distance), optional `onset` (Bool, head only: the fire that opened the episode vs a band re-fire); Step 41: optional `suppressed` (`TorsoSuppressReason` raw: `quiet`, `indoors`, `crossing_settle`, `standard_side`, `standard_center_hold`, `shoreline` — a decider torso cue the level did not render; `cue_audit.py` excludes these from `cues_per_min`) or `render` (`center_onset` / `center_strong`, a Standard onset tap) | cue router |
...
| `route_readiness` | `state` (`warming`, `ready`, `timed_out_fallback_gps`, `timed_out_tracking_limited`, or `cancelled`); warming also records `timeout_s` and `required_frames`; both timeouts record `reason` (`DepthReadiness.TimeoutReason.rawValue`, "" when the request deadline fired before the gate began) | `AppModel` route-start interlock (`failQueuedRouteStart`: `timed_out_tracking_limited` speaks "Camera tracking is limited, probably low light. Obstacle detection is running on LiDAR." at `.safety` 15 s and still starts the route — Step 49) |
| `light` | `state` (`unknown` / `lit` / `dark`), `lux` (smoothed, −1 before the first estimate), `torch`, `torch_by_app` | `AppModel.logLight` — every `LowLightPolicy` state change and every app-torch release / device cut-out (Step 49). `torch` records gain `by_app` on a request and `muted: true` on a swallowed confirmation; `scan` / `hazard_watch` gain `light: "dark"` while dark with no torch |
...
| `describe` | `provider`, `trigger` | `describeScene()` (incl. an accepted `cameraControlPressed()`) |
| `describe_skipped` | `reason` (`launch_grace` / `listening` / `launch_line` / `debounce`), `trigger` | `cameraControlPressed()` (Step 67) |
| `voice_launch` | `action: "skipped"`, `reason` | `listenAfterLaunchLine()` (Step 67; was `voice_menu`) |
...
Purpose: the arrival card's numbers — elapsed time, GPS-integrated distance, steps since route start. HealthKit refreshes carry a `TripRefreshGeneration` token captured at route start, so a late query result cannot overwrite a newer trip.
...
Purpose: start/update/end the navigation Live Activity (Dynamic Island + lock screen + StandBy + Smart Stack, rendered by `ios/CaneKitWidget`), coalescing updates because ActivityKit throttles; since Step 64 it also owns the island's phase (warming / walking / indoor / listening / thinking / arrived / stopped), the sensing fact (live / paused / none), `relevanceScore` and escalation alerts.

Contract type: `NavActivityAttributes` (`ios/Shared/LiveActivity/NavActivityAttributes.swift`, shared with the widget target): `routeName: String` and a `ContentState` (`instruction`, `distanceM`, `kind`, `obstacleStatus`, `obstacleDistanceM`, `headClearanceM`, `statusDetail`, `progress`, and Step 64's `phase`, `sensing`, `stepIndex`, `stepCount`, `warmupEndsAt`, `alertLevel`) — see the `NavActivityAttributes.swift` section under `watch-widget-shared`. `kind` ∈ `"turnLeft" | "turnRight" | "crossing" | "arrived" | "straight"` (`AppModel.lastNavKind`). Decisions are pure in CaneKitLogic: `IslandPhasePolicy`, `IslandAlertThrottle`, `IslandAlertLevel` (IslandPolicy.swift) and `LiveActivityCoalescer`.

**`final class LiveActivityController`** — `@MainActor @Observable`. Published: `isActive`, `lastError`. Private (`@ObservationIgnored`): `activity`, `activityRouteName` (the static name of the running activity), `requestSpec` / `requestDeferred` / `requestGeneration` (review round: a queued or foreground-deferred `Activity.request`), `coalescer`, `lastState`, `activityOperation` (serialized update/end chain), `inputs: IslandInputs` (`hasLiDAR = DepthEngine.supportsMesh`), `indoorSay`, `alertThrottle`, `closingActivity` + `closingTask` (the "Route stopped" card, ended after `static stoppedCardSeconds = 10`).

| Constant | Value |
|---|---|
| Update coalescing | `LiveActivityCoalescer.shouldEmit` — phase / sensing change at once (Step 64), hazard transitions at once (0.2 s flap guard), else 0.8 s floor + distance bands 2 / 5 / 10 m |
| Dismissal after end | arrival `.after(.now + 60)`; Stop `.after(.now + 10)` ("Route stopped" card, Step 64); restart / cancelled warm-up `.immediate` |
| Warm-up countdown | `warmupEndsAt = now + DepthReadiness.standardConfiguration.timeout` (5 s) |
| Relevance | `IslandPhasePolicy.relevance`: alert 100, guidance 75, voice 50, final card 10 |

- `beginWarmup(routeName:line:waitSeconds:)` — Step 64, from `AppModel.queueRouteStart`: sets `routeStartWaiting`; updates an existing activity in place to `warming` with `warmupEndsAt` (the indoor walk's at the handover), rewrites a queued request, else `request`.
- `cancelWarmup()` — from `AppModel.cancelPendingRouteStart`: ends a `warming` activity (or a warming request still queued / deferred) immediately; no-op for any other phase.
- `beginIndoor(name:say:stepIndex:stepCount:)` — review round, from `IndoorGuide.start`: `.indoor` inputs; updates an existing activity, rewrites a queued request, else `request` (which ends a closing "Route stopped" card first).
- `endIfIdle()` — review round, from `IndoorGuide.handOver` and `AppModel.buildRoute`'s no-fix / error branches: `end(immediate: true)` when an activity or request exists but no route, warm-up or indoor step drives it (logs `end_idle`).
- `flushPendingRequest()` — review round, from `AppModel.scenePhaseChanged(.active)`: performs a request deferred while not frontmost (logs `flushed`).
- `onLog: (([String: Any]) -> Void)?` — set in `AppModel.start()` to `logger.event("live_activity", …)`: `deferred`, `flushed`, `requested {phase, route}`, `request_failed {error}`, `request_cancelled`, `end_idle`.
- `start(routeName:instruction:distanceM:obstacleStatus:obstacleDistanceM:headClearanceM:statusDetail:depthTrusted:)` — from `AppModel.startRouteNow`: **any** existing activity (warming, or the indoor walk's) is updated in place to walking (coalescer reset); a queued request is rewritten; otherwise `request`. A failed queued start (`failQueuedRouteStart`) goes through `startRouteNow`, so it updates too.
- `private request(_:)` — `areActivitiesEnabled` guard → `lastError = "Live Activities are off in Settings"`; `endClosingCard()` + `end(immediate: true)` with the new inputs preserved; stores `requestSpec`, `isActive = true`; not `.active` → deferred (⚠ `Activity.request` throws from the background), else `enqueueRequest()`.
- `private enqueueRequest()` / `private performRequest(generation:)` — a main-actor task chained on `activityOperation` awaits `predecessor?.value` (the closing card's / old activity's end), then — if `requestGeneration` is unchanged and the app is not in the background (else re-deferred) — decides phase / sensing / indoor line / step now and calls `Activity.request(… relevanceScore:)`; failure → `lastError`, `isActive = false`.
- `update(instruction:distanceM:kind:obstacleStatus:obstacleDistanceM:headClearanceM:statusDetail:progress:depthTrusted:waypointIndex:waypointCount:now:)` — every GPS fix while navigating (`wireNavigation`): `depthTrusted` (depth running and `report.isTrusted`) updates the inputs together with `UIApplication.applicationState != .background`; waypoint index / count feed "3 of 6".
- `private push(…)` — the one path to `Activity.update`: `IslandPhasePolicy.decide(inputs)`; ⚠ `gatedGlance` sends the obstacle fields as clear / 0 unless sensing is `.live`; indoors the step line replaces the instruction and the step counter the waypoint counter; coalescer; `IslandAlertLevel.from`; `IslandAlertThrottle.shouldAlert(level:appActive: applicationState == .active, now:)` → `alertConfiguration` (title / body per level, `.default` sound); `ActivityContent(state:staleDate:relevanceScore:)`; chained `act.update(_:alertConfiguration:)`.
- `private republish()` — re-sends `lastState` with a freshly decided phase / sensing (no-op when nothing changed: the coalescer suppresses it).
- `setSensors(appActive:depthTrusted:)` — from `AppModel.scenePhaseChanged` (whose `.background` branch since Step 62/63 also keeps GPS on while `indoor.isActive`, for the outdoor handover): `.background` → `(false, false)` → sensing `.paused` at once ("Obstacles paused — unlock"); `.active` → `(true, false)` until the next fix brings the real trust.
- `setVoicePhase(listening:thinking:)` — from `AppModel.observeVoiceForIsland` (`voiceInput.isListening || isStarting`, `conversation.isProcessing`); no-op without an activity or without a change.
- `setIndoor(stepIndex:stepCount:say:)` — from `IndoorGuide.perform` (every step event) and `IndoorGuide.teardown` (nil / 0 / nil): **0-based** `stepIndex` (`IndoorProgress.index`), `stepCount`, the step's line; nil index or 0 count leaves indoor mode. No-op without an activity; a new step bypasses the floor (coalescer reset).
- `refreshObstacle(status:distanceM:headM:)` — depth path while standing still (keeps the last navigation fields). `refreshNavigation(instruction:distanceM:kind:progress:)` — `nav.onWaypointAdvanced`; re-derives the waypoint index from `progress × stepCount`.
- `end(final:immediate:stopped:)` — resets inputs, indoor line and throttle; bumps `requestGeneration` and drops a queued / deferred request (`request_cancelled` when there was no activity); content phase `.arrived` (final line, progress 1, 60 s) or `.stopped` ("Route stopped" / "Route ended", last progress; `stopped: true` → update now, `endClosingCard` in 10 s; otherwise immediate); `sensing: .none`; clears `activity`, `activityRouteName`, `lastState`, `isActive`. Callers: `nav.onArrived` (`final:`), `stopRoute` (`stopped: true`), `endRouteQuietly` (`immediate: true`), `request` and `cancelWarmup` (`immediate: true`).
- `endAllOrphanedActivities()` — at launch (Step 42), `.immediate`, each end chained on `activityOperation` (review round).
- `private endClosingCard()` — ends `closingActivity` immediately; from `closingTask` after 10 s, or from `request` when a new route starts first. Why: the Dynamic Island removes an *ended* activity at once (first Step 64 pictures: empty island 4 s after Stop), so Stop sends the card as an update (stale after 10 s) and ends it later.

⚠ Keep the coalescing — ActivityKit rate-limits and silently drops bursts; verify on a device walk. Any new `kind` string must also be handled by the widget's glyph switch. ⚠ Never let the widget show "Path clear" unless `sensing == .live` (AGENTS.md).
...
- `record(kind:text:fix:jpeg:distanceM:heightM:direction:headingDeg:speedMps:routeName:instruction:source:whatItSaw:severity:)` — creates the directory; debounces rapid sensor jitter (< 3.0s between records of the same kind or at the exact same location); if `jpeg` is given and fewer than `maxPhotos` records have a photo, writes `hazard-<session>-<n>.jpg` (`n = records.count + 1`) — **a failed photo never costs the hazard** (`try?`, `photo` stays nil); appends `HazardRecord` with the enriched telemetry fields — **no fix → recorded at (0, 0) with accuracy −1** so nothing is lost (`HazardGeoJSON` writes a null geometry for it; the JSONL `hazard` event keeps the context); then rewrites the whole GeoJSON (`HazardGeoJSON.encode(records)`, `.atomic`), `fileWritten = true`, `lastError = nil`. Any directory/encode/write throw → `lastError = "Hazard log: …"`.
...
- Step 64 mirrors of CaneKitLogic's island enums (the widget cannot link the package; same raw strings, lenient `init(from:)`): `NavIslandPhase` (`warming | walking | indoor | listening | thinking | arrived | stopped`, unknown → `.walking`), `NavIslandSensing` (`live | paused | none`, unknown → `.none`; `showsClear(isStale:)` = `self == .live && !isStale`, ⚠ mirror of `IslandPhasePolicy.showsClear`), `NavIslandAlert` (`none | near | curb | stop | head`, unknown → `.none`).
- `struct ContentState: Codable, Hashable, Sendable` — `instruction: String`, `distanceM: Int` (metres), `kind: String`, `obstacleStatus: LiveActivityObstacleGlance`, `obstacleDistanceM: Double`, `headClearanceM: Double`, `statusDetail: String` (≤ 120 chars), `progress: Double` (Step 47: 0…1, clamped); `routeName` is cut to `static maxRouteNameCharacters = 120` in `init(routeName:)` (review round); Step 64: `phase: NavIslandPhase` (default `.walking`), `sensing: NavIslandSensing` (default `.none` — ⚠ never green by default), `stepIndex: Int` (0-based indoor step, or the waypoint index while walking; ≥ 0), `stepCount: Int` (≥ 0; 0 = unknown), `warmupEndsAt: Date?` (warming countdown), `alertLevel: NavIslandAlert` (default `.none`). Custom `init(from decoder:)` with `decodeIfPresent` fallbacks for every field after `kind`, so old and new payloads decode both ways. Producer: `LiveActivityController`.
⚠ Compiled into two targets (`CaneKit` and `CaneKitWidget`); wire contract between processes; keep the payload well under ActivityKit's 4 KB.

---

### `ios/CaneKitWidget/CaneKitWidgetBundle.swift` — widget extension entry

`@main struct CaneKitWidgetBundle: WidgetBundle` — `body` returns `NavLiveActivity()`, and since Step 64 `OpenCaneAccessoryWidget()` and `TalkControl()` (OpenCaneControls.swift). `Info.plist`: `NSExtensionPointIdentifier = com.apple.widgetkit-extension`, `CFBundlePackageType = XPC!`. Bundle id `com.aritro.canekit.widget`, `SKIP_INSTALL: YES`, `SWIFT_ACTIVE_COMPILATION_CONDITIONS` adds `CANEKIT_WIDGET` (Step 64, for `TalkControlIntent`); the host app has `NSSupportsLiveActivities: true` and the `opencane` URL scheme.

---

### `ios/CaneKitWidget/NavLiveActivity.swift` — lock screen + Dynamic Island + StandBy + Smart Stack (redesigned Steps 47 and 63)

`struct NavLiveActivity: Widget` — `ActivityConfiguration(for: NavActivityAttributes.self)` with `.supplementalActivityFamilies([.small])`; the Dynamic Island carries `.keylineTint(keyline)`, `.widgetURL(OpenCaneLinks.guide)`, `.contentMargins(.horizontal, 14, for: .expanded)`. No buttons by design (phone on a cane is glanced at, not touched). Colours are `enum IslandPalette` (ink, ivory, ivoryDim, clear green, warning orange, head red, dropOff purple, paused amber, voice blue; `tint(_ NavIslandAlert)`) so the widget has no dependency on `Theme.swift`. design.md §6.7 has the per-phase sketches.

Private tables (one place for glyph, word and VoiceOver):
- `struct Manoeuvre(kind:)` — `symbol` / `word` / `spoken` / `tint` per `kind` (`turnLeft` / `turnRight` / `crossing` / `arrived` / else `figure.walk` "Straight on"). ⚠ Add a case when `AppModel.lastNavKind` gains a value.
- `struct Glance(state:isStale:)` — ⚠ Step 64 safety: `live = state.sensing.showsClear(isStale:)`; not live → paused ("Obstacles paused — unlock", amber) or unknown ("Sensing unknown" / "No update · sensing unknown"), never a hazard or a clear path; live → "Path clear" (green check) / "Obstacle 1.2 m" or "Stop · 0.5 m" (`alertLevel == .stop`) / "Head height 1.4 m" / "Drop-off ahead". Fields `symbol`, `tint`, `word`, `compact`, `spoken`, `isAlert`, `isClear`.
- `struct PhaseLook(state:)` — leading glyph / caption / tint per phase (hourglass "Warming up", manoeuvre, "Indoors", waveform "Listening", ellipsis "Thinking", flag "Arrived", stop.circle "Stopped").
- `NavLiveActivity.keyline(_:glance:)` (alert tint > voice blue > arrived green > paused amber > ivory), `showsGlance(_:)` (guidance and voice phases), `progressLabel(_:)` ("3 of 6", "Step 3 of 5"), `trailingCaption(_:isStale:)` ("to next · ±5m GPS", "to start", "indoor step", "No update" …), `summary(_:routeName:isStale:)` (one phase-specific VoiceOver sentence).
- `enum OpenCaneLinks` — `guide` (`opencane://guide`), `talk` (`opencane://talk`).

Views: `DistanceText` (`.numericText` transition), `WarmupCountdown(endsAt:)` (`Text(timerInterval:countsDown:)`, "…" when past), `TrailingFigure` (distance / countdown / "3/5" / seal / stopped count), `CompactFigure`, `CompactTrailing` (alert glyph + word, a phase word, or `ProgressRing` with the check only when `glance.isClear` and a pause glyph when paused), `ProgressRing`, `GlancePill`, `RouteProgressBar`, `ActivityRootView` (`activityFamily == .small` → `SmallCard`; `isActivityFullscreen` → `StandByCard`; else `LockScreenCard`), `LockScreenCard` (mark + "OpenCane · route" + `SensingBadge`; glyph · instruction ≤ 2 lines · figure; pill · bar · "3 of 6"), `SensingBadge` ("LiDAR live" / "Sensing paused" / "Sensing unknown" / "Warming up" / "No update"), `StandByCard` (64 pt glyph, 56 pt figure, glance word, mark, 8 pt level band), `SmallCard`.
Dynamic Island: expanded `.leading` = mark + glyph over caption; `.trailing` = `TrailingFigure` + caption; `.bottom` = instruction (3 lines, `minimumScaleFactor(0.85)`) then pill · bar · progress label (or route name); `compactLeading` = mark + glyph + `CompactFigure`; `compactTrailing` = `CompactTrailing`; `minimal` = the mark stroked in the alert tint or keyline.
Pictures: `make island` (`CaneKitIslandTour`) → `NN-island-{compact,expanded,walking,after-stop}.png` (after-stop is the "Route stopped" card since Step 64).
...
- Step 64: `LiveActivitySnapshot` gained `phase` and `sensing` raw strings (defaults `"walking"` / `"none"`, `decodeIfPresent`); a change in either emits at once, before the hazard and floor rules (rule 0). Pinned by `sensingChangeBypassesFloor`, `phaseChangeBypassesFloor`, `legacyJsonDecodesGracefully`.
- `static let staleAfter: TimeInterval = 300` (Step 47) — the `staleDate` offset the controller puts on every request and update; the widget dims the distance and says "No update" past it (`context.isStale`). 300 s outlives a crossing wait (no fixes while still) and catches an app killed mid-route within five minutes. Pinned by `staleAfterOutlivesACrossingWait`.
...
- `start(routeName:instruction:distanceM:obstacleStatus:obstacleDistanceM:headClearanceM:statusDetail:)` — calls `end(immediate: true)` first; primes `coalescer`; requests activity with `staleDate: Self.staleDate()`.
- `update(...)` — called on every GPS fix while navigating (`AppModel.wireNavigation`), now with `progress:` (waypoints passed / total). Gated by `coalescer.shouldEmit(...)` (progress is not part of the snapshot: it only changes with the instruction, which is); every update carries a fresh `staleDate`. `end(final:)` sends `progress` 1 on arrival and the last progress on Stop.
- `private static staleDate()` — `Date(timeIntervalSinceNow: LiveActivityCoalescer.staleAfter)`; never set on `end` (an ended activity is final).
- `refreshNavigation(instruction:distanceM:kind:progress:)` — from `nav.onWaypointAdvanced`: a manual Next at a standstill brings no fix, so this pushes the new line / distance / glyph / progress with the last obstacle glance and GPS detail, through the same coalescer.
- `endAllOrphanedActivities()` — at launch (`AppModel.start`, Step 42): ends every `Activity<NavActivityAttributes>` left by a crash or an Xcode rebuild, `.immediate`.
- `end(final:immediate:stopped:)` — stops activity and resets `coalescer`. Arrival stays 60 s; Stop leaves a 10 s "Route stopped" card (Step 64); restart dismisses immediately. Full controller API (Step 64 phases, sensing, alerts): the `LiveActivityController.swift` section under `navigation-trip`.


### Island policy (`ios/Logic/Sources/CaneKitLogic/IslandPolicy.swift`, Step 64)

Pure decisions behind the Dynamic Island; caller `LiveActivityController`. Tests: `ios/Logic/Tests/CaneKitLogicTests/IslandPolicyTests.swift` (`IslandPhasePolicyTests`, `IslandAlertLevelTests`, `IslandAlertThrottleTests`).
- `enum IslandPhase: String` — `warming | walking | indoor | listening | thinking | arrived | stopped`; `init(lenient:)` (unknown → `.walking`).
- `enum IslandSensing: String` — `live | paused | none`; `init(lenient:)` (unknown → `.none`).
- `enum IslandAlertLevel: String` — `none | near | curb | stop | head`; `rank` (0 / 1 near+curb / 2 stop+head); `static stopWithinM = 0.6`; `from(obstacle:distanceM:)` (warning ≤ 0.6 m with a known distance → `.stop`, else `.near`; head → `.head`; dropOff → `.curb`). Pinned by `levelFromGlance`, `ranks`.
- `struct IslandInputs` — `navigating`, `routeStartWaiting`, `voiceListening`, `voiceThinking`, `indoorStepIndex: Int?` (0-based), `indoorStepCount`, `appActive` (not backgrounded), `depthTrusted`, `hasLiDAR`; `hasIndoorStep`.
- `struct IslandState` — `phase`, `sensing`.
- `enum IslandPhasePolicy` — `decide(_:)` (listening > thinking > warming > indoor > walking; `phasePriority`, `indoorNeedsAValidStep`, `voiceKeepsSensing`); `sensing(_:)` (no LiDAR → none; background with a route, a warm-up or an indoor step script → paused (`backgroundIndoorIsPaused`, Step 62/63); warming → none; trusted → live; else none; `backgroundWithRouteIsPaused`, `backgroundWhileWarmingIsPaused`, `warmingIsNone`, `noLidarIsNone`, `untrustedDepthIsNone`, `walkingForegroundIsLive`); `showsClear(sensing:isStale:)` (⚠ live and not stale only; `showsClearOnlyWhenLive`); `relevance(phase:alert:)` (final 10, alert 100, voice 50, else 75; `relevanceOrder`); `unknownRawFallbacks`.
- `struct IslandAlertThrottle` — `static perLevelInterval = 30` s; `lastLevel`; `shouldAlert(level:appActive:now:)` (true only for a rank increase while not frontmost and ≥ 30 s after that level's last alert; records the level except for a throttled escalation, which keeps the previous level so it still alerts after the window — review round); `reset()` per route. Pinned by `clearToNearAlerts`, `escalationInsideWindowAlerts`, `deEscalationSilent`, `flappingIsThrottled`, `foregroundNeverAlerts`, `throttledEscalationAlertsAfterTheWindow`, `deEscalationUpdatesTheLevel`, `resetPerRoute`.

### `ios/Shared/Brand/ContourRingGeometry.swift` (app + widget, Step 64)

- `nonisolated struct SeededNoise` — SplitMix64 (moved from VoiceTile.swift unchanged).
- `nonisolated struct ContourRingParameters(index:)` — one ring's three harmonics (k, phase, amplitude multiplier), the Step 58 seeding bit for bit.
- `nonisolated enum ContourRingGeometry` — `ring(_:center:base:amplitude:gain:drift:steps:) -> Path`, used by `ContourRings.draw` (VoiceTile) and `OpenCaneMark`.
- `nonisolated struct OpenCaneMark: Shape` — `ringCount = 4` static rings (seeds from the tile's outer rings, radii 15–46 % of the side, larger relative wobble so it reads at 14 pt). Stroked by `NavLiveActivity`, `OpenCaneAccessoryWidget`.

### `ios/Shared/Intents/TalkControlIntent.swift` (app + widget, Step 64)

`struct TalkControlIntent: AppIntent` — title "Talk to OpenCane", `isDiscoverable = false`, `openAppWhenRun = true`. App copy `perform` → `IntentSupport.model()` → `toggleVoiceInput(source: "control")`; the widget copy (`#if CANEKIT_WIDGET`) is a no-op so the control compiles. Caller: `TalkControl`.

### `ios/CaneKitWidget/OpenCaneControls.swift` (Step 64)

- `struct TalkControl: ControlWidget` — kind `com.aritro.canekit.widget.talk`; `StaticControlConfiguration` + `ControlWidgetButton(action: TalkControlIntent())`, label "Talk to OpenCane" / `waveform.circle`. Control Center, Lock Screen, Action button.
- `struct OpenCaneAccessoryWidget: Widget` — kind `com.aritro.canekit.widget.accessory`; `StaticConfiguration` with `OpenCaneAccessoryProvider` (one entry, `.never`); `.accessoryCircular` (mark) and `.accessoryRectangular` (mark + "OpenCane / Tap to talk"); `widgetURL(OpenCaneLinks.talk)` → `CaneKitApp.onOpenURL` → `toggleVoiceInput(source: "widget")`.
- `OpenCaneAccessoryEntry`, `OpenCaneAccessoryProvider`, private `OpenCaneAccessoryView`.
No automated tests (Control Center / lock screen are unreachable from XCUITest).

### Tests that cover this module

| What | Test | How to run |
|---|---|---|
| LiveActivity coalescer & decoding | `LiveActivityCoalescerTests` (10 tests) | `make test` |
| Live Activity pictures (compact, expanded, walking, after Stop) | `ios/CaneKitUITests/CaneKitIslandTour.swift` `testDynamicIsland` (simulator; asserts only route start / stop, the island is a picture) | `make island` (PNGs `NN-island-*.png` in `SHOTS`) or `make uitest` |
...
### `Logic/Sources/CaneKitLogic/FallDetector.swift` — the cane went over (Step 43)
...
### `Logic/Sources/CaneKitLogic/ThreatWatch.swift` — a weapon in the vision reply (Step 43)
...
### `Logic/Sources/CaneKitLogic/ActionRateLimit.swift` — webhook button spam guard (Step 43)
...
### `ios/CaneKit/Alerts/FallWatcher.swift` — CoreMotion → FallDetector (Step 43)
...
## Module `conversation` — `ios/CaneKit/Conversation/` ("Talk to OpenCane", Step 23)

Files: `ConversationCoordinator.swift`, `VoiceInputEngine.swift`, `PostStore.swift`. The pure halves live in CaneKitLogic (`ConversationModels.swift` / `FastPathIntentClassifier.swift` / `ConversationPrompt.swift`, `UtteranceEnd.swift`, `HeadNodDetector.swift` — documented under module logic). All three classes are `@MainActor @Observable`, owned by `AppModel` (`conversation`, `voiceInput`; the store hangs off the coordinator).

### `ios/CaneKit/Conversation/ConversationCoordinator.swift` — one query, one of four paths

Purpose: a blind walker with one hand on the cane asks for things by voice — status, a destination, a post, a setting — without a screen. `handleQuery(_ rawQuery:) async` takes exactly one path, in order: (1) **fast path** (`FastPathIntentClassifier.classify`: deterministic, offline, 0 tokens — settings, status, route start / stop, posts, trip metrics → `executeAction`); (2) **scene question** (`isSceneQuestion` → `AppModel.askAboutScene` → `SceneDescriber.ask`, `CloudSceneGate` — never the chat model, even with a cloud key); (3) **no cloud model** (`client.cloudPrimary == nil` → one honest "I need a network model" line); (4) **cloud model** (`ConversationPrompt` + a camera frame → `ConversationResponseParser` → at most one tool call (`executeTool`) + one spoken sentence).
...
Step 65: `startListening(windowSeconds:kind:)` takes an `EarconPolicy.ListenKind` (default press / followUp by `windowSeconds`); after the 1519 haptic it plays `listening` (−8 dB for a follow-up); `stopListeningAndSubmit` plays `heard` for words, nothing for a self-hear drop, and for an empty press `nothing` + "I did not catch that." only on the 2nd in a row (`emptyPressCount`); empty launch / follow-up windows are silent.

Purpose: `SFSpeechRecognizer` (on-device when supported, Apple's server otherwise) on the iPhone's own microphone array, coordinated with `SpeechQueue` under AGENTS.md hard rule 7: keeps AirPods on A2DP (never Bluetooth HFP), silences the beacon while listening and restores `.playback` at once on release / end of utterance / timeout, holds the speech channel (`SpeechQueue.setVoiceHold` — set just before the microphone lease since Step 55; route and obstacle lines queue with their TTLs, `.safety` still breaks through; released before the answer), never answers its own voice (Step 55: the tap drops buffers while `SpeechQueue.isSpeaking` + `SelfHearFilter.tailSeconds`, and a transcript matching a line dispatched during the press is dropped silently — `voice_self_hear {action: paused | dropped, pause_ms, transcript, matched_line}`), ends listening on its own (`UtteranceEndDetector`), and gives every press an audible outcome (an answer, "I did not catch that.", or why it could not listen — silence is a bug).

- `enum VoiceInputState: Equatable, Sendable { idle, listening, recognizing(partialText:), processing, error(String) }` — published as `state`; no view reads it (the UI keys off `isListening`, `isStarting` and `ConversationCoordinator.isProcessing`).
- `@MainActor @Observable final class VoiceInputEngine` — published `state`, `isListening`, `isStarting`, `latestTranscript`; callbacks `onTranscriptionFinalized: ((String) -> Void)?` (→ `ConversationCoordinator.handleQuery`, then `finishProcessing()`), `shouldRestorePlaybackSession: (() -> Bool)?` (false while `SoundWatcher` owns the microphone), `onEvent: ((String, [String: Any]) -> Void)?` (→ trip log: `logStart` / `logEnd`, `voice_end` carries the reason). Methods: `startListening()` (resets the self-hear history), `stopListeningAndSubmit(reason: "press" | "silence" | "timeout")` (runs the filter first), `finishProcessing()`, `cancel()`, `noteDispatched(_:)` (Step 55, fed by `AppModel`'s `speech.onDispatch`; records only while starting / listening); private `speakingChanged(_:)` (the `onSpeakingChanged` listener: pause now, resume after the 0.3 s tail), `endSelfHearPause()` (resume + `voice_self_hear {action: paused, pause_ms}`, also from `cleanupAudioPipeline`); private `startEngine(attempt:sessionField:)` (format retry, one `UtteranceEndDetector` per session, the end ticker), `recognitionUpdate(text:isFinal:error:)`, `checkEnd()`, `cleanupAudioPipeline()`, `denied(speech:)`, `fail(with:)`. Route/interruption callbacks and permission/engine polling live for the full capture lifetime and are fenced by `VoiceInputGuard`.
- Relays: `private nonisolated final class SpeechBufferBox: @unchecked Sendable` (installs the audio tap; the tap fires on an audio thread; `paused: Mutex<Bool>` + `setPaused(_:)` since Step 55 — read inside the tap, never an actor hop) and `SpeechResultsRelay: Sendable` (carries only Sendable values into a main-actor hop, mirroring `SoundResultsRelay`). ⚠ Every framework callback closure formed in a main-actor method must be `@Sendable` (or formed in a nonisolated relay): an inferred `@MainActor` closure invoked off main traps (Step 24 device crash; 23 crash reports pulled on 2026-09-12).
- Owner: `AppModel.voiceInput`, built in `AppModel.init` with the app's `SpeechQueue` and `BeaconEngine`. Callers: `AppModel.toggleVoiceInput(source:)` (GuideCard "Talk to OpenCane", the Action Button's empty `TalkToOpenCaneIntent`) and `AppModel.startVoiceInput()` (AirPods double nod — start only, never a toggle). Tests: the numbers are `UtteranceEndTests` (1.5 s silence, 10 s cap, 0.25 s tick) and `SoundAlertsTests` (`MicrophoneStart` format retry); the engine itself is device-tested (CHANGELOG Steps 23–24, 30).

### `ios/CaneKit/Conversation/PostStore.swift` — the walker's posts survive a relaunch

`@MainActor @Observable final class PostStore` — `private(set) var markers: [WalkMarker]` (oldest first), `private(set) var lastError: String?` (one line prefixed "Posts: "; nil after a successful save; copied into the `marker_dropped` record's `error` by the coordinator), `fileURL` = Documents/posts/posts.json, `init()` (loads synchronously; a missing file starts empty silently, an unreadable one starts empty with `lastError` set — ⚠ the next `append` then overwrites it with this session's posts, not a merge), `append(_ marker:) -> Bool` (whole-file atomic rewrite per drop, like `HazardLog`). The JSON is the plain `Codable` form of `[WalkMarker]` (CaneKitLogic) — no new schema type. Owner: `ConversationCoordinator.store`; writer `dropPost(name:)`; readers `ConversationCoordinator.markers` (sent to the cloud model as `savedMarkers`, and the "Marker N" default name). No view shows the posts yet; the file is reachable through Files → On My iPhone → OpenCane. Nothing ever deletes a post. Tests: `ConversationLogicTests.walkMarkerJSONRoundTrip` pins the on-disk shape; the store itself is device-tested.
...
- Fields: `title: String`, `subtitle: String? = nil`, `systemImage: String`, `role: Role = .primary` (`enum Role { primary, secondary, destructive }`), `layout: Layout = .row` (`enum Layout { row, tile }`, Step 47), `hint: String? = nil`, `value: String? = nil`, `action: () -> Void`.
- Body: `Button` whose label is `rowLabel` or `tileLabel` by `layout`, `.buttonStyle(CKBigButtonStyle(role:))`.
  - `rowLabel` — `ViewThatFits(in: .horizontal)`: `HStack(icon, VStack(text, subtitle ≤ 2 lines).frame(maxWidth: .infinity), Spacer, chevron when primary)` else `VStack(icon, text, subtitle)` at accessibility sizes; padding H `lg` / V `md` (`sm` with a subtitle); `.frame(maxWidth: .infinity, minHeight: 72)`; `.contentShape(RoundedRectangle(18))`. The flexible `VStack` is what keeps a long subtitle from making `ViewThatFits` stack one button while its neighbours stay rows ("Simulate walk", Step 47).
  - `tileLabel` — icon `.title` over centred `text` (≤ 2 lines, 90 % scale) over centred `subtitle`; padding `md`; `.frame(maxWidth: .infinity, minHeight: CKMetrics.tile (108), maxHeight: .infinity)`. For a side-by-side pair: put both in an `HStack(alignment: .top)` with `.fixedSize(horizontal: false, vertical: true)` so both take the taller one's height (GuideCard "Where am I" / "Talk to OpenCane").
...
- `ContentView.keyboardUp` (Step 50) — `@State` flipped by `UIResponder.keyboardWillShow/HideNotification`; the `CKTabBar` stays mounted but collapses while true (`frame(height: 0)`, `opacity(0)`, `accessibilityHidden`) (iOS 26's floating keyboard "Done" capsule sat on the Profile icon; UIKit tab bars go behind the keyboard too).
- `private struct SceneEngineCard: View` (Step 47) — `CKCard(title: "Scene engine")` between the status card and the obstacle grid. Builds one `SceneEngineFacts` in `facts(at:)` from `model.describer` (`cloudName`, `onDeviceName`, `lastSource`, `lastCloudMs`, `lastLatencyMs` (nil until `lastAt` is set), `lastFallbackReason`, `lastGate`, `lastError`, `lastAt`, `lastTrigger`), `model.hazards` (`watchInterval`, `hazardCloudDeadlineS`, `lastWatchSource`, `lastWatchMs`, `lastWatchReason`, `lastWatchAt`), `model.hazardWatchEnabled`, `model.cueLevel.title` / `cuePlace.title` / `obstacleNamesEnabled`, and renders `SceneEngineSummary` rows: a `CKStatusPill` (`brain`, `chain.text` — "MUSE → ON-DEVICE" / "ON-DEVICE ONLY", spoken `chain.spoken`), `row("Where am I", lastAnswer)`, the `when` line in `CKFont.secondary` with `.updatesFrequently`, `row("Gate", gate)` only when the cloud answered, `row("Watch", hazardWatch)`, `row("Light", light)` (Step 49; `lightState: model.lightState, ambientLux: model.ambientLux, torchOn: model.torchEnabled, torchByApp: model.torchLitByApp`), `row("Cues", cues)`. `row(_:_:)` is the Hazards card's detection shape (uppercased caption in `CKFont.pill` + body text), one VoiceOver element labelled with the line's `spoken` sentence. Only the age- and lux-bearing rows (`when`, `Watch`, `Light` — `ambientLux` is not observed, it changes at 30 Hz) sit inside a `TimelineView(.periodic(from: .now, by: ageRefresh))` (`ageRefresh` 10 s, the "just now" window), so the tick redraws those lines and never the headline row VoiceOver may be reading (Muse review). `facts(at:)` also passes `lastWatchError: h.lastError`. ⚠ The 8 s / 2.5 s in the words come from the scanner, never from this file.
...
- `SettingsPage.cueSettings(_ model: Bindable<AppModel>)` (Step 36) — `CKCard(title: "Cues")`: `Picker("Cue detail", selection: model.cueLevel)` segmented over `CueLevel.allCases` (`title`: Quiet / Standard / Detailed; hint "How much OpenCane says and taps on its own. …"), `Picker("Place", selection: model.cuePlace)` segmented (Outdoors / Indoors; hint "Indoors shortens the head-height distance, …"), then `cueLevelCaption` (what the level changes — names, signs and, since Step 41, the torso taps per level: Quiet "No cane taps for torso obstacles, head height only.", Standard "One tap at 1.5 meters, a strong triple at 0.6, no side taps.", Detailed "Continuous centre taps and side taps."; an Indoors sentence ending "no torso taps"; a "Names are off" reminder when `obstacleNamesEnabled` is false outside Quiet / Indoors). The Step 36 sentence "Haptics are the same at every level for now" is gone; both picker hints describe the taps too. Each picker has a visible `Text` label (VoiceOver-hidden) and `.accessibilityElement(children: .contain)` + `.accessibilityLabel` so VoiceOver announces "Cue detail" / "Place" around the segments. ⚠ Segment titles are the `testCuePickersChangeAndRestore` contract.
- `SettingsPage.mountSettings(_ model: Bindable<AppModel>)` — `CKCard(title: "Mount")`: first `MountAimRow()`, then system `Toggle`s (labels are the XCUITest `switches[...]` keys; Step 49 adds "Flashlight on in the dark (routes)" → `autoTorchInDark`, between the beacon and trip-log toggles, hint "When the camera sees low light while a route guides, OpenCane turns the flashlight on so the cameras can see and drivers can see you, and off again when the light returns or the route ends. Obstacle detection works in the dark either way."):
...
| "60 fps camera (warmer)" | `highFrameRateCamera` (default false) | "Smoother live view; uses more battery and heat. Obstacle cues are the same either way." |
...
- `SettingsPage.familyAlertsCard(_ model: Bindable<AppModel>)` (Step 39/43) — `CKCard(title: "Family alerts")`: `Toggle("Send cane events to family", isOn: model.familyAlertsEnabled)` (disabled, with a red "No webhook key. Add OPENCANE_GROKBOT_WEBHOOK_URL and _KEY to Secrets.plist." line, unless `family.isConfigured`); `Toggle("Detect the cane falling", isOn: model.fallDetectionEnabled)` (hint says the thresholds are not tuned; disabled with an explanation when `fallWatcher.isSupported` is false); `FamilyContactsEditor()` (private struct: the address list with per-row "Remove <address>" buttons, a "Family email" `TextField("Add an address")` + "Add" button, and a `CKBigButton("Save family emails")` that is `.primary` while `familyContactsNeedSave` and calls `model.saveFamilyContacts()` — the app sends no email, "Save" posts the list to the bot); `Toggle("Add AI context", isOn: model.familyAlertsAIContext)` (disabled unless `family.canSummarize`; "Context written by <summarizerName>." or "No model key, so alerts carry facts only."); `CKBigButton("Send test event", "antenna.radiowaves.left.and.right", .secondary)` → `model.sendFamilyTestEvent()`; then `family.lastStatus` (a11y "Last family alert: …").
- `private struct IndoorRecordCard: View` (Step 62) — `CKCard(title: "Record indoor route")`, reads `model.indoor` (`@Bindable`). Idle: `TextField("Route id")` bound to `indoor.recordingID` (no autocapitalization / autocorrect), `CKBigButton("Start recording", "record.circle")` → `indoor.startRecording()` (disabled while indoors), `CKBigButton("Start indoor route", "figure.walk")` → `model.startIndoorRouteForTesting()` (disabled while indoors, navigating or a route start waits), the catalog line ("Indoor routes: <name> (not walked yet)"). Recording: the live line "Recording · N steps, M pedometer steps, K landmarks", "Add landmark" (`mic.fill`, disabled once finished or while awaiting), "Finish at the exit" (`door.left.hand.open`, value "reading GPS" while the window is open), "Save" (primary, enabled once finished), "Cancel recording" (destructive). Always: `indoor.recordingStatus`. New labels only (not in the UI-test contract).
...
Purpose: the Guide — instruction, distance, GPS/heading pills, the voice shell's giant microphone (`VoiceTile`, Step 58) with a compact row under it, the rest of the navigation buttons, beacon / headphone pills, and the route picker (demo route or typed MapKit destination). Everything the XCUITests drive lives here; since Step 58 several of those buttons start below the fold and the tests `scrollTo` them.
...
- Body inside `CKCard(title: "Guide")`:
  ...
  - GPS pills: `CKStatusPill(gpsWord, gpsTone, "location", spoken: "GPS: …", updatesFrequently)`; plus `"GPS weak"` (`.warning`, `exclamationmark.triangle`) when `model.nav.gpsWeak`; plus `"Dark"` (`.warning`, `moon.fill`, spoken "Low light; obstacle detection still works") while `model.lightState == .dark` (Step 49).
  - `VoiceTile()` (Step 58, own file) — the microphone button (≥ 60 % of the page height) and the assistant's last answer. Then the **compact row** (tiles, `fixedSize` vertically): navigating → `"Repeat"` (primary → `repeatInstruction()`), `"Next"` (secondary → `nextWaypoint()`), `"Stop route"` (destructive → `stopRoute()`) — the Guide's one three-in-a-row (design.md §6.1 exception); idle → `CKBigButton(title: isDescribing ? "Describing…" : "Where am I", "eye", .secondary, value "in progress" while busy)` → `describeScene()`, disabled while describing, and `"Start route to CIF"` (`figure.walk`, primary, disabled while `routeStartWaiting`) → `startDemoRoute()`. Then `model.describer.lastDescription` (a11y label `"Scene: …"`) and `lastError` in `laneUrgent`.
  - Indoor state (Step 62, `model.indoor.isActive`): the instruction `Text` shows `indoor.currentSay`; a plain `Text(indoor.guideLine)` ("Indoors · step 3 of 5", `CKFont.button`, `.updatesFrequently`) follows it; the compact row is the navigating one (`nav.isNavigating || indoor.isActive`: Repeat | Next | Stop route, same labels and actions — the AppModel hooks route them to the indoor step); below the fold only "Simulate walk" / "Stop simulation" (`indoor.isSimulating`). `.onChange(of: indoor.isActive)` resets the Stop route gate.
  - When `model.nav.isNavigating`:
    - (Repeat / Next / Stop route are in the compact row above since Step 58; Stop route keeps the `StopRouteConfirmation` two-tap gate, its armed line shown under that row.)
    ...
    - `"Simulate walk"` / `"Stop simulation"` (secondary).
  - Else (idle):
    ...
    - ("Start route to CIF" is in the compact row above since Step 58.) While `model.routeStartWaiting`, a destructive `"Cancel route start"` button appears and calls `model.cancelRouteStart()`; its spoken/watch confirmation is distinct from stopping an active route.
...
### ios/CaneKit/UI/VoiceTile.swift (Step 58)

Purpose: the voice shell's face — one giant round microphone in the middle of the Guide, inside ~40 thin hand-drawn-looking contour rings; tap anywhere on the rings to talk (owner decision 2026-09-13).

- `struct VoiceTile: View` — `@Environment(AppModel.self)`, `accessibilityReduceMotion`, `@ScaledMetric disc = 132`. One `Button` → `model.toggleVoiceInput()`; label a `ZStack` of `ContourRings` and the disc (`CKColor.accent`, `danger` while listening / starting; glyph `mic.fill` / `waveform` / `hourglass`), `.frame(maxWidth: .infinity)`, `.containerRelativeFrame(.vertical) { h * 0.6 }`, `.contentShape(Rectangle())`. Accessibility label exactly "Talk to OpenCane" / "Listening…" / "Starting…" / "Thinking…" (the old Talk tile's state logic), value "listening" / "starting", hint unchanged. Status line under it: the assistant's last answer (a11y "Assistant: …") or `hintLine` "Say route, where am I, or help.".
- `struct ContourRings: View` — `enum Motion { still, ripple, breathe }` (ripple while `voiceInput.isListening`, breathe while `speech.isSpeaking`, still otherwise or under Reduce Motion); `TimelineView(.animation(minimumInterval: 1/30, paused: still))` + `Canvas`; `nonisolated static func draw(in:size:t:motion:ink:)` strokes `ringCount = 40` closed paths, radius = base + three seeded sine harmonics (wobble grows outward), inner rings darker. The path math (and `SeededNoise`, SplitMix64, which makes every ring identical per frame and per launch) moved to `ios/Shared/Brand/ContourRingGeometry.swift` in Step 64 so the island's `OpenCaneMark` draws the same rings.
- Hidden from VoiceOver except the button. No unit test (drawing); `make tour` pictures.
...
- `struct ProfilePage: View` — `@Environment(AppModel.self) private var model`; `@State private var showingEditor = false` (the `EditMedicalIDSheet`), `@State private var showingShareSheet = false`. The top Privacy card exposes the opt-in `model.cloudSharingEnabled` switch; fresh installs contain no seeded identity, DOB, address or emergency phone.
- `medicalIDCard: some View` — `CKCard(title: "EMERGENCY MEDICAL ID")`:
  ...
  - Emergency contact card with direct tap-to-call button (`tel:` + `EmergencyConfirm.telDigits(phone)` — the same digits the voice call dials, Step 59) and explicit accessibility label.
  - `"Announce Medical ID"` button: reads the user-requested emergency identification paragraph via `model.speech.say(summary, .scene, ttl: 20)`, so an obstacle or navigation warning may interrupt it and it can be resumed/repeated under the normal speech policy.
...
### ios/CaneKit/Trip/MedicalProfileStore.swift (Step 44, Step 46, Step 68)

Step 68: `private static seedingContact(_:)` — `init` passes the stored (or default) profile through `EmergencyContactSeed.seeded` with the git-ignored Secrets.plist keys `EMERGENCY_CONTACT_NAME` / `EMERGENCY_CONTACT_PHONE`; applied in memory only when the stored phone is blank (a typed number wins; never saved by the seed). `ProfilePage` and `ConversationCoordinator` (EmergencyConfirm) read the result.
...
Purpose: the "Hazards" card — the camera's second job made visible: switches for the hazard sources (LiDAR drop-offs, sign reading, hazard watch, naming people), the last thing each source said, which backend the hazard watch uses, the hazard-map count with a share button, three more sensor switches (microphone, nod to talk, front-camera head tracking), and the two optional camera views (`LiveCameraView`, `BothCamerasView`) for a sighted spotter / the demo video. Last card on the Sense page, after the obstacle grid (owner `SensePage` in ContentView.swift).

- `struct HazardsCard: View` — `@Environment(AppModel.self) private var model`, `@Environment(\.scenePhase) private var scenePhase` (owns no other state: no `@State` frame, no refresh loop). Body inside `CKCard(title: "Hazards")`, all `CKFont.body` / `textPrimary`. **Ten toggles**, in order:
  ...
  - `Toggle("Name people ahead", isOn: $model.namePeopleEnabled)` — hint "When you ask where am I, says how many people are ahead, which way and how far. Experimental — unverified on cane walks." (default off on a fresh install; persisted legacy opt-ins remain explicitly experimental; `PeopleAhead` plus `PeopleDetection`).
  ...
  - Detection rows via `private func detection(_ source:, _ text:)` (caption `source.uppercased()` in `CKFont.pill` / `textSecondary` + the line in `CKFont.body`; one combined a11y element), each only when non-nil: `detection("LiDAR", model.lastGroundHazard)`, `detection("Sign", model.hazards.lastSign)`, `detection("Watch", model.hazards.lastCaution)`; further down `detection("Sound", model.sounds.lastAlert)` (in `soundStatus`, with `sounds.lastError` in `laneUrgent` and `sounds.labelReport` while running — all spoken rows) and `detection("Front camera", "detecting: \(model.faceHead.readout)")` (in `frontCameraReadout`, only while `faceHeadTrackingEnabled` and `DepthEngine.supportsFrontCameraWithLiDAR`, followed by "Head direction only — ARKit gives one camera picture at a time, and it is the back camera's." ⚠ never re-word this row into anything a reader could take for a selfie preview).
  - Error line `model.hazards.lastError ?? model.hazardLog.lastError` in `CKFont.secondary` / `laneUrgent`.
  - When `model.hazardLog.fileWritten`: `ShareLink(item: model.hazardLog.fileURL)` labelled "Share hazard map" (`square.and.arrow.up`), hint "Shares a GeoJSON map of every hazard found on this walk".
  - `Toggle("Listen for sirens and horns", isOn: $model.dangerSoundsEnabled)` — hint "Uses the microphone to warn about sirens, horns and vehicle sounds. Needs the microphone, so it is off by default." (`SoundWatcher`; the one feature that leaves the single `.playback` session; not persisted).
  - `Toggle("Nod to talk", isOn: $model.nodToTalkEnabled)` — hint "Nod twice with AirPods on while walking a route to start talking to OpenCane. Off by default." (`HeadPoseTracker` → `HeadNodDetector` → `AppModel.startVoiceInput`; not persisted, thresholds untuned).
  - `Toggle("Head tracking without AirPods", isOn: $model.faceHeadTrackingEnabled)` — hint "Uses the front camera to follow your head direction, so the beacon works without AirPods. Cannot change while a route is guiding you." (disabled without front-camera-with-LiDAR support; while `model.nav.isNavigating` a `liveCaption` says it "cannot change while a route is guiding you", while `model.routeStartWaiting` "… while a route is starting" — the model refuses the change, `FaceTrackingChange`; not persisted).
  - `Toggle("Live camera view", isOn: $model.liveViewEnabled)` — hint "Shows what the camera sees, for a sighted helper" (not persisted). Below it `liveView`: `LiveView.state(enabled:hot: model.hazards.paused, cameraRunning: model.depth.isRunning, highFrameRate: model.highFrameRateCamera, foreground: scenePhase == .active)` → `.off` nothing, `.hot` caption "Live view paused: phone is hot", `.cameraOff` caption "Camera off", `.live(fps)` → `LiveCameraView(session: model.depth.arSession, framesPerSecond: fps)` at 3:4, clipped to `CKRadius.button`, **`accessibilityHidden(true)`**.
  - `Toggle("Both cameras (pauses obstacle detection)", isOn: $model.bothCamerasEnabled)` — hint "Shows the front and back cameras at the same time for a sighted helper. While it is on, obstacle warnings, depth and hazard detection stop. It cannot be used while a route is guiding you." Below it `bothCameras`: `BothCameras.state(enabled:supported: DualCameraSession.isSupported, navigating:foreground:)` → `.blockedByRoute` caption "Both cameras cannot run while a route is guiding you. Stop the route on the Guide tab first.", `.unsupported` caption "This phone cannot show two cameras at once, so the two-camera view is not available." (no single-camera fallback), `.live` → `BothCamerasView(session: model.bothCameras)` (3:4, VoiceOver-hidden) plus a **spoken** red line ("Back camera with the front camera inset. Obstacle detection is paused." or "Back camera only — the front camera could not be opened. …") and `bothCameras.lastError`. ⚠ Never make this block quieter: it is the one control that switches the safety channel off.
  - `Toggle("Flashlight", isOn: Binding(get: { model.torchEnabled }, set: { model.setTorch($0) }))` — hint "Turns the back-camera flashlight on or off. Works while a route guides and while both cameras are on. Off at every launch."
- `private func liveCaption(_:)` — the small grey explanatory line under a camera mode that shows nothing; spoken by VoiceOver.
- Debug buttons "Both cameras self test" and "Front camera self test" (`CKBigButton`) appear at the bottom **only** under `AppModel.selfTestControlsVisible` (a launch flag); the front-camera one is disabled while navigating.
- Tests: the view decisions are pure — `LiveViewTests` (`LiveView.state`, `BothCameras.state`, `FaceTrackingChange`, `BothCamerasLayout`), `HazardTests`, `SoundAlertsTests`, `HeadNodDetectorTests`, and `PeopleDetectionTests`. `CaneKitUITests.testAccessibilityLabelsExist` opens Profile and asserts its privacy/Medical ID actions and metric tile; other hazard copy remains a VoiceOver stability contract even where no XCUITest query exists.

---

### ios/CaneKit/UI/LiveCameraView.swift

Purpose: the Hazards card's "Live camera view" — ARKit's own camera frames drawn on the GPU for a sighted helper and the demo video. Replaced the ~3 Hz JPEG refresh loop (`HazardsCard.refreshLoop` + `AppModel.liveFrameJPEG`, both removed) that looked choppy and cost a CPU encode per frame.

- `struct LiveCameraView: UIViewRepresentable` (main actor) — `let session: ARSession` (the app's single session, `DepthEngine.arSession`; **displayed only**, never run, paused or delegated from here — `DepthEngine` owns the lifecycle and its `SessionObserver` must stay the delegate, because those frames feed the obstacle lanes), `let framesPerSecond: Int` (`LiveView.live(fps:)` = `CameraRate.previewFramesPerSecond`: the camera's rate capped at `CameraRate.previewCap` = 30, so the preview is 30 fps even when the Mount card's "60 fps camera (warmer)" runs the camera at 60 — a helper's picture must not add heat that pauses the walker's warnings; `liveViewRendersAtMostThirtyFramesPerSecond`).
- `makeUIView` — an `ARSCNView` with the shared session attached (its own default `ARSession` is never run and is dropped), automatic lighting / camera grain / motion blur off, `preferredFramesPerSecond`, touches and VoiceOver off; `updateUIView` refreshes the rate; `static dismantleUIView`; `makeCoordinator` → `nonisolated final class Coordinator: NSObject, ARSCNViewDelegate` that returns nil for every anchor (hundreds of mesh anchors get no SceneKit nodes) — nonisolated because SceneKit calls it on its render thread; it holds no state.
- Evidence (headless probes in the iOS 27 simulator, 2026-09-11): assigning a delegated `ARSession` to `ARSCNView.session` leaves delegate and delegateQueue untouched, and the view issues no `run` / `pause` — see the file header.
- Built only while the switch is on, the phone is cool, ARKit is running and the app is foreground (`LiveView.state`, CaneKitLogic); hidden from VoiceOver. Caller: `HazardsCard.liveView`. Tests: `LiveViewTests` (state + rate); the view renders only on a device.

### ios/CaneKit/UI/BothCamerasView.swift

Purpose: the two-up picture — the back camera filling the frame with the **front** camera inset in a corner, both live — the only place in CaneKit with two camera images on screen. Exists because `AVCaptureMultiCamSession`, not ARKit, can do it (measurements in `DualCameraSession.swift`).

- `struct BothCamerasView: UIViewRepresentable` — `let session: DualCameraSession` (`AppModel.bothCameras`; displayed only — `AppModel` starts / stops it together with pausing and resuming ARKit). `makeUIView` builds a `DualPreviewHostView`, `attach(back: session.backLayer, front: session.frontLayer)`, user interaction and accessibility off; `updateUIView` is a no-op; `static dismantleUIView` detaches.
- `final class DualPreviewHostView: UIView` — hosts the two `AVSampleBufferDisplayLayer`s owned by `DualCameraSession` (`attach(back:front:)`, `detach()`); `layoutSubviews` places them from `BothCamerasLayout` (CaneKitLogic), so the inset geometry is pinned by tests. No SwiftUI redraw and no CPU pixel copy in the video path. ⚠ Deliberately **not** `AVCaptureVideoPreviewLayer` (Apple forums 742501: LiDAR depth goes unreliable once a preview layer joins a depth-producing session).
- A spotter / demo feature: while it is on ARKit is paused and the obstacle lanes, depth and hazard detection are off; `AppModel` says so aloud on the way in and out. The view is hidden from VoiceOver; the caption and the switch label carry the meaning. Caller: `HazardsCard.bothCameras`, only in the `BothCameras.state == .live` branch. Tests: `LiveViewTests.bothCamerasInsetSitsInTheBottomTrailingCorner`, `bothCamerasInsetIsClampedInAShortBox` (geometry); no automated test of the view itself (no multi-cam in the simulator). Per-camera buffer rotation is decided in `DualCameraSession`, not here.
...
- `private func scrollTo(_:) -> XCUIElement` (Step 58) — up to 4 swipes up, then up to 8 down, until the element exists and is hittable; the voice tile pushes Recenter, Simulate walk, Go, Destination and Navigate to CIF from here below the fold. Used before those taps (and before Stop route after scrolling). Same helper in both tours.
...
- `testNavigateToCIFButtonIsOnTheIdleGuide` — `buttons["Navigate to CIF from here"]` exists (10 s) and is enabled on the idle Guide; after "Start route to CIF" → "Stop route" it no longer exists (route controls replace the picker); after Stop it is back (10 s).

(10 `func test…` methods in all, counting the skipped Street View one.)
...
### ios/CaneKitUITests/CaneKitIslandTour.swift (Step 47)

Purpose: a camera for the Live Activity — the only automated eye on the widget. `final class CaneKitIslandTour: XCTestCase`, `continueAfterFailure = true`, `springboard = XCUIApplication(bundleIdentifier: "com.apple.springboard")` created in `setUp` (an inline default is a main-actor value in a nonisolated context under Swift 6).

- `testDynamicIsland()` — tap "Start route to CIF", wait for "Stop route", `XCUIDevice.shared.press(.home)` → `01-island-compact`; long-press the island (`springboard.coordinate(dx 0.5, dy 0.035).press(forDuration: 1.2)`) → `02-island-expanded`; tap away; `app.activate()`, tap "Simulate walk" if present, 6 s, home → `03-island-walking`; activate, "Stop route", home → `04-island-after-stop`. Asserts only route start / stop (the same contract as `CaneKitUITests`); everything about the island is a picture.
- `snap` / `save` / `pause` / `scrollTo` (Step 58, before Simulate walk and Stop route) — same shape as the visual tour, kept local so the two tours stay independent. Run: `make island` (PNGs in `SHOTS`), also part of `make uitest`. The simulator shows no system location pill, so the blue background-location indicator the phone adds during a route is not in these pictures.
...
- `testTour()` — order of shots: `01-idle-top`, `02-idle-middle`, `03-idle-bottom` (scrolling), scroll to top, tap Start, wait for Stop, pause 1.5 s → `navigating`; `Repeat` (0.5 s) → `after-repeat`; `Next` (1.0 s) → `after-next`; `Recenter` (0.5 s) → `after-recenter`; `navigating-middle`, `navigating-bottom`; scroll top; tap each haptic test button if present (0.3 s); flip "Silence haptics" → `haptics-silenced` (via `snapElement`), flip back; tap "Where am I" (1.5 s) → `where-am-i-no-key` (the name predates the on-device fallback; in the simulator it shows the no-camera state); scroll top; Stop twice within the confirmation window, wait for Start → `stopped`; tap `Go` empty → `go-empty`. Filenames are `String(format: "%02d-%@", shotIndex, name)`.
...
- `scrollDown()` = `swipeUp` + 0.4 s; `scrollToTop()` = 4× `swipeDown` + 0.4 s; `scrollTo(_:)` (Step 58) before Recenter, Stop route and Go; `pause(_:)` = RunLoop spin; `flip(_:)` identical to the other file.
...
Runs the `CaneKitLogic` Swift Testing suite from `ios/Logic` (654 `@Test` annotations in 53 files at Step 51; recount with `grep -rhoE '^\s*@Test' ios/Logic/Tests | wc -l` rather than maintaining a stale per-file count). If `xcode-select -p` points at `Xcode.app`: `exec swift test "$@"`. Otherwise (Command Line Tools only) adds `-Xswiftc -Fsystem <CLT Frameworks>`, `-disable-cross-import-overlays`, and linker `-F`/`-rpath` so Swift Testing links (tests use only core Testing + Foundation).
...
| `island` | (Step 47) same as `tour` but `-only-testing:CaneKitUITests/CaneKitIslandTour` — Dynamic Island pictures `NN-island-*.png` in `SHOTS` |
...
| `audit` | (Step 35) `python3 scripts/cue_audit.py --selftest`, then `python3 scripts/cue_audit.py $(if $(LOG),"$(LOG)",--pull)` — cue load of one walk from its trip log: `LOG=path` for a file, otherwise the log is pulled off the phone |
...
`.PHONY` lists every target: gen test build install launch run sim devices clean check-env uitest tour island sim17 sim-grant e2e uitest-streetview audit. The header comment block lists every one of them (including `uitest-streetview`, `island` and `audit`) with a one-line purpose.
...
- **Step 66 (stress campaign)** — opt-in, never in `all`:
  - `--scenario stress_<walk>` / `stress_all` / `--trace FILE` — `stress_names()` (the stems of `scripts/stress/traces/*.json`), `load_trace(path)` (trace JSON `{name, route, points}` or a GPX track on the bundled route), `route_waypoints(route)`, `ticks(points, speed)` (simctl seconds, ≥ 1 tick per vertex), `perturb(points, seed, jitter_m, speed, dropouts)` (densified to `speed` m per vertex, AR(1) Gaussian jitter ρ 0.7 with the ends pinned, `dropouts` cuts between 15 % and 85 %; seeded, no clock; segments share their cut vertex). `run_scenario` gained `route_file` (→ `SIMCTL_CHILD_CANEKIT_ROUTE_FILE`, `RouteSource.bundled`), `segments` + `gap_s` (each segment is its own `simctl location start`; the harness sleeps the segment's ticks + `gap_s`, so the app gets no fixes in the gap) and `log_name`. `check_stress(events, wps, max_lpm) -> (failures, metrics)`: route start carries the route file's waypoint count, waypoint indices strictly ascending, the last `gps` fix at or before `arrived` ≤ 25 m from the last waypoint, `speech` (non-repeat) lines between route start and arrival ≤ `--max-lines-per-min` per minute, no identical consecutive lines; metrics `walk_s`, `lines`, `lines_per_min`, `arrival_m`, `veers`, `passed`, `gps_weak`, `duplicates`. The usual `check` also runs (its span test discounts the gaps).
  - `--scenario indoor_isr` — `run_indoor(udid)`: GPS set at `indoor_isr.json`'s exit, launch with `SIMCTL_CHILD_CANEKIT_INDOOR_ROUTE=1`, `…_INDOOR_SIM_STEPS_PER_S=3`, `…_MUTE=1` (no demo hook), poll until a `route start` at or after the first `indoor {action: handover}`, +6 s, one relaunch if no `indoor start` within 25 s. `check_indoor(events)`: `start`; `advanced` indices never decreasing, within 1…n−1 and ending on n−1 (IndoorGuide logs the mirrored index, so two advances in one update repeat it); exactly one `exit` and one `handover`, start ≤ exit ≤ handover; a route start after the handover; no `field_kind`.
  - New args `--jitter-m` (3), `--dropouts` (2), `--gap-s` (20), `--max-lines-per-min` (15), `--report PATH` (default `build/e2e/report.json`). The report entry gains `perturbation`, `route`, `stress` / `indoor` metrics; `report` gains `seed`.

### ios/scripts/stress/ (Step 66)

Purpose: the stress campaign's route data and repeat-run driver; how to run it is `ios/scripts/stress/README.md`.
- `build_routes.py` — OSRM foot routing over OpenStreetMap (`routing.openstreetmap.de/routed-foot`, ODbL) between `CampusPlaces` entrances (`PLACES`, ⚠ keep in sync with `CampusPlaces.swift`). `instruction(step)` (MapKit-style text), `route_from_osrm(resp, route_name, dest_name)` (mirror of `RouteBuilder.waypoints`: a waypoint at each step end, the next step's line, 15 / 20 m, `crossing` on "cross"), `geometry(resp)`, `wrong_turn(resp, points)` (50 m past the first ≥ 60° turn after 80 m, then back), `write_trace` (JSON + GPX). `--offline` rebuilds from `osm/*.json` byte-identically. Outputs `osm/`, `routes/` (cif_isr, isr_grainger, isr_union, cif_siebel), `traces/` (those + `isr_cif` on the bundled route + `isr_union_wrong_turn`). Also prints how far OSM's ISR → CIF strays from the recorded route (218 m on 2026-09-13).
- `campaign.py` — `configs(seeds, only)` (the four base scenarios, `indoor_isr`, every trace × seed), one `e2e.py` process per run, never parallel; reports in `--out/reports/<config>-r<round>.json` (+ `wall_s`, `exit`), console logs in `--out/console/`; `summarize(out)` writes `results.md` (per-run table and a determinism table comparing `signature(entry)` = pass, waypoints, wrist cues, spoken lines with numbers masked, indoor `advanced`); `--summarize` rebuilds only.
...
- **Accessibility** (§6/§7): action buttons have a visible word plus a symbol and a hint; the root tab bar is icon-only with VoiceOver words `Guide` / `Sense` / `Settings` / `Profile`; pills carry `spoken` when terse; cards are `.contain` containers with `.isHeader` titles; rows of the grid are single elements; developer-only views and the HazardsCard live camera image are `accessibilityHidden`; no `accessibilitySortPriority` (visual order is focus order); no colour-only meaning; no text under 15 pt for the user; no custom fonts.
- **Divergence to know**: design.md §6 matches the shipped four icon-only root tabs (Guide / Sense / Settings / Profile, each a scroll of the cards documented above) and the shipped labels ("Where am I", "Repeat", "Next", "Recenter", "Start route to CIF", "Cancel route start", "Stop route"). Profile coverage lives in `CaneKitUITests.testAccessibilityLabelsExist`. What is still out of step: design.md §3 says 48 pt minimum on the watch, while `WatchTheme.swift` uses `WKSpacing.touchTarget = 44`. The tests follow the code; per the file header, the Swift wins.
