...
| `namePeopleEnabled` | `true` (runs only on "Where am I", never in the cue path) | `sceneContext.setPeopleEnabled(_:)`. HazardsCard "Name people ahead" |
...
| `highFrameRateCamera` | `false` (heat untested over a long walk) | `depth.setHighFrameRate(_:)` (re-runs the AR session); also read by `HazardsCard` for the live-view rate. Mount card "60 fps camera (warmer)" |
...
- `fileprivate sessionFailed(code:message:)` — maps `ARError.Code` (`.cameraUnauthorized` → "Camera access denied — enable it in Settings"; `.sensorUnavailable`/`.sensorFailed` → "LiDAR sensor unavailable"; `.unsupportedConfiguration` → "Unsupported AR configuration"; else raw message) into "AR error: …", sets `isRunning = false` and notifies `AppModel` so stale obstacle cues are cleared while GPS guidance remains explicit.
...
- `struct ProfilePage: View` — `@Environment(AppModel.self) private var model`; `@State private var showingEditor = false` (the `EditMedicalIDSheet`), `@State private var showingShareSheet = false`.
...
- `@MainActor @Observable public final class MedicalProfileStore` — owner `AppModel.medicalProfile`. Persists `CKMedicalProfile` under `"opencane_medical_profile"` and the completed-trip count under `"opencane_completed_trips_count"` in `UserDefaults.standard`; `public private(set) var isFetchingPedometer` is true while a `CMPedometer` query is in flight; `save()`, `recordCompletedTrip()`, `refreshMobilityStats()`, private `triggerSync()`. `init` migrates a saved profile in place: blood type `O+` → `A-`, a "Rolling Ball" cane spec → "130 cm · Standard Tip", and (Step 47) the seeded `placeholderPhone` `+1 (555) 234-5678` → the owner's real `+1 (925) 791-8082` (`standardDefault.emergencyContactPhone`); a number the owner typed is never touched. Queries `CMPedometer` data since `startOfDay` via `refreshMobilityStats()`. Tracks completed trips via `recordCompletedTrip()`. Automatically syncs profiles and mobility stats to Supabase.
...
- `struct HazardsCard: View` — `@Environment(AppModel.self) private var model`, `@Environment(\.scenePhase) private var scenePhase` (owns no other state: no `@State` frame, no refresh loop). Body inside `CKCard(title: "Hazards")`, all `CKFont.body` / `textPrimary`. **Ten toggles**, in order:
  ...
  - `Toggle("Name people ahead", isOn: $model.namePeopleEnabled)` — hint "When you ask where am I, says how many people are ahead, which way and how far" (default on; `PeopleAhead`).
...
- Tests: the view decisions are pure — `LiveViewTests` (`LiveView.state`, `BothCameras.state`, `FaceTrackingChange`, `BothCamerasLayout`), `HazardTests`, `SoundAlertsTests`, `HeadNodDetectorTests`. No XCUITest queries this card's strings, so none is a test contract — keep them stable for VoiceOver users anyway.
