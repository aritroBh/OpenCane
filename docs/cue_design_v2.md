...
| 3 | Head **speech** episode ends only after 2 s of trusted clear frames, plus a closing requirement — **shipped Step 52 without the closing requirement** (the episode lives in `CueDecider`; `headEpisodeEndsOnlyAfterTwoSecondsOfTrustedClear`, `aSweepDoesNotHoldTheHeadEpisodeOpen`, `headHeightIsSpokenOncePerEpisode`, `secondHeadLineNeedsUnderSixtyCentimetres`) | `NavSupport.swift` (`CueSpeechPolicy`), `AppModel.handle` | `headSpeechDoesNotRestartAfterABriefClear`, `headSpeechRestartsAfterTwoSecondsClear`, `untrustedFramesDoNotCountTowardClear`, `stationaryOnsetBuzzesButDoesNotSpeak`; rewrite `headEpisodesAreRateLimitedAcrossEpisodes` | low (speech only; haptic unchanged) | S |
...
| 13 | Gravity-corrected metric head band — **shipped Step 51** (`LaneGeometry`, coverage flags; `LaneGeometryTests`, 16) | `LaneMath.swift`, `DepthFrameProcessor` | `headBandUsesMetricHeightAtTenDegreePitch` | high | L |
