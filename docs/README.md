...
the top of [`CHANGELOG.md`](../CHANGELOG.md) for what landed most recently (Step 37 at `076fcaa`; the
three commits after it, up to `d775d4b`, are documentation passes plus `ios/scripts/streetview_stim.py`).
...
at; the top of `CHANGELOG.md` and the code are the newest truth, and the "WHERE WE ARE" block at the
top of `docs/todo.md` is an older (Fri 2026-09-11) snapshot.
...
| [`CHANGELOG.md`](../CHANGELOG.md) | You need to know what landed in each build step, why, how it was reviewed and verified, and its "test on device" list. Newest first. Step numbers 15, 16, 17, 21, 22 and 25 each appear twice, on purpose (see the note under Step 27). |
...
| [`docs/todo.md`](todo.md) | You want to know what is still open. The strict build checklist, including the "Cue design v2 — Steps 35–45" plan; tick it in the same commit. Its "WHERE WE ARE" block at the top is dated Fri 2026-09-11. |
...
<<<<<<< HEAD
| **Testing** | See [`ios/README.md` §5](../ios/README.md#5-testing). Logic tests are in `ios/Logic/Tests/CaneKitLogicTests/` (`make test`, 457 `@Test` annotations in 34 files; Step 37 ran 457/457). UI tests and the screenshot tour are in `ios/CaneKitUITests/` (`make uitest`, `make tour`). GPS-replay end-to-end runs through `ios/scripts/e2e.py` (`make e2e`). Automation is silent under `CANEKIT_MUTE=1` / `CANEKIT_UITEST=1`. |
...
=======
| **Testing** | See [`ios/README.md` §5](../ios/README.md#5-testing). Logic tests are in `ios/Logic/Tests/CaneKitLogicTests/` (`make test`, 378 current `@Test` annotations). UI tests and the screenshot tour are in `ios/CaneKitUITests/` (`make uitest`, `make tour`). GPS-replay end-to-end runs through `ios/scripts/e2e.py` (`make e2e`). |
...
| **What's left to do** | [`todo.md`](todo.md) (the "Cue design v2 — Steps 35–45" section is the live plan). The latest `CHANGELOG.md` entry (Step 37) lists its device checks; older entries keep their historical test notes. |
...
| **What's left to do** | [`todo.md`](todo.md). The latest `CHANGELOG.md` entry (Step 29) lists the sensor-mode interlock device checks; older step entries retain their historical test notes. |
>>>>>>> 21b1717 (Step 29: interlock AR sensor mode restarts during routes)
