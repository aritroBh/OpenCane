...
| **Testing** | See [`ios/README.md` §5](../ios/README.md#5-testing). Logic tests are in `ios/Logic/Tests/CaneKitLogicTests/` (`make test`, 130 tests). UI tests and the screenshot tour are in `ios/CaneKitUITests/` (`make uitest`, `make tour`). GPS-replay end-to-end runs through `ios/scripts/e2e.py` (`make e2e`). |
