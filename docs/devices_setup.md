...
break the walk. This page is the human checklist plus what OpenCane does on its own.
...
2. Settings → Bluetooth → (i) next to the AirPods → **Spatial Audio: Off** and **Head Tracking: Off**
   for the demo. OpenCane does its own spatialisation (HRTF) and its own head tracking; the system
...
3. Settings → Bluetooth → (i) → **Automatic Ear Detection: On** (default). Taking one AirPod out
   pauses system audio; OpenCane keeps running — put it back in.
4. Motion & Fitness permission: OpenCane asks at route start ("uses motion to … track your head
   direction with AirPods"). Tap **Allow**. If you tapped Don't Allow: Settings → Privacy & Security
   → Motion & Fitness → OpenCane → On.
...
2. Developer Mode on the watch (Settings → Privacy & Security → Developer Mode) — needed to install
   the OpenCane watch app from Xcode / the Watch app.
3. Install the watch app: it rides inside the phone app. With **Watch app → General → Automatic App
   ...
   OpenCane → Install.
4. Open **OpenCane on the watch once** and accept the Health permission (it runs a walking workout
...
5. Before the walk: open OpenCane on the watch. The phone's Watch card must say **Reachable** (it says
...
- At route start the phone says "Watch not reachable. Open OpenCane on the watch." if the watch is
...
Nothing in OpenCane talks to the Mac at runtime. The Mac only signs and installs.
...
4. **Phone settings for the walk:** screen stays on by itself while OpenCane is open (the app disables
...
the model finish downloading (Wi-Fi, a few minutes, once). With it off, OpenCane still answers with a
...
| Symptom | Fix |
|---|---|
| "Compass only" with AirPods Pro in | Motion & Fitness permission denied → Settings → Privacy → Motion & Fitness → OpenCane |
...
| Watch says Asleep | Raise wrist / open OpenCane on the watch; check Wrist Detection |
