...
The merged safety line also audits route-time sensor changes and push-to-talk microphone failures:
`SensorModeInterlock` refuses AR session restarts while guiding, and `VoiceInputGuard` fails closed
on route, interruption, permission, and analyzer/engine errors through the existing cue channels.
...
|---|---|---|
| `.safety` (3, top) | "Head height." (`CueSpeechPolicy`); **"Close."** (Step 68, `CueSpeechPolicy.close`; `.safety` since the review round Steps 67–68 — `closeTier` — because at `.obstacle` it queued behind the route intro and its 3 s TTL dropped it: the centre torso tile turns red, < 0.7 m; once per red episode, a new one after 3 s out of the red, ≥ 4 s apart; a NearHold substitution never starts one; while a route or indoor script guides, at every cue level including Quiet — `closeAllowed`; it never cuts "Head height." nor is cut by it, equal priority queues in order of arrival); LiDAR ground hazards "Two meters ahead, drop-off." / "… hole." / "… step up." / "… low obstacle."; route-start failure "Obstacle detection is not ready. Route did not start. Check the camera and reopen OpenCane."; "Obstacle detection did not restart. Close and reopen OpenCane." (2 s after Both cameras off, only if depth is not delivering); **GPS fallback** (Step 42, `failQueuedRouteStart`) "Obstacle detection warming up. Guiding with GPS." when the depth interlock times out and the route starts on GPS alone (§5.3) | 6 s "Head height.", 3 s "Close." and ground hazards, 30 s route-start failure and the depth-did-not-restart line, 15 s the GPS fallback |
| `.nav` (2) | Waypoint `say` lines; "Route to <destination>. <WP1 say>" (Step 68; was "Route started. <route>. First: …"); "Passed <place>. <Next place> in N meters." / "Passed one waypoint."; "Veer left." / "Veer right."; "GPS weak." / "GPS back." (`GPSAnnouncer`, review round Steps 67–68: weak 10 s after GPS turned bad — a fix worse than 20 m or older than 12 s, dated from the 5 s guidance pause, or no fix 20 s into the route — again no sooner than 60 s; back after 10 s good, at most once per 2 min; never spoken indoors, though the bad clock keeps running); "You are close to <place>. Keep going toward it, or press Next to finish."; the arrival trip summary; "Recentered."; "Route stopped."; "Route start canceled."; "No route running."; "OpenCane ready." (the whole launch announcement since Step 67 — no launch menu; the listening tone follows; "help" / "menu" / "options" read `VoiceMenu.helpLine`); "Starting." (after the route-ready bell, Step 65 — the warm-up sentence is gone); "Screen locked. Obstacle warnings off." (ARKit stops with the screen; once per route); **family alerts** "Possible fall detected. Telling your family." (`FallWatcher` episode with "Detect the cane falling" and "Send cane events to family" both on) and the webhook spam guard "Just a moment. Try again in N seconds." (`ActionRateLimit`, 10 s between repeats of Save family emails / Send test event); **refusals** "Both cameras cannot run while a route is starting. Wait for obstacle detection to be ready."; "Both cameras cannot run while a route is guiding you. Stop the route first."; "Head tracking without AirPods cannot change while a route is guiding you. Stop the route first." / "… while a route is starting. Wait for obstacle detection to be ready." (all four prefetched in `AppModel.commonLines`); "This phone cannot show two cameras at once."; **Both cameras** "Both cameras on. Obstacle detection, distance warnings and sign reading are paused." / "Both cameras off. Obstacle detection is restarting." / "Obstacle detection is back." / a failed start "<why> Obstacle detection is back on."; **Cues card** "Quiet cues." / "Standard cues." / "Detailed cues." / "Outdoor mode." / "Indoor mode." (`CueLevel` / `CuePlace.spokenLine`, spoken once per change, prefetched via `CueRules.allSpokenLines`); **voice feature switches** (Shortcuts "Turn a feature on or off") "Obstacle names on." plus `CueRules.namesLimitLine` when the level or place limits names ("Quiet cues name nothing." / "Standard cues name only doors, on a route." / "Indoor mode names nothing."), or "<feature> off. <consequence>"; headphone and channel lines ("<AirPods> connected.", "Headphones disconnected. Beacon paused." with no route, "AirPods disconnected." once per route mid-route, and at route start at most one status line: "Haptics unavailable. Obstacle cues on the watch." / "… will be spoken." — Step 68: "No headphones…", "Watch not reachable…" and "Camera too steep for head-height cover…" are no longer spoken; the Guide card, Mount card and "status" keep them); "Phone is hot. Door and wall names and sign reading paused."; "Location access is off. Turn on Location for OpenCane in Settings to navigate."; "Camera access is off, so obstacle warnings cannot work. Turn on Camera for OpenCane in Settings."; MapKit route lines | 12 s waypoint lines and the arrival hint, 30 s arrival summary, 20 s channel, Location and Camera lines, 15 s "Both cameras on…", 12 s failed-start line, 10 s refusals, "Both cameras off…", "…cannot show two cameras…", voice feature switches and "Phone is hot…", 8 s "Obstacle detection is back." / warm-up / default, 6 s Cues card changes, 10 s "Screen locked…" and the fall line, 4 s "Just a moment…" |
...
**Earcons — calm feedback (Step 65).** Owner, 2026-09-13: "don't over-stimulate the blind person too
much or else they won't listen." Waiting, listening and busy states are short tones, not words
(`Earcon` / `EarconPolicy`, CaneKitLogic, `EarconTests`; played by `EarconPlayer` via
`SpeechQueue.playEarcon`). Every tone ≤ 180 ms, ≤ −12 dBFS, silent edges; synthesized in memory and
played through the existing session (no `setCategory`, hard rule 7); ducks nothing, is not a queued
line, never plays while a `.safety` line plays, silent under automation; a soft haptic
(`UIImpactFeedbackGenerator(.soft)`) goes with every tone except `listening` (it has the 1519 haptic).
Trip log `earcon {name, played, reason}`.

| Event | Earcon (notes) | Words |
|---|---|---|
| Mic opens: press, launch listen, emergency yes/no window | `listening` C5 60 ms → G5 80 ms, −18 dBFS | — |
| Mic opens: follow-up window | `listening` at −8 dB | — |
| Mic closes with words | `heard` C6 45 ms, −20 dBFS | — |
| Mic closes on the app's own voice (self-hear) / superseded turn | — | — |
| Press closes empty | `nothing` E5 70 ms → B4 90 ms, −20/−22 dBFS | "I did not catch that." only on the 2nd empty press in a row |
| Launch / follow-up window closes empty | — | — |
| Cloud turn still thinking (1.5 s, 4 s) | `thinking` A6 20 ms, −28 dBFS | — (was "One moment.") |
| Cloud timeout (8 s since Step 67; was 4 s) or failure | `error` B♭3 55 ms, G3 65 ms, −16 dBFS | "No answer." (was "That is taking too long. Ask again in a moment.") |
| "Where am I" while a description runs | `busy` D5 45 ms ×2, −22 dBFS | — (was "Still describing the previous scene.") |
| Answer ≥ 100 characters | `done` E6 bell 170 ms, −16 dBFS | the answer |
| Route start waiting for depth | `thinking`, at 0 / 2 / 4 s (max 3) | — (was "Obstacle detection warming up. Route will start when it is ready.") |
| Depth ready, route begins | `done` | "Starting." then the intro |

Safety and refusal lines stay words. The indoor draft caveat is "Draft route. Use your cane."
...
- **One voice** (Steps 53–54, owner decision 2026-09-13 "ElevenLabs is the one voice for everything"). Which voice speaks a line is decided once, when it is dispatched, by `VoiceEngineChoice.decide` (CaneKitLogic, `VoiceEngineChoiceTests`), in this order — the **Engine** column of every band above:
  ...
  | any other uncached line — route lines, answers, "No answer.", status clauses | `race`: fetch for at most 2.5 s in total | `race`, then `speech_engine` `race_won` / `race_timeout` / `race_failed` |
...
| GPS worse than 20 m, stale (> 5 s) or absent | pill after 10 s bad accuracy (or at once when stale); spoken "GPS weak." 10 s after GPS turned bad for `GPSAnnouncer` (worse than 20 m, older than 12 s — dated from the 5 s pause — or no fix 20 s into the route) while guiding outdoors, again no sooner than 60 s; "GPS back." after 10 s good, at most once per 2 min (review round Steps 67–68) | — | — | GPS pill ±N M (warning) + GPS WEAK pill |
