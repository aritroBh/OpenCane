...
//  a labelled switch ("Detect drop-offs", "Read signs", "Hazard watch", "Listen for sirens and
//  horns", "Head tracking without AirPods", "Live camera view"); the live view is hidden from
//  VoiceOver (it carries no information a blind user needs) but the front-camera readout is not —
//  it is the only proof a blind walker has that their head direction is being followed.
//
